import { supabaseAdmin } from '../db/client.js';

export const AUDIT_ACTIONS = {
  USER_CREATED: 'user.created',
  USER_LOGGED_IN: 'user.logged_in',
  USER_LOGGED_OUT: 'user.logged_out',
  /** Core: persisted active tenant selection (users.active_organization_id). */
  AUTH_ACTIVE_ORG_SELECTED: 'auth.active_org_selected',
  AUTH_UI_LANGUAGE_SET: 'auth.ui_language_set',
  ORGANIZATION_CREATED: 'organization.created',
  ORGANIZATION_UPDATED: 'organization.updated',
  MEMBERSHIP_CREATED: 'membership.created',
  MEMBERSHIP_UPDATED: 'membership.updated',
  MEMBERSHIP_DELETED: 'membership.deleted',
  ROLE_ASSIGNED: 'role.assigned',
  MODULES_VIEWED: 'modules.viewed',
  SUBSCRIPTION_VIEWED: 'subscription.viewed',
  SUBSCRIPTION_UPDATED: 'subscription.updated',
  MODULE_ACTIVATED: 'module.activated',
  MODULE_DEACTIVATED: 'module.deactivated',
  MODULE_ACTIVATION_FAILED: 'module.activation_failed',
  MODULE_ACCESS_DENIED: 'module.access_denied',
  MODULE_DEPENDENCY_CHECK_FAILED: 'module.dependency_check_failed',
  MODULE_ENTITLEMENT_CHECK_FAILED: 'module.entitlement_check_failed',
  MODULE_INIT_RUN: 'module.init_run',
  MODULE_PLAN_SELECTED: 'module_plan_selected',
  MODULE_SUBSCRIPTION_CREATED: 'module_subscription_created',
  MODULE_SUBSCRIPTION_CHANGED: 'module_subscription_changed',
  MODULE_PURCHASE_STARTED: 'module_purchase_started',
  MODULE_PURCHASE_CONFIRMED: 'module_purchase_confirmed',
  MODULE_ACTIVATION_BLOCKED: 'module_activation_blocked',
  SYSTEM_MODULE_PROTECTED: 'system_module_protected',
  ORGANIZATION_LEGAL_IDENTITY_SET: 'organization_legal_identity_set',
  ORGANIZATION_LEGAL_IDENTITY_LOCKED: 'organization_legal_identity_locked',
  TRIAL_START_REQUESTED: 'trial_start_requested',
  TRIAL_STARTED: 'trial_started',
  TRIAL_START_BLOCKED: 'trial_start_blocked',
  TRIAL_DENIED_ALREADY_USED: 'trial_denied_already_used',
  TRIAL_DENIED_INVALID_IDENTITY: 'trial_denied_invalid_identity',
  TRIAL_EXPIRED: 'trial_expired',
  TRIAL_CONVERTED: 'trial_converted',
  LEGAL_IDENTITY_LOCKED: 'legal_identity_locked',
  MODULE_ACCESS_VIA_TRIAL: 'module_access_via_trial',
  MODULE_ACCESS_DENIED_AFTER_TRIAL: 'module_access_denied_after_trial',
  MODULE_LOADED: 'module.loaded',
  MODULE_REGISTERED: 'module.registered',
  // Phase 3 shared entities
  CLIENT_CREATED: 'client.created',
  CLIENT_UPDATED: 'client.updated',
  CLIENT_ARCHIVED: 'client.archived',
  CLIENT_TAX_ID_CHANGED: 'client.tax_id_changed',
  CLIENT_VIEWED: 'client.viewed',
  CLIENT_CONTACT_ADDED: 'client_contact.added',
  CLIENT_CONTACT_UPDATED: 'client_contact.updated',
  CLIENT_CONTACT_REMOVED: 'client_contact.removed',
  CLIENT_NOTE_ADDED: 'client_note.added',
  CLIENT_NOTE_UPDATED: 'client_note.updated',
  CLIENT_NOTE_DELETED: 'client_note.deleted',
  CLIENT_SENSITIVE_NOTE_VIEWED: 'client_note.sensitive_viewed',
  CLIENT_FILE_ATTACHED: 'client_file.attached',
  CLIENT_FILE_VIEWED: 'client_file.viewed',
  CLIENT_FILE_LINK_REMOVED: 'client_file.link_removed',
  CLIENT_TAG_ADDED: 'client_tag.added',
  CLIENT_TAG_REMOVED: 'client_tag.removed',
  CLIENT_RESTORED: 'client.restored',
  CLIENTS_IMPORT: 'clients.import',
  // Phase 4 Document Hub
  DOCUMENT_CREATED: 'document.created',
  DOCUMENT_UPDATED: 'document.updated',
  DOCUMENT_ARCHIVED: 'document.archived',
  DOCUMENT_VIEWED: 'document.viewed',
  DOCUMENT_SENSITIVE_VIEWED: 'document.sensitive_viewed',
  DOCUMENT_VERSION_UPLOADED: 'document_version.uploaded',
  DOCUMENT_VERSION_VIEWED: 'document_version.viewed',
  DOCUMENT_LINKED: 'document.linked',
  DOCUMENT_UNLINKED: 'document.unlinked',
  DOCUMENT_STATUS_CHANGED: 'document.status_changed',
  DOCUMENT_REMOVED: 'document.removed',
  SETTINGS_FILE_VIEWED: 'settings_file.viewed',
  // Secure file access
  FILE_OPENED: 'file.opened',
  FILE_ACCESS_DENIED: 'file.access.denied',
  ORGANIZATION_SETTINGS_LOGO_UPDATED: 'organization_settings.logo.updated',
  ORGANIZATION_SETTINGS_SIGNATURE_UPDATED: 'organization_settings.signature.updated',
  CLIENT_TAX_SETTINGS_UPDATED: 'client_tax_settings.updated',
  /** Legacy — prefer CLIENT_TAX_PAYMENT_SECRET_REVEALED */
  CLIENT_TAX_FULL_CARD_ACCESSED: 'client_tax_settings.full_card_accessed',
  /** Single-field reveal/copy after secure session (card_number | expiry only; no CVV) */
  CLIENT_TAX_PAYMENT_SECRET_REVEALED: 'client_tax_settings.payment_secret_revealed',
  PAYMENT_CARD_ACCESS_CODE_SENT: 'payment_card.access_code_sent',
  PAYMENT_CARD_ACCESS_VERIFIED: 'payment_card.access_verified',
  /** הגדרות הנה״ח — client workspace accounting tab */
  CLIENT_ACCOUNTING_SETTINGS_UPDATED: 'client_accounting.settings_updated',
  CLIENT_ACCOUNTING_VEHICLES_REPLACED: 'client_accounting.vehicles_replaced',
  /** פרופיל עסקי — הגדרות הנה״ח (accounting first block) */
  CLIENT_ACCOUNTING_BUSINESS_PROFILE_CREATED: 'client_accounting.business_profile_created',
  CLIENT_ACCOUNTING_BUSINESS_PROFILE_UPDATED: 'client_accounting.business_profile_updated',
  ACCOUNTING_SETTINGS_EXPENSES_UPDATED: 'accounting_settings.expenses_updated',
  ACCOUNTING_SETTINGS_INCOME_UPDATED: 'accounting_settings.income_updated',
  ACCOUNTING_SETTINGS_DOCUMENTS_UPDATED: 'accounting_settings.documents_updated',
  ACCOUNTING_SETTINGS_VEHICLES_UPDATED: 'accounting_settings.vehicles_updated',
  ACCOUNTING_SETTINGS_EXPENSE_MANAGEMENT_UPDATED: 'accounting_settings.expense_management_updated',
  /** שכ״ט — עדכון הסכם (שדות הסכם / הנחה / חידוש) דרך פקודה */
  CLIENT_FEE_AGREEMENT_UPDATED: 'client_fee_agreement.updated',
  /** שכר — עדכון דרך פקודות payroll tab */
  CLIENT_PAYROLL_UPDATED: 'client_payroll.updated',
  /** דוח שנתי — פקודות annual tab */
  CLIENT_ANNUAL_REPORT_UPDATED: 'client_annual_report.updated',
  /** מסמכי לקוח — פקודות workspace תיקיות ומסמכים */
  CLIENT_DOCUMENTS_WORKSPACE_UPDATED: 'client_documents_workspace.updated',
  /** שמירת כרטיס "פרטי לקוח" במסך תפעול הלקוח (לא שינוי סכימת audit_log) */
  CLIENT_OPERATIONS_WORKSPACE_PROFILE_UPDATED: 'client_operations.workspace_profile.updated',
  CLIENT_OBLIGATION_COMMAND_EXECUTED: 'client_obligation.command_executed',
  CLIENT_TASK_COMMAND_EXECUTED: 'client_task.command_executed',
  CLIENT_OPERATIONS_NIGHT_PASS_EXECUTED: 'client_operations.night_pass_executed',
  // Accounting Base command layer
  ACCOUNTING_BASE_PERIOD_CREATED: 'accounting_base.period_created',
  ACCOUNTING_BASE_PERIOD_LOCKED: 'accounting_base.period_locked',
  ACCOUNTING_BASE_PERIOD_CLOSED: 'accounting_base.period_closed',
  ACCOUNTING_BASE_ENTRY_CREATED: 'accounting_base.entry_created',
  ACCOUNTING_BASE_ENTRY_DRAFT_UPDATED: 'accounting_base.entry_draft_updated',
  ACCOUNTING_BASE_ENTRY_FINALIZED: 'accounting_base.entry_finalized',
  ACCOUNTING_BASE_ENTRY_ARCHIVED: 'accounting_base.entry_archived',
  ACCOUNTING_BASE_CATEGORY_CREATED: 'accounting_base.category_created',
  ACCOUNTING_BASE_CATEGORY_UPDATED: 'accounting_base.category_updated',
  ACCOUNTING_BASE_CATEGORY_DEACTIVATED: 'accounting_base.category_deactivated',
  ACCOUNTING_BASE_ENTRY_LINKED: 'accounting_base.entry_linked',
  ACCOUNTING_BASE_ENTRY_UNLINKED: 'accounting_base.entry_unlinked',
  ACCOUNTING_BASE_SUMMARY_RECOMPUTED: 'accounting_base.summary_recomputed',
  ACCOUNTING_BASE_PAYMENT_RECORDED: 'accounting_base.payment_recorded',
  ACCOUNTING_BASE_PAYMENT_ALLOCATED_TO_INCOME_DOCUMENT:
    'accounting_base.payment_allocated_to_income_document',
  ACCOUNTING_BASE_INCOME_INVOICE_PARTIALLY_PAID_EVENT_EMITTED:
    'accounting_base.income_invoice_partially_paid_event_emitted',
  ACCOUNTING_BASE_INCOME_INVOICE_PAID_EVENT_EMITTED:
    'accounting_base.income_invoice_paid_event_emitted',
  ACCOUNTING_BASE_PAYMENT_ALLOCATION_FAILED: 'accounting_base.payment_allocation_failed',
  ACCOUNTING_BASE_PAYMENT_ALLOCATION_REVERSED: 'accounting_base.payment_allocation_reversed',
  ACCOUNTING_BASE_CUSTOMER_CREDIT_OPENED: 'accounting_base.customer_credit_opened',
  ACCOUNTING_BASE_PAYMENT_ALLOCATION_REVERSAL_FAILED:
    'accounting_base.payment_allocation_reversal_failed',
  INCOME_DOCUMENT_PAYMENT_ORCHESTRATION_STARTED: 'income.document_payment_orchestration_started',
  INCOME_DOCUMENT_PAYMENT_AB_LINKED: 'income.document_payment_ab_linked',
  INCOME_DOCUMENT_PAYMENT_RECEIPT_DRAFT_CREATED: 'income.document_payment_receipt_draft_created',
  INCOME_DOCUMENT_PAYMENT_RECEIPT_ISSUED: 'income.document_payment_receipt_issued',
  INCOME_DOCUMENT_PAYMENT_RECEIPT_LINKED: 'income.document_payment_receipt_linked',
  INCOME_DOCUMENT_PAYMENT_ORCHESTRATION_FAILED: 'income.document_payment_orchestration_failed',
  // Phase 7 Country Pack - platform owner access/security base
  OWNER_LOGIN_SUCCESS: 'owner_login_success',
  OWNER_LOGIN_FAILED: 'owner_login_failed',
  OWNER_ACCESS_KEY_ROTATED: 'owner_access_key_rotated',
  OWNER_PHONE_UPDATED: 'owner_phone_updated',
  OWNER_EMAIL_UPDATED: 'owner_email_updated',
  OWNER_PASSWORD_UPDATED: 'owner_password_updated',
  /** Platform owner password recovery (SMS OTP); never log OTP in payload */
  OWNER_PASSWORD_RECOVERY_EMAIL_REJECTED: 'owner_password_recovery.email_rejected',
  OWNER_PASSWORD_RECOVERY_FAILED: 'owner_password_recovery.failed',
  OWNER_PASSWORD_RECOVERY_OTP_SENT: 'owner_password_recovery.otp_sent',
  OWNER_PASSWORD_RECOVERY_OTP_FAILED: 'owner_password_recovery.otp_failed',
  OWNER_PASSWORD_RECOVERY_OTP_VERIFIED: 'owner_password_recovery.otp_verified',
  OWNER_PASSWORD_RECOVERY_COMPLETED: 'owner_password_recovery.completed',
  OWNER_SECURITY_CHECK_FAILED: 'owner_security_check_failed',
  OWNER_COUNTRY_LEGAL_ACCESS_REQUEST_CREATED: 'owner_country_legal_access.request_created',
  OWNER_COUNTRY_LEGAL_INVITATION_CREATED: 'owner_country_legal_access.invitation_created',
  OWNER_COUNTRY_LEGAL_ACCESS_REQUEST_APPROVED: 'owner_country_legal_access.request_approved',
  OWNER_COUNTRY_LEGAL_ACCESS_REQUEST_REJECTED: 'owner_country_legal_access.request_rejected',
  OWNER_COUNTRY_LEGAL_ASSIGNMENT_CREATED: 'owner_country_legal_access.assignment_created',
  OWNER_COUNTRY_LEGAL_COUNTRY_ADDED: 'owner_country_legal_access.country_added',
  OWNER_COUNTRY_LEGAL_COUNTRY_REMOVED: 'owner_country_legal_access.country_removed',
  OWNER_COUNTRY_LEGAL_CAPABILITY_CHANGED: 'owner_country_legal_access.capability_changed',
  OWNER_COUNTRY_LEGAL_ACTIVATE_GRANTED: 'owner_country_legal_access.activate_granted',
  OWNER_COUNTRY_LEGAL_ACTIVATE_REMOVED: 'owner_country_legal_access.activate_removed',
  OWNER_COUNTRY_LEGAL_ASSIGNMENT_SUSPENDED: 'owner_country_legal_access.assignment_suspended',
  OWNER_COUNTRY_LEGAL_ASSIGNMENT_REVOKED: 'owner_country_legal_access.assignment_revoked',
  COUNTRY_CREATED: 'country_created',
  COUNTRY_DISABLED: 'country_disabled',
  COUNTRY_ENABLED: 'country_enabled',
  COUNTRY_LOCALIZATION_UPDATED: 'country_localization_updated',
  COUNTRY_PACK_CREATED: 'country_pack_created',
  COUNTRY_PACK_ENABLED: 'country_pack_enabled',
  COUNTRY_PACK_DISABLED: 'country_pack_disabled',
  RULESET_CREATED: 'ruleset_created',
  RULESET_METADATA_UPDATED: 'ruleset_metadata_updated',
  RULESET_ACTIVATED: 'ruleset_activated',
  RULESET_DEACTIVATED: 'ruleset_deactivated',
  ORGANIZATION_COUNTRY_PACK_ASSIGNED: 'organization_country_pack_assigned',
  ACTIVE_RULESET_CHANGED: 'active_ruleset_changed',
  ORGANIZATION_COUNTRY_SETTINGS_UPDATED: 'organization_country_settings_updated',
  LEGAL_VALUE_CREATED: 'legal_value_created',
  LEGAL_VALUE_METADATA_UPDATED: 'legal_value_metadata_updated',
  LEGAL_VALUE_VERSION_CREATED: 'legal_value_version_created',
  LEGAL_VALUE_VERSION_UPDATED: 'legal_value_version_updated',
  LEGAL_VALUE_VERSION_ACTIVATED: 'legal_value_version_activated',
  LEGAL_VALUE_VERSION_DEACTIVATED: 'legal_value_version_deactivated',
  LEGAL_VALUE_VERSION_AUTHORITY_PINNED: 'legal_value_version_authority_pinned',
  LEGAL_VALUE_VERSION_AUTHORITY_UNPINNED: 'legal_value_version_authority_unpinned',
  TAX_SOURCE_CREATED: 'tax_source_created',
  TAX_RULE_CREATED: 'tax_rule_created',
  TAX_RULE_VERSION_CREATED: 'tax_rule_version_created',
  TAX_RULE_VERSION_DRAFT_UPDATED: 'tax_rule_version_draft_updated',
  TAX_SOURCE_ACTIVATED: 'tax_source_activated',
  TAX_SOURCE_RETIRED: 'tax_source_retired',
  TAX_SOURCE_METADATA_UPDATED: 'tax_source_metadata_updated',
  TAX_RULE_METADATA_UPDATED: 'tax_rule_metadata_updated',
  TAX_RULE_VERSION_SOURCE_PINNED: 'tax_rule_version_source_pinned',
  TAX_RULE_VERSION_SOURCE_UNPINNED: 'tax_rule_version_source_unpinned',
  TAX_RULE_VERSION_LEGAL_VALUE_BOUND: 'tax_rule_version_legal_value_bound',
  TAX_RULE_VERSION_LEGAL_VALUE_UNBOUND: 'tax_rule_version_legal_value_unbound',
  TAX_RULE_RELATIONSHIP_CREATED: 'tax_rule_relationship_created',
  TAX_RULE_RELATIONSHIP_DELETED: 'tax_rule_relationship_deleted',
  TAX_RULE_UNRESOLVED_LEGAL_REFERENCE_CREATED: 'tax_rule_unresolved_legal_reference_created',
  TAX_RULE_UNRESOLVED_LEGAL_REFERENCE_UPDATED: 'tax_rule_unresolved_legal_reference_updated',
  TAX_RULE_UNRESOLVED_LEGAL_REFERENCE_ACCEPTED: 'tax_rule_unresolved_legal_reference_accepted',
  TAX_RULE_UNRESOLVED_LEGAL_REFERENCE_DISCARDED: 'tax_rule_unresolved_legal_reference_discarded',
  TAX_RULE_UNRESOLVED_LEGAL_REFERENCE_RESOLVED: 'tax_rule_unresolved_legal_reference_resolved',
  TAX_RULE_VERSION_ACTIVATED: 'tax_rule_version_activated',
  TAX_RULE_VERSION_RETIRED: 'tax_rule_version_retired',
  TAX_RULE_VERSION_EFFECTIVE_TO_CLOSED: 'tax_rule_version_effective_to_closed',
  TAX_RULE_VERSION_SUPERSEDED: 'tax_rule_version_superseded',
  TAX_DOMAIN_CREATED: 'tax_domain_created',
  TAX_DOMAIN_METADATA_UPDATED: 'tax_domain_metadata_updated',
  TAX_LEGAL_NODE_KIND_CREATED: 'tax_legal_node_kind_created',
  TAX_LEGAL_NODE_CREATED: 'tax_legal_node_created',
  TAX_LEGAL_NODE_METADATA_UPDATED: 'tax_legal_node_metadata_updated',
  TAX_RULE_LEGAL_NODE_LINKED: 'tax_rule_legal_node_linked',
  TAX_RULE_LEGAL_NODE_UNLINKED: 'tax_rule_legal_node_unlinked',
  LEGAL_TRAINING_DOCUMENT_UPLOADED: 'legal_training_document_uploaded',
  LEGAL_TRAINING_EXTRACTION_STARTED: 'legal_training_extraction_started',
  LEGAL_TRAINING_PAGE_RETRIED: 'legal_training_page_retried',
  LEGAL_TRAINING_STRUCTURE_REBUILT: 'legal_training_structure_rebuilt',
  LEGAL_TRAINING_LAYOUT_EXTRACTION_STARTED: 'legal_training_layout_extraction_started',
  LEGAL_TRAINING_STRUCTURE_REBUILT_WITH_LAYOUT: 'legal_training_structure_rebuilt_with_layout',
  LEGAL_TRAINING_STRUCTURE_RUN_ACTIVATED: 'legal_training_structure_run_activated',
  LEGAL_TRAINING_STRUCTURE_RUN_FAILED: 'legal_training_structure_run_failed',
  LEGAL_TRAINING_CANDIDATE_EDITED: 'legal_training_candidate_edited',
  LEGAL_TRAINING_CANDIDATE_ACCEPTED: 'legal_training_candidate_accepted',
  LEGAL_TRAINING_CANDIDATE_REJECTED: 'legal_training_candidate_rejected',
  LEGAL_TRAINING_CANONICAL_DRAFT_CREATED: 'legal_training_canonical_draft_created',
  LEGAL_TRAINING_DOCUMENT_OPENED: 'legal_training_document_opened',
  LEGAL_TRAINING_LEGAL_TEXT_DRAFT_CREATED: 'legal_training_legal_text_draft_created',
  LEGAL_TRAINING_LEGAL_TEXT_DRAFT_TEXT_UPDATED: 'legal_training_legal_text_draft_text_updated',
  LEGAL_TRAINING_LEGAL_TEXT_DRAFT_IDENTITY_UPDATED: 'legal_training_legal_text_draft_identity_updated',
  LEGAL_TRAINING_LEGAL_TEXT_DRAFT_REPARENTED: 'legal_training_legal_text_draft_reparented',
  LEGAL_TRAINING_LEGAL_TEXT_DRAFT_BOUNDARY_SET: 'legal_training_legal_text_draft_boundary_set',
  LEGAL_TRAINING_LEGAL_TEXT_DRAFT_RESET_TO_SOURCE: 'legal_training_legal_text_draft_reset_to_source',
  LEGAL_TRAINING_LEGAL_TEXT_DRAFT_REVIEW_STATUS_SET: 'legal_training_legal_text_draft_review_status_set',
  LEGAL_TRAINING_LEGAL_TEXT_DRAFTS_PREPARED: 'legal_training_legal_text_drafts_prepared',
  LEGAL_TRAINING_MANUAL_LEGAL_TEXT_DRAFT_CREATED: 'legal_training_manual_legal_text_draft_created',
  LEGAL_TRAINING_OWNER_STRUCTURE_COMPLETENESS_CONFIRMED: 'legal_training_owner_structure_completeness_confirmed',
  LEGAL_TRAINING_OWNER_STRUCTURE_COMPLETENESS_RETRACTED: 'legal_training_owner_structure_completeness_retracted',
  LEGAL_TRAINING_TAX_KNOWLEDGE_PROPOSAL_CREATED: 'legal_training_tax_knowledge_proposal_created',
  LEGAL_TRAINING_TAX_KNOWLEDGE_PROPOSAL_GENERATED: 'legal_training_tax_knowledge_proposal_generated',
  LEGAL_TRAINING_TAX_KNOWLEDGE_PROPOSAL_REVIEW_STATUS_SET: 'legal_training_tax_knowledge_proposal_review_status_set',
  LEGAL_TRAINING_TAX_KNOWLEDGE_PROPOSAL_CORRECTED: 'legal_training_tax_knowledge_proposal_corrected',
  LEGAL_TRAINING_TAX_KNOWLEDGE_PROPOSAL_OWNER_PRESENTATION_ENSURED:
    'legal_training_tax_knowledge_proposal_owner_presentation_ensured',
  AI_PROVIDER_CREATED: 'ai_provider_created',
  AI_PROVIDER_CONFIGURATION_CHANGED: 'ai_provider_configuration_changed',
  AI_PROVIDER_CREDENTIAL_SET: 'ai_provider_credential_set',
  AI_PROVIDER_CREDENTIAL_REMOVED: 'ai_provider_credential_removed',
  AI_PROVIDER_ENABLED: 'ai_provider_enabled',
  AI_PROVIDER_DISABLED: 'ai_provider_disabled',
  AI_PROVIDER_ROUTING_CHANGED: 'ai_provider_routing_changed',
  AI_PROVIDER_CONNECTION_TESTED: 'ai_provider_connection_tested',
  TAX_STRATEGY_CREATED: 'tax_strategy_created',
  TAX_STRATEGY_METADATA_UPDATED: 'tax_strategy_metadata_updated',
  TAX_STRATEGY_EXCLUSIVE_GROUP_CREATED: 'tax_strategy_exclusive_group_created',
  TAX_STRATEGY_EXCLUSIVE_GROUP_UPDATED: 'tax_strategy_exclusive_group_updated',
  TAX_STRATEGY_VERSION_CREATED: 'tax_strategy_version_created',
  TAX_STRATEGY_VERSION_DRAFT_UPDATED: 'tax_strategy_version_draft_updated',
  TAX_STRATEGY_VERSION_ACTIVATED: 'tax_strategy_version_activated',
  TAX_STRATEGY_VERSION_RETIRED: 'tax_strategy_version_retired',
  TAX_STRATEGY_VERSION_EFFECTIVE_TO_CLOSED: 'tax_strategy_version_effective_to_closed',
  TAX_STRATEGY_VERSION_SUPERSEDED: 'tax_strategy_version_superseded',
  TAX_STRATEGY_RULE_PINNED: 'tax_strategy_rule_pinned',
  TAX_STRATEGY_RULE_UNPINNED: 'tax_strategy_rule_unpinned',
  TAX_STRATEGY_CALCULATION_PINNED: 'tax_strategy_calculation_pinned',
  TAX_STRATEGY_CALCULATION_UNPINNED: 'tax_strategy_calculation_unpinned',
  TAX_FACT_DEFINITION_CREATED: 'tax_fact_definition_created',
  TAX_FACT_DEFINITION_METADATA_UPDATED: 'tax_fact_definition_metadata_updated',
  TAX_FACT_DEFINITION_ACTIVATED: 'tax_fact_definition_activated',
  TAX_FACT_DEFINITION_RETIRED: 'tax_fact_definition_retired',
  TAX_FACT_DEFINITION_VERSION_CREATED: 'tax_fact_definition_version_created',
  TAX_FACT_DEFINITION_VERSION_DRAFT_UPDATED: 'tax_fact_definition_version_draft_updated',
  TAX_FACT_DEFINITION_VERSION_ACTIVATED: 'tax_fact_definition_version_activated',
  TAX_FACT_DEFINITION_VERSION_RETIRED: 'tax_fact_definition_version_retired',
  TAX_FACT_DEFINITION_VERSION_EFFECTIVE_TO_CLOSED: 'tax_fact_definition_version_effective_to_closed',
  TAX_FACT_ENUM_OPTION_ADDED: 'tax_fact_enum_option_added',
  TAX_FACT_ENUM_OPTION_UPDATED: 'tax_fact_enum_option_updated',
  TAX_FACT_ENUM_OPTION_REMOVED: 'tax_fact_enum_option_removed',
  TAX_FACT_PRESENTATION_CREATED: 'tax_fact_presentation_created',
  TAX_FACT_PRESENTATION_UPDATED: 'tax_fact_presentation_updated',
  TAX_FACT_PRESENTATION_DELETED: 'tax_fact_presentation_deleted',
  TAX_ADVISORY_CASE_CREATED: 'tax_advisory.case_created',
  TAX_ADVISORY_FACT_SET: 'tax_advisory.fact_set',
  TAX_ADVISORY_FACT_CLEARED: 'tax_advisory.fact_cleared',
  TAX_ADVISORY_CASE_ARCHIVED: 'tax_advisory.case_archived',
  OWNER_NOTE_UPDATED: 'owner_note_updated',
  USAGE_HINT_UPDATED: 'usage_hint_updated',
  MODULE_SCOPE_UPDATED: 'module_scope_updated',
  MODULE_PRICE_UPDATED: 'module_price_updated',
  PACKAGE_PRICE_UPDATED: 'package_price_updated',
  MODULE_PLAN_CREATED: 'module_plan_created',
  OWNER_TRIAL_EXTENDED: 'owner_trial_extended',
  OWNER_MODULE_ACCESS_ACTIVATED: 'owner_module_access_activated',
  OWNER_PRICING_ADJUSTMENT_CREATED: 'owner_pricing_adjustment_created',
  OWNER_PRICING_ADJUSTMENT_CANCELLED: 'owner_pricing_adjustment_cancelled',
  OWNER_FREE_ACCESS_GRANTED: 'owner_free_access_granted',
  /** INV-13A — Owner Invoice Document Builder layout lifecycle. */
  OWNER_INVOICE_LAYOUT_DRAFT_CREATED: 'owner_invoice_layout.draft_created',
  OWNER_INVOICE_LAYOUT_SECTION_MOVED: 'owner_invoice_layout.section_moved',
  OWNER_INVOICE_LAYOUT_SECTION_RESIZED: 'owner_invoice_layout.section_resized',
  OWNER_INVOICE_LAYOUT_FIELD_MOVED: 'owner_invoice_layout.field_moved',
  OWNER_INVOICE_LAYOUT_FIELD_PLACED: 'owner_invoice_layout.field_placed',
  OWNER_INVOICE_LAYOUT_FIELD_VISIBILITY_SET: 'owner_invoice_layout.field_visibility_set',
  OWNER_INVOICE_LAYOUT_TABLE_COLUMN_SET: 'owner_invoice_layout.table_column_set',
  OWNER_INVOICE_LAYOUT_SECTION_LOCK_SET: 'owner_invoice_layout.section_lock_set',
  OWNER_INVOICE_LAYOUT_VERSION_PUBLISHED: 'owner_invoice_layout.version_published',
  OWNER_INVOICE_LAYOUT_VERSION_ARCHIVED: 'owner_invoice_layout.version_archived',
  EMAIL_PROVIDER_CONFIG_SAVED: 'email_provider_config_saved',
  PLATFORM_PUBLIC_URL_SAVED: 'platform_public_url_saved',
  DOCFLOW_INVITATION_CREATED: 'invitation_created',
  DOCFLOW_INVITATION_ACCEPTED: 'invitation_accepted',
  DOCFLOW_INVITATION_REVOKED: 'invitation_revoked',
  DOCFLOW_PORTAL_ACCESS_REVOKED: 'portal_access_revoked',
  DOCFLOW_SESSION_CREATED: 'session_created',
  DOCFLOW_SESSION_REVOKED: 'session_revoked',
  DOCFLOW_THREAD_CREATED: 'thread_created',
  DOCFLOW_THREAD_STATUS_CHANGED: 'thread_status_changed',
  DOCFLOW_THREAD_ASSIGNMENT_CHANGED: 'thread_assignment_changed',
  DOCFLOW_THREAD_DEADLINE_SET: 'thread_deadline_set',
  DOCFLOW_MESSAGE_CREATED: 'message_created',
  DOCFLOW_ATTACHMENT_ADDED: 'message_attachment_added',
  DOCFLOW_THREAD_READ_MARKED_OFFICE: 'thread_read_marked_office',
  DOCFLOW_THREAD_READ_MARKED_CLIENT: 'thread_read_marked_client',
  DOCFLOW_REQUEST_TEMPLATE_SAVED: 'docflow_request_template_saved',
  DOCFLOW_REQUEST_TEMPLATE_ARCHIVED: 'docflow_request_template_archived',
  DOCFLOW_DOCUMENT_REQUEST_SENT: 'docflow_document_request_sent',
  /** DocFlow → Work Engine bridge: intake threw or returned hard failure before audit trail (rare). */
  DOCFLOW_WORK_ENGINE_BRIDGE_INTAKE_FAILED: 'docflow.work_engine_bridge.intake_failed',
  // Work Engine (Stage 2 foundation; canonical workflow memory).
  WORK_ITEM_CREATED: 'work_engine.work_item_created',
  WORK_ITEM_ASSIGNED: 'work_engine.work_item_assigned',
  WORK_ITEM_PICKED_UP: 'work_engine.work_item_picked_up',
  WORK_ITEM_TRANSFERRED: 'work_engine.work_item_transferred',
  WORK_ITEM_CLAIMED: 'work_engine.work_item_claimed',
  WORK_ITEM_CLAIM_RELEASED: 'work_engine.work_item_claim_released',
  /** Stage 10 Phase 2 — review workflow (action review_requested; state review_pending). */
  WORK_ITEM_REVIEW_REQUESTED: 'work_engine.review_requested',
  WORK_ITEM_REVIEW_APPROVED: 'work_engine.review_approved',
  WORK_ITEM_REVIEW_REJECTED: 'work_engine.review_rejected',
  WORK_ITEM_STATE_CHANGED: 'work_engine.work_item_state_changed',
  /** Stage 10 Phase 3A — operational SLA obligations (not legal deadlines). */
  WORK_ITEM_SLA_OBLIGATION_STARTED: 'work_engine.work_item_sla_obligation_started',
  WORK_ITEM_SLA_OBLIGATION_MET: 'work_engine.work_item_sla_obligation_met',
  WORK_ITEM_SLA_OBLIGATION_BREACHED: 'work_engine.work_item_sla_obligation_breached',
  WORK_ITEM_SLA_STATUS_RECOMPUTED: 'work_engine.work_item_sla_status_recomputed',
  REMINDER_CANDIDATE_CREATED: 'work_engine.reminder_candidate_created',
  REMINDER_CANDIDATE_EDITED: 'work_engine.reminder_candidate_edited',
  REMINDER_CANDIDATE_APPROVED: 'work_engine.reminder_candidate_approved',
  REMINDER_CANDIDATE_SENT: 'work_engine.reminder_candidate_sent',
  REMINDER_CANDIDATE_CANCELLED: 'work_engine.reminder_candidate_cancelled',
  REMINDER_CANDIDATE_SNOOZED: 'work_engine.reminder_candidate_snoozed',
  REMINDER_DELIVERY_INTENT_CREATED: 'work_engine.reminder_delivery_intent_created',
  REMINDER_DELIVERY_FAILED: 'work_engine.reminder_delivery_failed',
  /** Stage 10 Phase 3C-1 — escalation (operational ownership, not reminders). */
  WORK_ITEM_ESCALATED: 'work_engine.work_item_escalated',
  WORK_ITEM_ESCALATION_ACKNOWLEDGED: 'work_engine.work_item_escalation_acknowledged',
  WORK_ITEM_ESCALATION_RESOLVED: 'work_engine.work_item_escalation_resolved',
  WORK_ITEM_ESCALATION_OWNER_REASSIGNED: 'work_engine.work_item_escalation_owner_reassigned',
  WORK_ITEM_AUTO_ESCALATION_SKIPPED: 'work_engine.work_item_auto_escalation_skipped',
  WORK_ENGINE_SCHEDULER_RUN: 'work_engine.scheduler_run',
  WORK_ITEM_DEADLINE_SET: 'work_engine.work_item_deadline_set',
  WORK_ITEM_OVERRIDE_APPLIED: 'work_engine.work_item_override_applied',
  WORK_EVENT_APPENDED: 'work_engine.work_event_appended',
  // Work Engine Stage 3A — event intake outcomes.
  WORK_EVENT_RECEIVED: 'work_engine.event_received',
  WORK_EVENT_DUPLICATE_SKIPPED: 'work_engine.event_duplicate_skipped',
  WORK_ITEM_AUTO_CREATED_FROM_EVENT: 'work_engine.work_item_auto_created_from_event',
  WORK_ITEM_EXISTING_REUSED_FROM_EVENT: 'work_engine.work_item_existing_reused_from_event',
  WORK_EVENT_ACCEPTED_PENDING_MAPPING: 'work_engine.event_accepted_pending_mapping',
  // Work Engine Stage 3B — explicit mapping layer outcomes (allowlist).
  // Backend never invents a mapping; only allowlisted event_type values resolve.
  WORK_EVENT_MAPPING_RESOLVED: 'work_engine.event_mapping_resolved',
  WORK_EVENT_MAPPING_PENDING: 'work_engine.event_mapping_pending',
  /** Income INC-1b — persisted issuer workspace context selection. */
  INCOME_ISSUER_CONTEXT_SELECTED: 'income.issuer_context_selected',
  /** Income INC-2 — customers, items, drafts (operational only). */
  INCOME_CUSTOMER_CREATED: 'income.customer_created',
  INCOME_CUSTOMER_UPDATED: 'income.customer_updated',
  INCOME_ONE_TIME_CUSTOMER_CREATED: 'income.one_time_customer_created',
  INCOME_ITEM_CREATED: 'income.item_created',
  INCOME_DOCUMENT_DRAFT_CREATED: 'income.document_draft_created',
  INCOME_DOCUMENT_DRAFT_UPDATED: 'income.document_draft_updated',
  INCOME_DOCUMENT_DRAFT_RESUMED: 'income.draft_resumed',
  INCOME_DOCUMENT_PREVIEW_GENERATED: 'income.document_preview_generated',
  INCOME_DOCUMENT_DISCOUNT_UPDATED: 'income.document_discount_updated',
  INCOME_DOCUMENT_ALLOCATION_NUMBER_UPDATED: 'income.document_allocation_number_updated',
  INCOME_DOCUMENT_BRANDING_UPDATED: 'income.document_branding_updated',
  INCOME_DOCUMENT_BRANDING_LOGO_UPLOADED: 'income.document_branding_logo_uploaded',
  INCOME_DOCUMENT_BRANDING_SIGNATURE_UPLOADED: 'income.document_branding_signature_uploaded',
  INCOME_DOCUMENT_DRAFT_CANCELLED: 'income.document_draft_cancelled',
  INCOME_DOCUMENT_CONVERSION_CREATED: 'income.document_conversion_created',
  INCOME_DOCUMENT_CONVERSION_TARGET_LINKED: 'income.document_conversion_target_linked',
  INCOME_DOCUMENT_CONVERSION_LINEAGE_CONFLICT: 'income.document_conversion_lineage_conflict',
  INCOME_PRELIMINARY_DOCUMENT_EDITED: 'income.preliminary_document_edited',
  INCOME_PRELIMINARY_DOCUMENT_CANCELLED: 'income.preliminary_document_cancelled',
  /** Income INC-3.5 — issuer profile projection synced from Core org business profile. */
  INCOME_ISSUER_PROFILE_SYNCED: 'income.issuer_profile_synced',
  /** Income INC-4 — issued document + numbering allocation. */
  INCOME_DOCUMENT_ISSUED: 'income.document_issued',
  /** Income INV-2.2C — issue + email delivery orchestration (retainer preview). */
  INCOME_DOCUMENT_ISSUE_AND_SEND: 'income.document_issue_and_send',
  INCOME_DOCUMENT_NUMBER_ALLOCATED: 'income.document_number_allocated',
  /** Income INC-5 — Accounting Base posting lifecycle for issued documents. */
  INCOME_ACCOUNTING_POSTING_STARTED: 'income.accounting_posting_started',
  INCOME_ACCOUNTING_POSTING_SUCCEEDED: 'income.accounting_posting_succeeded',
  INCOME_ACCOUNTING_POSTING_FAILED: 'income.accounting_posting_failed',
  INCOME_ACCOUNTING_POSTING_NOT_REQUIRED: 'income.accounting_posting_not_required',
  /** Income INC-6 — PDF render lifecycle. */
  INCOME_PDF_RENDER_STARTED: 'income.pdf_render_started',
  INCOME_PDF_RENDER_SUCCEEDED: 'income.pdf_render_succeeded',
  INCOME_PDF_RENDER_FAILED: 'income.pdf_render_failed',
  INCOME_PDF_DOWNLOADED: 'income.pdf_downloaded',
  /** Income INV-1 — document email delivery lifecycle. */
  INCOME_DOCUMENT_EMAIL_SENT: 'income.document_email_sent',
  INCOME_DOCUMENT_EMAIL_SEND_FAILED: 'income.document_email_send_failed',
  INCOME_DOCUMENT_DOCFLOW_SEND_ATTEMPTED: 'income.document_docflow_send_attempted',
  INCOME_DOCUMENT_DOCFLOW_POSTED: 'income.document_docflow_posted',
  INCOME_DOCUMENT_DOCFLOW_FAILED: 'income.document_docflow_failed',
  /** Income INC-8 — Work Engine intake bridge failure (additive). */
  INCOME_WORK_ENGINE_BRIDGE_INTAKE_FAILED: 'income.work_engine_bridge.intake_failed',
  /** Income — recurring document profile (retainer template). */
  INCOME_RECURRING_DOCUMENT_PROFILE_CREATED: 'income.recurring_document_profile_created',
  INCOME_RECURRING_DOCUMENT_PROFILE_UPDATED: 'income.recurring_document_profile_updated',
  INCOME_RECURRING_PROFILE_DRAFT_CREATED: 'income_recurring_profile_draft_created',
  INCOME_RECURRING_PROFILE_GENERATION_FAILED: 'income_recurring_profile_generation_failed',
  INCOME_RECURRING_PROFILE_ADVANCED: 'income_recurring_profile_advanced',
  INCOME_RECURRING_PROFILE_CYCLE_CREATED: 'income_recurring_profile_cycle_created',
  INCOME_RECURRING_PROFILE_CYCLE_FAILED: 'income_recurring_profile_cycle_failed',
  INCOME_RECURRING_PROFILE_CYCLE_APPROVED: 'income_recurring_profile_cycle_approved',
  INCOME_RECURRING_PROFILE_CYCLE_DRAFT_REVIEW_OPENED: 'income_recurring_profile_cycle_draft_review_opened',
  INCOME_RECURRING_CYCLE_OVERRIDE_OPENED: 'income_recurring_cycle_override_opened',
  INCOME_RECURRING_CYCLE_OVERRIDE_SAVED: 'income_recurring_cycle_override_saved',
  INCOME_RECURRING_CYCLE_OVERRIDE_DELETED: 'income_recurring_cycle_override_deleted',
} as const;

type AuditPayload = Record<string, unknown>;

export async function writeAudit(params: {
  organizationId: string | null;
  actorUserId: string | null;
  actorSessionId?: string | null;
  moduleCode?: string | null;
  entityType: string;
  entityId?: string | null;
  action: string;
  payload?: AuditPayload | null;
  ipAddress?: string | null;
  userAgent?: string | null;
}): Promise<void> {
  await supabaseAdmin.from('audit_log').insert({
    organization_id: params.organizationId,
    actor_user_id: params.actorUserId,
    actor_session_id: params.actorSessionId ?? null,
    module_code: params.moduleCode ?? null,
    entity_type: params.entityType,
    entity_id: params.entityId ?? null,
    action: params.action,
    payload_json: params.payload ?? null,
    ip_address: params.ipAddress ?? null,
    user_agent: params.userAgent ?? null,
  });
}
