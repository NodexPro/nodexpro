...
export function slowAggregateThresholdMs(): number {
  const n = Number(process.env.SLOW_AGGREGATE_THRESHOLD_MS ?? '3000');
  return Number.isFinite(n) && n > 0 ? n : 3000;
}

function sanitizeObsPayload(payload: Record<string, unknown>): Record<string, unknown> {
...
export function logCommandReceived(params: CommandObsParams): number {
  console.info(
    '[command_received]',
    sanitizeObsPayload({
...
export function logCommandCompleted(
  params: CommandObsParams & { started_ms: number; income_document_id?: string | null },
): void {
  console.info(
    '[command_completed]',
    sanitizeObsPayload({
...
export function logCommandFailed(
  params: CommandObsParams & { started_ms: number; error: unknown },
): void {
  console.error(
    '[command_failed]',
    sanitizeObsPayload({
...
export function emitSlowAggregateWarning(payload: {
  ...
  if (payload.duration_ms < slowAggregateThresholdMs()) return;
...
export async function withCriticalCommandObs<T>(
  params: CommandObsParams & { enabled: boolean },
  run: () => Promise<T>,
  onSuccess?: (result: T) => { income_document_id?: string | null },
): Promise<T> {
  if (!params.enabled) return run();
