...
export interface IncomeRenderLineRow {
  description: string;
...
export interface IncomeRenderPartyBlock {
  legal_name: string;
  display_name: string;
  tax_id: string | null;
  phone: string | null;
...
export interface IncomeDocumentRenderSnapshot {
  captured_at: string;
  issuer: IncomeRenderPartyBlock;
  customer: {
    display_name: string;
    tax_id: string | null;
    phone: string | null;
  ...
  /** INC-8.5 — PDF layout: issuer left, recipient right (Hebrew RTL). */
  issuer_block: IncomeRenderPartyBlock;
  recipient_block: {
    display_name: string;
    tax_id: string | null;
    phone: string | null;
...
function parseAddressLines(addressJson: unknown): string[] {
  if (!addressJson || typeof addressJson !== 'object' || Array.isArray(addressJson)) return [];
  const a = addressJson as Record<string, unknown>;
  const lines: string[] = [];
  for (const key of ['line1', 'line_1', 'address_line_1', 'street']) {
    if (a[key]) lines.push(String(a[key]).trim());
  }
...
function parseLineRow(raw: unknown): IncomeRenderLineRow {
...
export function buildIncomeDocumentRenderSnapshot(input: {
  ...
  organization_footer_note?: string | null;
}): IncomeDocumentRenderSnapshot {
  ...
  const subtotal =
    typeof input.totals_snapshot_json?.subtotal_reference === 'number'
      ? (input.totals_snapshot_json.subtotal_reference as number)
      : lines.reduce((sum, l) => sum + (l.amount_reference ?? 0), 0) || null;

  const footer =
    input.organization_footer_note?.trim() ||
    (language === 'he'
      ? 'מסמך הונפק במערכת NodexPro — עותק להמחאה בלבד'
      : 'Issued via NodexPro — for reference only');

  const issuerBlock: IncomeRenderPartyBlock = {
    ...
    business_type_label:
      issuer.business_type_label != null
