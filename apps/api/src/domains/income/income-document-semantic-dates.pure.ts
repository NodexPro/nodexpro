...
export function calendarDateIso(raw: string | null | undefined): string | null {
  if (raw == null) return null;
  const match = /^(\d{4}-\d{2}-\d{2})/.exec(String(raw).trim());
  return match ? match[1] : null;
}

export function formatIncomeCalendarDateHe(raw: string | null | undefined): string {
...
export type IncomeDocumentSemanticDates = {
