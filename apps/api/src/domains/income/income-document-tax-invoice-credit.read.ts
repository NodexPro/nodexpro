...
import {
  ...
  INCOME_TAX_INVOICE_CREDIT_REASON_OPTIONS,
  creditSourceReferenceDisplay,
  type IncomeTaxInvoiceCreditMode,
...
export function buildTaxInvoiceCreditAction(params: {
  ...
  modes: Array<{ key: IncomeTaxInvoiceCreditMode; label: string }>;
  reason_options: typeof INCOME_TAX_INVOICE_CREDIT_REASON_OPTIONS;
...
export async function loadIssuedCreditAmountsByInvoice(
  orgId: string,
  invoiceIds: string[],
  ...
  for (const row of (data ?? []) as Array<{ source_invoice_id: string; credited_amount_reference: number | null }>) {
...
export async function loadIssuedCreditRowsForInvoices(
  orgId: string,
  invoiceIds: string[],
): Promise<
  Array<{
...
export async function loadCreditSourceReferenceForDocument(
  orgId: string,
  ...
  const number = (data as { source_invoice_number: string | null } | null)?.source_invoice_number;
