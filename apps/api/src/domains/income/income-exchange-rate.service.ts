...
import {
  addDaysIso,
  boiSourceLabel,
  formatBoiRateDisplay,
  normalizeIsoDate,
  parseBoiPublicApiRates,
  parseBoiSdmxCsv,
  parseBoiSdmxJsonObservations,
  pickBoiRateForDate,
  publicApiRateForCurrency,
  type BoiOfficialRate,
  type BoiObservation,
} from './income-boi-exchange-rate.pure.js';
...
const BOI_SERIES_KEY: Record<Exclude<IncomeDraftLineCurrency, 'ILS'>, string> = {
  USD: 'RER_USD_ILS',
  EUR: 'RER_EUR_ILS',
  GBP: 'RER_GBP_ILS',
};
...
type CacheEntry = {
  rate: BoiOfficialRate;
  expiresAt: number;
};

const officialRateCache = new Map<string, CacheEntry>();
...
export function setIncomeExchangeRateFetchForTests(impl: FetchLike | null): void {
  fetchImpl = impl ?? ((url, init) => fetch(url, init));
}

export function clearIncomeExchangeRateCacheForTests(): void {
  officialRateCache.clear();
}

function cacheKey(currency: string, documentDate: string): string {
  return `${currency}:${normalizeIsoDate(documentDate)}`;
}

function readCache(key: string): BoiOfficialRate | null {
...
function writeCache(key: string, rate: BoiOfficialRate): void {
...
function buildSdmxUrl(currency: Exclude<IncomeDraftLineCurrency, 'ILS'>, documentDate: string): {
  jsonUrl: string;
  csvUrl: string;
} {
...
async function loadObservationsFromSdmx(
  currency: Exclude<IncomeDraftLineCurrency, 'ILS'>,
  documentDate: string,
): Promise<BoiObservation[]> {
...
export async function resolveOfficialBoiExchangeRate(
  currency: Exclude<IncomeDraftLineCurrency, 'ILS'>,
  documentDate: string,
): Promise<BoiOfficialRate> {
  ...
  if (requested >= today) {
    ...
    if (live) {
      const adjusted: BoiOfficialRate = {
        ...live,
        requested_date: requested,
...
export async function resolveOfficialBoiRatesForCurrencies(
  currencies: IncomeDraftLineCurrency[],
  documentDate: string,
): Promise<Map<IncomeDraftLineCurrency, BoiOfficialRate>> {
  const unique = [...new Set(currencies.filter((c) => c !== 'ILS'))] as Exclude<
    IncomeDraftLineCurrency,
    'ILS'
  >[];
  const map = new Map<IncomeDraftLineCurrency, BoiOfficialRate>();
  await Promise.all(
...
export function officialRateSourceLabel(rate: BoiOfficialRate): string {
