...
export function findVisibleLogoBounds(
  ...
  width: number,
  height: number,
  alphaThreshold: number = LOGO_ALPHA_THRESHOLD,
): VisibleLogoBounds | null {
...
export function computeLargestSafeLogoFitScale(input: LogoFitScaleInput): number {
  ...
  if (!(cw > 0) || !(ch > 0) || !(frameW > 0) || !(frameH > 0)) return 1;
  const innerW = frameW * (1 - 2 * pad);
  const innerH = frameH * (1 - 2 * pad);
  return Math.min(innerW / cw, innerH / ch);
}

export function cropRgbaToBounds(
  rgba: Uint8Array | Buffer,
  srcWidth: number,
  bounds: VisibleLogoBounds,
): Buffer {
 ...
 */
export function trimTransparentMarginsFromPngBuffer(buffer: Buffer): {
  buffer: Buffer;
  bounds: VisibleLogoBounds;
  source_width: number;
...
export function logoCssFitPercent(paddingRatio: number = LOGO_FRAME_PADDING_RATIO): string {
