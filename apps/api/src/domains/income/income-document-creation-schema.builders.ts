import type { IncomeDocumentCreationSchema, IncomeWorkspacePermissions } from './income.types.js';

export function buildDocumentCreationSchema(perms: IncomeWorkspacePermissions): IncomeDocumentCreationSchema {
  const draftActions: string[] = [];
  if (perms.edit) {
    draftActions.push(
