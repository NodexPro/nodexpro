...
async function loadIssuedDocumentForPdf(orgId: string, documentId: string): Promise<IssuedDocForPdf> {
  const { data, error } = await supabaseAdmin
    ...
    .select(
      'id, organization_id, issuer_business_id, represented_client_id, document_type, document_number, issue_date, currency, language, notes, issuer_snapshot_json, customer_snapshot_json, lines_snapshot_json, totals_snapshot_json, legal_snapshot_json, pdf_render_status, pdf_asset_id',
    )
    .eq('id', documentId)
...
export async function renderIncomeDocumentPdf(
  ...
  try {
    ...
    const snapshot = buildIncomeDocumentRenderSnapshot({
      ...
      organization_footer_note: (settings as { document_footer_note?: string | null } | null)
