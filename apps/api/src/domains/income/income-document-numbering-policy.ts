...
export const IL_NUMBERING_POLICY_KEY = 'il_series_v1' as const;
/** Sentinel year bucket — IL series is not calendar-year based. */
export const IL_NUMBERING_SERIES_YEAR = 0;

export type IlNumberingOverflowStrategy = 'overflow_jump' | 'continue_after_range' | 'todo_policy';

export type IlSeriesPolicy = {
  policy_key: typeof IL_NUMBERING_POLICY_KEY;
  document_type: IncomeDocumentType;
  first_number: number;
  range_start: number;
  range_end: number | null;
  overflow_next: number | null;
  overflow_strategy: IlNumberingOverflowStrategy;
};

const IL_SERIES_POLICIES: Record<IncomeDocumentType, IlSeriesPolicy> = {
  quote: {
...
export function resolveIlSeriesPolicy(documentType: IncomeDocumentType): IlSeriesPolicy {
 ...
 */
export function computeNextIlSeriesNumber(
  currentNumber: number,
  policy: IlSeriesPolicy,
): { next_number: number; overflow_applied: boolean } {
  if (currentNumber <= 0) {
    return { next_number: policy.first_number, overflow_applied: false };
  }
  if (
    policy.overflow_strategy === 'overflow_jump' &&
...
export function formatIlSeriesDocumentNumber(sequenceNumber: number): string {
