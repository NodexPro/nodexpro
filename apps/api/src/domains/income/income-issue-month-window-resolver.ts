...
import {
  ...
  type IncomeIssueMonthWindowResolution,
} from './income-issue-month-window-fallback.pure.js';

export type { IncomeIssueMonthWindowResolution };
...
export async function resolveIncomeIssueMonthWindowForOrg(
  orgId: string,
  countryCode: string,
  date: string,
): Promise<IncomeIssueMonthWindowResolution> {
  ...
  try {
    ...
    if (ctx.ruleset_id) {
      const version = await resolveLegalValue(
        cc,
        IL_ISSUE_MONTH_WINDOW_LEGAL_VALUE_KEY,
        day,
        ctx.ruleset_id,
      );
      ...
      const fromMap = parseIssueMonthWindowFromLegalPayload(
        ctx.resolved_values_map[IL_ISSUE_MONTH_WINDOW_LEGAL_VALUE_KEY],
      );
      if (fromMap) {
        return {
          ...fromMap,
