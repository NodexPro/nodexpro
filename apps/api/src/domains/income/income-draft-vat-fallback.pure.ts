...
export function compactVatSelectLabel(vat: IncomeDraftVatResolution): string {
  return `מע״מ ${vat.standard_rate_percent_label}`;
}

export function readVatResolutionFromDraftPreview(
  raw: unknown,
  documentDate: string,
): IncomeDraftVatResolution | null {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
  const o = raw as Record<string, unknown>;
  const cache = o.vat_resolution_cache;
  if (!cache || typeof cache !== 'object' || Array.isArray(cache)) return null;
  const c = cache as Record<string, unknown>;
  if (c.document_date !== documentDate) return null;
  const rate = Number(c.standard_rate);
  if (!Number.isFinite(rate) || rate < 0 || rate > 1) return null;
  const pctLabel =
    typeof c.standard_rate_percent_label === 'string'
      ? c.standard_rate_percent_label
      : formatPercentLabel(rate);
  return {
    standard_rate: rate,
    standard_rate_percent_label: pctLabel,
    standard_vat_mode_option_label:
      typeof c.standard_vat_mode_option_label === 'string'
...
export function vatResolutionCachePayload(
  documentDate: string,
  vat: IncomeDraftVatResolution,
