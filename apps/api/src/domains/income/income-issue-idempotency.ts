/**
 * Lease-based idempotency for issue_income_document.
 */

import { supabaseAdmin } from '../../db/client.js';
import { badRequest, conflict } from '../../shared/errors.js';
import { INCOME_COMMAND_ISSUE_DOCUMENT } from './income.types.js';

export type IncomeIssueIdempotencyLease =
...
export function parseIssueIdempotencyKey(body: Record<string, unknown>): string | null {
  return normalizeIdempotencyKey(body.idempotency_key);
}

export async function beginIncomeIssueIdempotency(args: {
  organizationId: string;
  idempotencyKey: string;
  sourceDraftId: string;
}): Promise<IncomeIssueIdempotencyLease> {
  ...
  const insertResp = await supabaseAdmin
    ...
    .insert({
      ...
      command_type: INCOME_COMMAND_ISSUE_DOCUMENT,
  ...
  if (row.command_type !== INCOME_COMMAND_ISSUE_DOCUMENT) {
    throw badRequest(
...
export async function completeIncomeIssueIdempotency(args: {
  leaseRowId: string;
...
export async function abortIncomeIssueIdempotency(leaseRowId: string): Promise<void> {
  const { error } = await supabaseAdmin
    .from('income_command_idempotency')
    .delete()
    .eq('id', leaseRowId);
