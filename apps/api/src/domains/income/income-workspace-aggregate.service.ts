...
async function loadCustomers(scope: ActiveIncomeIssuerScope): Promise<IncomeCustomersTableRow[]> {
  let query = supabaseAdmin
    ...
    .select(
      'id, display_name, phone, email, tax_id, default_payment_terms, is_one_time, status, created_at',
    )
...
async function loadIssuedDocuments(
  ...
  const pdfStatusLabel = (status: string): string => {
    if (status === 'rendered') return 'PDF ready';
    if (status === 'failed') return 'PDF failed';
    if (status === 'pending') return 'PDF pending';
    return status;
  };
...
export async function buildIncomeWorkspaceAggregate(
  ...
  const [customersCount, itemsCount, draftsCount, issuedCount, postedCount, postingFailedCount] =
    await Promise.all([
  ...
  const canCreateDocument =
    scope.permissions.edit && docTypesResult.available_document_types.some((t) => t.enabled);
  ...
  return {
    ...
    cards: buildIncomeWorkspaceCards(
      scope.permissions,
      {
