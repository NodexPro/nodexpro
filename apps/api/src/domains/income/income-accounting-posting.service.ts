...
import {
  ...
  type IncomeAccountingDisplayStatus,
...
export interface IncomeDocumentAccountingRow {
  id: string;
...
export async function applyAccountingPostingForIssuedDocument(
  ctx: RequestContext,
  doc: IncomeDocumentAccountingRow,
): Promise<{
  accounting_posting_status: string;
  accounting_entry_id: string | null;
  accounting_entry_link_id: string | null;
  accounting_posting_signature: string | null;
  accounting_display_status: IncomeAccountingDisplayStatus;
}> {
...
export async function retryAccountingPostingForIssuedDocument(
  ctx: RequestContext,
  orgId: string,
  ...
  const { data, error } = await supabaseAdmin
    .from('income_documents')
    .select(
      'id, organization_id, document_type, document_number, issue_date, currency, represented_client_id, totals_snapshot_json, lines_snapshot_json, accounting_posting_status, accounting_entry_id',
    )
    .eq('id', incomeDocumentId)
    .eq('organization_id', orgId)
    .maybeSingle();
  if (error) throw error;
  if (!data) throw new Error('Income document not found');

  const row = data as IncomeDocumentAccountingRow;
