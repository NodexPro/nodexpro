...
function preliminaryEditSourceDocumentId(row: { document_settings_json: unknown }): string | null {
  return readPreliminaryEditSourceDocumentId(row.document_settings_json);
}

function preservePreliminaryEditMarker(
...
async function loadPreliminaryEditSourceDocumentNumber(
  scope: ActiveIncomeIssuerScope,
  sourceDocumentId: string,
): Promise<string | null> {
  const { data, error } = await supabaseAdmin
  ...
  const documentNumber =
    data && typeof (data as { document_number?: unknown }).document_number === 'string'
      ? String((data as { document_number: string }).document_number).trim()
      : '';
...
export async function updateIncomeDocumentDiscount(
  ...
  const patch = {
    document_settings_json: preservePreliminaryEditMarker(
      row,
      serializeDocumentSettingsJson(nextSettings),
    ),
  };
...
export async function generateIncomeDocumentPreview(
  ...
  const priorCache =
    ...
    !Array.isArray(row.draft_totals_preview_json)
      ? (row.draft_totals_preview_json as Record<string, unknown>)
      : {};
  const cachedDocumentNumber =
    typeof priorCache.document_number_preview === 'string' && priorCache.document_number_preview.trim()
...
export async function buildReadOnlyIncomeDocumentPreviewOverlay(
  scope: ActiveIncomeIssuerScope,
  draftId: string,
  options?: { issued_document_number?: string | null },
): Promise<WizardDraftOverlay> {
...
export async function updateIncomeDocumentAllocationNumber(
  scope: ActiveIncomeIssuerScope,
  body: Record<string, unknown>,
): Promise<WizardDraftOverlay> {
  const draft_id = reqUuid(body.draft_id, 'draft_id');
  ...
  if (!row.document_type) throw badRequest('document_type is required');
  const docType = await resolveDocType(scope, row.document_type);
  const { resolveIncomeTaxAllocationNumberPolicyForOrg } = await import(
    './income-document-allocation-number-resolver.js'
  );
  const {
    buildIncomeDocumentAllocationNumberField,
    normalizeAllocationNumberInput,
    validateAllocationNumberFormat,
  } = await import('./income-document-allocation-number.pure.js');
  const documentDate = row.document_date ?? new Date().toISOString().slice(0, 10);
  const policyResolution = await resolveIncomeTaxAllocationNumberPolicyForOrg(
    scope.org_id,
    'IL',
    documentDate,
  );
...
async function buildCustomerSnapshotForDraftRow(
  scope: ActiveIncomeIssuerScope,
  row: IncomeWizardDraftRow,
...
async function savePreliminaryDocumentEditIfNeeded(
  scope: ActiveIncomeIssuerScope,
  draftId: string,
  row: IncomeWizardDraftRow & { status: string },
  docType: IncomeAvailableDocumentType,
): Promise<WizardDraftOverlay | null> {
  const sourceDocumentId = preliminaryEditSourceDocumentId(row);
  ...
  if (!sourceRaw) throw notFound('Income document not found');
  ...
  const draftExtras = row as IncomeWizardDraftRow & {
  ...
  const { error: updateDocErr } = await supabaseAdmin
    ...
    .update({
      ...
      tax_allocation_number:
        typeof row.tax_allocation_number === 'string' && row.tax_allocation_number.trim()
  ...
  if (source.source_draft_id) {
    ...
    const sourceDraftSettingsJson = serializeDocumentSettingsJson(
      parseDocumentSettingsJson(row.document_settings_json),
    );
...
export async function saveIncomeDocumentDraft(
  ...
  if (preliminaryEditOverlay) return preliminaryEditOverlay;
  ...
  const existingUserSavedAt = (row as { user_saved_at?: string | null }).user_saved_at;
  ...
  const overlay = await wizardDraftMutationOverlay(
    ...
    docType,
    savePatch,
    { action: 'save_draft', user_saved_at_set: Object.keys(savePatch).length > 0 },
  );
