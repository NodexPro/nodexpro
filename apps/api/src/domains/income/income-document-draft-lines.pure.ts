...
import {
  DRAFT_LINE_CURRENCY_INVALID_MESSAGE,
  DRAFT_LINE_EXCHANGE_RATE_INVALID_MESSAGE,
  parseDraftLineCurrency,
  parseDraftLineCurrencyFromPatch,
  parseDraftLineExchangeRateOverride,
...
function finalizeDraftLine(line: IncomeDraftLineRecord): IncomeDraftLineRecord {
...
export function normalizeDraftLines(raw: unknown): IncomeDraftLineRecord[] {
  ...
  const lines: IncomeDraftLineRecord[] = [];
  arr.forEach((item, index) => {
    ...
    let exchange_rate_to_ils_override =
      Number.isFinite(overrideRaw) && overrideRaw > 0 ? overrideRaw : null;
    if (currency === 'ILS') exchange_rate_to_ils_override = null;
...
export function serializeDraftLines(lines: IncomeDraftLineRecord[]): unknown[] {
  return lines.map((l) => ({
    line_id: l.line_id,
...
export function applyLineFieldUpdate(
  ...
  if (idx < 0) throw badRequest('line_id not found');
  ...
  if (patch.quantity !== undefined) {
    ...
    if (!Number.isFinite(q) || q <= 0) throw badRequest('quantity must be a positive number');
    quantity = q;
  }
  if (patch.unit_price_reference !== undefined) {
    ...
    if (v === null || v === '') {
      ...
      const n = Number(v);
      if (!Number.isFinite(n)) throw badRequest('unit_price_reference must be numeric');
  ...
  const hasExchangePatch =
    patch.exchange_rate_to_ils_override !== undefined || patch.exchange_rate_override !== undefined;

  if (currency !== 'ILS' && hasExchangePatch) {
    try {
      exchange_rate_to_ils_override = parseDraftLineExchangeRateOverride(
        currency,
        patch.exchange_rate_to_ils_override ?? patch.exchange_rate_override,
      );
    } catch {
...
export function reorderDraftLines(lines: IncomeDraftLineRecord[], orderedLineIds: string[]): IncomeDraftLineRecord[] {
  ...
  const reordered: IncomeDraftLineRecord[] = [];
  for (const id of orderedLineIds) {
    const line = byId.get(id);
    if (!line) throw badRequest(`Unknown line_id in reorder: ${id}`);
    reordered.push(line);
  }
  return reordered.map((l, i) => ({ ...l, sort_index: i }));
}

export function deleteDraftLine(lines: IncomeDraftLineRecord[], lineId: string): IncomeDraftLineRecord[] {
  const filtered = lines.filter((l) => l.line_id !== lineId);
  if (filtered.length === lines.length) throw badRequest('line_id not found');
