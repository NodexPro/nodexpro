...
export async function buildIncomeIssuerSnapshotForScope(
  ...
  if (scope.acting_mode === 'office_representative' && scope.represented_client_id) {
    const { data: client, error: cErr } = await supabaseAdmin
      ...
      .maybeSingle();
    if (cErr) throw cErr;
    if (!client) throw notFound('Client not found for issuer snapshot');
    ...
    const businessType = normalizeIssuerBusinessType(
      (profile as { business_type?: string | null } | null)?.business_type ?? null,
    );
    const vatFlag = (profile as { vat_registered_flag?: boolean | null } | null)?.vat_registered_flag;
