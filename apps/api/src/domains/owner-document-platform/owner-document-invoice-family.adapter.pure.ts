...
} from '../owner-invoice-document-layout/owner-invoice-document-layout.types.js';
import type {
  OwnerDocumentSectionModel,
  OwnerDocumentWidgetCatalogEntry,
  OwnerDocumentWidgetType,
} from './owner-document-platform.types.js';

export const INVOICE_TEMPLATE_FAMILY = 'invoice' as const;

export const INVOICE_SECTION_LABELS: Record<string, string> = {
  logo: 'Logo',
...
function widgetTypeForField(field: OwnerInvoiceFieldCatalogEntry): OwnerDocumentWidgetType {
  if (field.field_key === 'logo') return 'logo';
  if (field.display_variants.includes('table_column')) return 'table';
  if (field.field_key === 'signature_block' || field.field_key === 'stamp') return 'signature';
  if (field.field_key === 'legal_wording' || field.field_key === 'platform_footer') return 'legal_block';
  if (field.group === 'totals') return 'totals_block';
  if (field.field_key.includes('address')) return 'address_block';
  if (
    field.field_key.includes('phone') ||
    field.field_key.includes('email') ||
    field.field_key === 'customer_contact'
  ) {
    return 'contact_block';
  }
  if (field.group === 'document' && field.field_key.includes('payment')) return 'payment_block';
  return 'text';
}
 ...
 */
export function buildInvoiceFamilyWidgetCatalog(): OwnerDocumentWidgetCatalogEntry[] {
 ...
 */
export function buildInvoiceFamilySectionModel(
  definition: OwnerInvoiceLayoutDefinitionV1,
  sectionLabels: Record<string, string> = INVOICE_SECTION_LABELS,
): OwnerDocumentSectionModel[] {
...
export function buildInvoiceWidgetCatalogGroups(
  widgets: OwnerDocumentWidgetCatalogEntry[],
): Array<{ group_key: string; group_label: string; widgets: OwnerDocumentWidgetCatalogEntry[] }> {
  const order: string[] = [];
  const map = new Map<string, OwnerDocumentWidgetCatalogEntry[]>();
