 ...
 */
export async function validateOrgFileOwnership(
  ctx: RequestContext,
  ...
  if ((row as FileAssetRow).organization_id !== organizationId) {
    writeAudit({
      organizationId,
      actorUserId: ctx.user.id,
      entityType: 'file_asset',
 ...
 */
export function assertFileAllowedForSettingsImage(file: FileAssetRow): void {
  if (file.archived_at) {
...
export type FileAccessPurpose = 'settings';
 ...
 */
export async function getSecureOpenUrlForOrgFile(
  ctx: RequestContext,
  organizationId: string,
  fileAssetId: string,
  accessPurpose: FileAccessPurpose
  ...
  if (accessPurpose === 'settings') {
    ...
    const s = settings as { logo_file_asset_id?: string; signature_image_file_asset_id?: string } | null;
