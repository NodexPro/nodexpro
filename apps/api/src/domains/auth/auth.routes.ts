...
router.get('/me', authMiddleware, async (req, res, next) => {
  try {
    ...
    let activeOrgId = ctx.organizationId ?? null;
    if (!activeOrgId && orgList.length === 1) activeOrgId = orgList[0].id;
    ...
    // Modules catalog: show when user can manage billing/modules. Do NOT depend on enabledModules, trial, or purchased modules.
    ...
    if (activeOrgId) {
      ...
      for (const m of modList) {
        ...
        const label =
          mo.code === 'client-operations' ? 'Nodex לקוחות' : (mo.nav_label ?? mo.code);
