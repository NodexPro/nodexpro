...
const LEGAL_ENTITY_TYPES = ['exempt_dealer', 'registered_dealer', 'company', 'other_corporation', 'other'] as const;

const LEGAL_ENTITY_TYPE_TO_LABEL_KEY: Record<string, string> = {
...
const TRIAL_STATUS_TO_LABEL_KEY: Record<string, string> = {
...
export interface OrganizationSettingsDto {
  ...
  profile?: {
    ...
    /** Canonical organization country — always `organizations.country_code`. */
    country: string | null;
    /** Set when `countries` registry is non-empty and this code is missing (GET only; PATCH rejects unknown codes). */
  ...
  country_configuration?: {
    ...
    active_ruleset: string | null;
  };
}

function getLegalIdLabelKey(legalEntityType: string | null): string {
  if (!legalEntityType) return 'settings.legalIdLabel.generic';
...
async function assertCountryCodeRegisteredWhenRegistryPopulated(code: string): Promise<void> {
  const cc = code.trim().toUpperCase().slice(0, 2);
  if (cc.length !== 2) throw badRequest('Country must be a 2-letter code.', 'INVALID_COUNTRY_CODE');
  ...
  throw badRequest(
    'Country code is not registered in the platform country registry. Choose a registered country or add it via platform governance.',
    'COUNTRY_NOT_REGISTERED'
  );
}

async function getCountryRegistryWarningForDisplay(code: string | null): Promise<string | null> {
  ...
  const { data: sample, error: sampleErr } = await supabaseAdmin.from('countries').select('code').limit(1);
  if (sampleErr || !sample?.length) return null;
...
async function autoConfigureOrganizationCountrySettings(
  organizationId: string,
  countryCode: string
): Promise<{ activePackCode: string; activeRulesetCode: string }> {
  ...
  let rulesetId = '';
  let rulesetCode = '';
  let activeForDate: { id: string; ruleset_code: string } | null = null;
  try {
  ...
  if (activeForDate) {
    ...
    const latest =
      (latestRows ?? []).find((r) => String(r.status ?? '').trim().toLowerCase() === 'active') ??
      (latestRows ?? [])[0] ??
      null;
    if (!latest) {
...
function getTrialStatusDisplay(
  trialStatus: string | undefined,
  endsAt: string | null | undefined
): { labelKey: string; value: string | null } {
  if (!trialStatus) return { labelKey: 'settings.ownerIdentity.trialNotStarted', value: null };
...
async function isOwner(ctx: RequestContext, orgId: string): Promise<boolean> {
  const { data: org } = await supabaseAdmin
    .from('organizations')
    .select('owner_user_id')
    .eq('id', orgId)
    .single();
  return (org as { owner_user_id: string | null } | null)?.owner_user_id === ctx.user.id;
}

export async function getOrganizationSettings(ctx: RequestContext, orgId: string): Promise<OrganizationSettingsDto> {
  ...
  const canEditSettings =
    owner && (perms.includes('settings:write') || perms.includes('access_settings'));
  ...
  const result: OrganizationSettingsDto = {
    capabilities: {
      canEditSettings,
  ...
  if (ownerIdentity) {
    result.ownerIdentity = {
      ...
      legalIdLabelKey:
        ownerIdentity.legalIdentityType === 'tz' ? 'settings.legalIdLabel.teudatZehut' : 'settings.legalIdLabel.generic',
  ...
  const orgRow = org as { name: string; legal_name: string | null; country_code: string } | null;

  const profileCountry = orgRow?.country_code ?? (s?.country as string) ?? null;

  result.profile = {
    organizationName: (s?.organization_name as string) ?? orgRow?.name ?? null,
    legalEntityType: (s?.legal_entity_type as string) ?? null,
    legalIdNumber: (s?.legal_id_number as string) ?? null,
    legalIdLabelKey: getLegalIdLabelKey((s?.legal_entity_type as string) ?? null),
    addressLine1: (s?.address_line_1 as string) ?? null,
    addressLine2: (s?.address_line_2 as string) ?? null,
    city: (s?.city as string) ?? null,
    postalCode: (s?.postal_code as string) ?? null,
    country: profileCountry,
    countryWarning: await getCountryRegistryWarningForDisplay(profileCountry),
    phone: (s?.phone as string) ?? null,
    website: (s?.website as string) ?? null,
    logoFileAssetId: (s?.logo_file_asset_id as string) ?? null,
  };

  result.documentIdentity = {
    displayNameOnDocuments: (s?.display_name_on_documents as string) ?? null,
    ...
    documentFooterNote: (s?.document_footer_note as string)?.trim() || null,
  };

  result.signature = {
    signatureText: (s?.signature_text as string) ?? null,
    signatureImageFileAssetId: (s?.signature_image_file_asset_id as string) ?? null,
  };

  if (owner) {
    result.bankDetails = {
      bankAccountHolder: (s?.bank_account_holder as string) ?? null,
      bankName: (s?.bank_name as string) ?? null,
      bankBranch: (s?.bank_branch as string) ?? null,
      bankAccountNumber: (s?.bank_account_number as string) ?? null,
      iban: (s?.iban as string) ?? null,
      swift: (s?.swift as string) ?? null,
  ...
  const { data: ocs } = await supabaseAdmin
    ...
    .eq('organization_id', orgId)
    .maybeSingle();
  const activePackId = String((ocs as { active_country_pack_id?: string | null } | null)?.active_country_pack_id ?? '');
  const activeRulesetId = String((ocs as { active_ruleset_id?: string | null } | null)?.active_ruleset_id ?? '');
  let activePackCode: string | null = null;
  let activeRulesetCode: string | null = null;
  if (activePackId) {
 ...
 */
export async function getSettingsFileOpenUrl(
  ctx: RequestContext,
  orgId: string,
  fileAssetId: string
...
export interface PatchOrganizationSettingsBody {
  organizationName?: string | null;
  legalEntityType?: string | null;
  legalIdNumber?: string | null;
  addressLine1?: string | null;
...
function sanitizeWebsite(url: string | null | undefined): string | null {
...
export async function patchOrganizationSettings(
  ctx: RequestContext,
  orgId: string,
  body: PatchOrganizationSettingsBody
): Promise<OrganizationSettingsDto> {
  if (ctx.organizationId !== orgId) throw forbidden('Organization context required');
  if (!(await isOwner(ctx, orgId))) throw forbidden('Only the organization owner can edit settings');

  if (body.legalEntityType != null && body.legalEntityType !== '' && !LEGAL_ENTITY_TYPES.includes(body.legalEntityType as (typeof LEGAL_ENTITY_TYPES)[number])) {
  ...
  if (!orgBefore) throw notFound('Organization not found');
  const previousOrgCountry = String((orgBefore as { country_code: string }).country_code).trim().toUpperCase().slice(0, 2);

  let effectiveCountryCode = previousOrgCountry;

  if (body.country !== undefined) {
    const raw = body.country?.trim() ?? '';
    if (!raw) throw badRequest('Country is required.', 'INVALID_COUNTRY_CODE');
  ...
  const shouldAutoConfigureCountrySettings =
    effectiveCountryCode.length === 2 && (body.country !== undefined || !hasConfiguredBinding);
  ...
  const base = (existing ?? { organization_id: orgId }) as Record<string, unknown>;

  if (body.organizationName !== undefined) base.organization_name = body.organizationName?.trim() || null;
  if (body.legalEntityType !== undefined) base.legal_entity_type = body.legalEntityType?.trim() || null;
  if (body.legalIdNumber !== undefined) base.legal_id_number = body.legalIdNumber?.trim() || null;
  if (body.addressLine1 !== undefined) base.address_line_1 = body.addressLine1?.trim() || null;
  if (body.addressLine2 !== undefined) base.address_line_2 = body.addressLine2?.trim() || null;
  if (body.city !== undefined) base.city = body.city?.trim() || null;
  if (body.postalCode !== undefined) base.postal_code = body.postalCode?.trim() || null;
  if (body.phone !== undefined) base.phone = sanitizePhone(body.phone);
  if (body.website !== undefined) base.website = sanitizeWebsite(body.website);
  if (body.logoFileAssetId !== undefined) base.logo_file_asset_id = body.logoFileAssetId || null;
  if (body.displayNameOnDocuments !== undefined) base.display_name_on_documents = body.displayNameOnDocuments?.trim() || null;
  if (body.displayPhoneOnDocuments !== undefined) base.display_phone_on_documents = body.displayPhoneOnDocuments;
  if (body.displayWebsiteOnDocuments !== undefined) base.display_website_on_documents = body.displayWebsiteOnDocuments;
  if (body.displayAddressOnDocuments !== undefined) base.display_address_on_documents = body.displayAddressOnDocuments;
  if (body.documentFooterNote !== undefined) base.document_footer_note = body.documentFooterNote?.trim() || null;
  if (body.signatureText !== undefined) base.signature_text = body.signatureText?.trim() || null;
  if (body.signatureImageFileAssetId !== undefined) base.signature_image_file_asset_id = body.signatureImageFileAssetId || null;
  if (body.bankAccountHolder !== undefined) base.bank_account_holder = body.bankAccountHolder?.trim() || null;
  if (body.bankName !== undefined) base.bank_name = body.bankName?.trim() || null;
  if (body.bankBranch !== undefined) base.bank_branch = body.bankBranch?.trim() || null;
  if (body.bankAccountNumber !== undefined) base.bank_account_number = body.bankAccountNumber?.trim() || null;
  if (body.iban !== undefined) base.iban = body.iban?.trim() || null;
  if (body.swift !== undefined) base.swift = body.swift?.trim() || null;
  if (body.displayBankDetailsOnDocuments !== undefined) base.display_bank_details_on_documents = body.displayBankDetailsOnDocuments;
