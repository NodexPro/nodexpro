...
} from '../income/income-issue-month-window-fallback.pure.js';
...
const VALUE_KEY_EDITOR_MAP: Record<string, string> = {
  [IL_INCOME_ISSUE_MONTH_WINDOW_VALUE_KEY]: OWNER_LEGAL_VALUE_EDITOR_ISSUE_MONTH_WINDOW,
};

function monthLabel(count: number, direction: 'back' | 'forward'): string {
...
function numberField(
  ...
  label: string,
  value: number,
  opts?: { min?: number; max?: number; help_text?: string },
): OwnerLegalValueEditorField {
  return {
    key,
    label,
    input_type: 'number',
...
function dateField(key: string, label: string, value: string, required: boolean): OwnerLegalValueEditorField {
  return {
    key,
    label,
    input_type: 'date',
...
export function resolveOwnerLegalValueEditorKey(
  valueKey: string,
  valueType: string | null | undefined,
): string | null {
  const key = valueKey.trim();
  if (VALUE_KEY_EDITOR_MAP[key]) return VALUE_KEY_EDITOR_MAP[key];
...
export function formatOwnerLegalValueHumanSummary(
  valueKey: string,
  valueType: string | null | undefined,
  payload: unknown,
): OwnerLegalValueHumanSummary | null {
  ...
  if (editorKey === OWNER_LEGAL_VALUE_EDITOR_ISSUE_MONTH_WINDOW) {
    const parsed =
      parseIssueMonthWindowFromLegalPayload(payload) ?? IL_ISSUE_MONTH_WINDOW_FALLBACK;
...
function readIntegerField(payload: Record<string, unknown>, key: string): number | null {
...
export function assembleLegalValuePayloadFromOwnerEditorInput(
  valueKey: string,
  valueType: string | null | undefined,
...
export function ownerEditorInputPresent(
  valueKey: string,
  valueType: string | null | undefined,
  payload: Record<string, unknown>,
): boolean {
