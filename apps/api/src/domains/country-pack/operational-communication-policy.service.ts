...
export type { ReminderTemplateRenderContext } from './operational-communication-owner-payload.js';

export const DEFAULT_REMINDER_POLICY_VALUE_KEY = 'comm.reminder.policy';

export type ResolvedReminderTemplateVersion = {
...
export type ResolvedOperationalCommunicationPolicies = {
  country_code: string | null;
  ruleset_id: string | null;
  policy_version_id: string | null;
  active_reminder_policy: OperationalReminderPolicyPayload | null;
  templates_by_key: Record<string, ResolvedReminderTemplateVersion[]>;
  warnings: string[];
};

export async function resolveOperationalCommunicationPolicies(
  organizationId: string,
  asOfDate: string,
): Promise<ResolvedOperationalCommunicationPolicies> {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .select(
      'id, value_payload_json, country_legal_values!inner(value_key, country_code, category, module_scope)',
    )
  ...
  let activePolicy: OperationalReminderPolicyPayload | null = null;
  const templatesByKey: Record<string, ResolvedReminderTemplateVersion[]> = {};
  ...
  for (const row of data ?? []) {
    ...
    if (isOperationalReminderPolicyPayload(vpj)) {
      try {
        ...
        warnings.push(
          `invalid_reminder_policy:${valueKey}:${e instanceof Error ? e.message : 'invalid'}`,
        );
      }
      continue;
    }
    ...
    if (isOperationalReminderTemplatePayload(vpj)) {
      try {
        ...
        warnings.push(
          `invalid_reminder_template:${valueKey}:${e instanceof Error ? e.message : 'invalid'}`,
        );
      }
    }
  }
