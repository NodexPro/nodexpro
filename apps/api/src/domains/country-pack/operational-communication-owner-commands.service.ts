...
async function refreshedPanel(ctx: RequestContext): Promise<OperationalCommunicationOwnerCommandResponse['refreshed']> {
  return {
    aggregate_key: 'owner_legal_control_panel_aggregate',
...
async function assertRulesetScope(
  countryCode: string,
...
async function activateVersionIfRequested(
  ctx: RequestContext,
  versionId: string,
  activate: boolean,
): Promise<void> {
...
function parseSaveScope(payload: Record<string, unknown>): SaveScope {
...
type LoadedPolicyVersionContext = {
  id: string;
...
function parseReminderWorkflowTypeField(raw: unknown): ReminderWorkflowType {
...
async function loadPolicyVersionContext(versionId: string): Promise<LoadedPolicyVersionContext> {
  const { data, error } = await supabaseAdmin
    .from('country_legal_value_versions')
    .select(
      'id, legal_value_id, country_pack_ruleset_id, effective_from, effective_to, status, value_payload_json, country_legal_values!inner(country_code, value_key, category)',
    )
    .eq('id', versionId)
    .maybeSingle();
  if (error) throw error;
  if (!data) throw notFound('Policy version not found', 'policy_version_not_found');
  const joined = data.country_legal_values as { country_code?: string; value_key?: string; category?: string };
...
async function deactivateLegalValueVersionInPlace(ctx: RequestContext, versionId: string): Promise<void> {
  const { error } = await supabaseAdmin
    .from('country_legal_value_versions')
...
async function activateLegalValueVersionInPlace(
  ctx: RequestContext,
  source: LoadedPolicyVersionContext,
): Promise<void> {
  await assertNoOverlapLegalValueVersions({
...
async function republishPolicyVersionWithPayload(
  ctx: RequestContext,
  source: LoadedPolicyVersionContext,
  newPolicy: OperationalReminderPolicyPayload,
...
async function fetchLatestPolicyPayloadForRuleset(
  legalValueId: string,
  rulesetId: string,
): Promise<OperationalReminderPolicyPayload | null> {
  const { data, error } = await supabaseAdmin
    .from('country_legal_value_versions')
    ...
    .limit(1);
  if (error) throw error;
  const row = (data ?? [])[0] as { value_payload_json?: unknown } | undefined;
  if (!row?.value_payload_json) return null;
  try {
...
export async function handleSaveOperationalReminderWorkflow(
  ctx: RequestContext,
  payload: Record<string, unknown>,
): Promise<OperationalCommunicationOwnerCommandResponse> {
  ...
  const templateMeta: Array<{ template_key: string; legal_value_version_id: string; created: boolean }> = [];
...
export async function handleEditOperationalReminderWorkflow(
  ctx: RequestContext,
  payload: Record<string, unknown>,
): Promise<OperationalCommunicationOwnerCommandResponse> {
...
export async function handleDisableOperationalReminderWorkflow(
  ctx: RequestContext,
  payload: Record<string, unknown>,
): Promise<OperationalCommunicationOwnerCommandResponse> {
...
export async function handleEnableOperationalReminderWorkflow(
  ctx: RequestContext,
  payload: Record<string, unknown>,
): Promise<OperationalCommunicationOwnerCommandResponse> {
  const versionId = asString(payload.policy_legal_value_version_id, 'policy_legal_value_version_id');
...
export async function handleSaveOperationalReminderPolicy(
  ctx: RequestContext,
  payload: Record<string, unknown>,
): Promise<OperationalCommunicationOwnerCommandResponse> {
  const scope = parseSaveScope(payload);
  ...
  const label =
    asOptionalString(payload.policy_label) ?? 'Work Engine reminder policy';
...
export async function handleSaveOperationalReminderTemplate(
  ctx: RequestContext,
  payload: Record<string, unknown>,
): Promise<OperationalCommunicationOwnerCommandResponse> {
  const scope = parseSaveScope(payload);
...
export async function handleSaveOperationalReminderPolicyVersion(
  ctx: RequestContext,
  payload: Record<string, unknown>,
): Promise<OperationalCommunicationOwnerCommandResponse> {
  const scope = parseSaveScope(payload);
  ...
  try {
    ...
    throw notFound(
      'Reminder policy legal value does not exist for this country. Create a policy first.',
      'reminder_policy_legal_value_missing',
    );
  }
...
export async function handleSaveOperationalReminderTemplateVersion(
  ctx: RequestContext,
  payload: Record<string, unknown>,
): Promise<OperationalCommunicationOwnerCommandResponse> {
  const scope = parseSaveScope(payload);
  ...
  try {
    ...
    throw notFound(
      'Reminder template legal value does not exist for this workflow/language. Create a template first.',
      'reminder_template_legal_value_missing',
    );
  }
