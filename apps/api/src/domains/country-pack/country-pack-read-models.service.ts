...
import {
  ...
  buildReminderWorkflowEditableForm,
} from './operational-communication-owner-form.js';
import {
  ...
  isOperationalReminderTemplatePayload,
...
async function buildOwnerCommercialControlsAggregate(queryInput?: Partial<CommercialControlsQuery>): Promise<Record<string, unknown>> {
  ...
  if (orgIdsAll.length && moduleIds.length) {
    ...
    for (const r of orgModsRes.data ?? []) {
      const oid = String((r as any).organization_id);
      orgHasModuleActivationHistory.add(oid);
      activationByOrgModule.set(`${oid}:${String((r as any).module_id)}`, String((r as any).status ?? 'inactive'));
    }
    for (const r of subsRes.data ?? []) {
      const oid = String((r as any).organization_id);
      orgHasSubscriptionHistory.add(oid);
      subByOrgModule.set(`${oid}:${String((r as any).module_id)}`, {
  ...
  if (orgIdsAll.length && moduleIds.length) {
    ...
    for (const r of adjs ?? []) {
      ...
      const key = `${oid}:${String((r as any).module_id)}`;
  ...
  let meaningful = (orgs ?? []).filter((o) => {
    ...
    if (excludedNamePrefix.test(name)) return false;
    const clientsCount = clientsCountByOrg.get(orgId) ?? 0;
    const hasClients = clientsCount > 0;
    const hasHistory =
      orgHasSubscriptionHistory.has(orgId) ||
...
function assertOrgAggregateAccess(ctx: RequestContext, organizationId: string): void {
  try {
...
function statusBadge(status: string): { code: string; label: string; tone: 'ok' | 'warn' | 'muted' } {
  switch (status) {
    case 'active':
    case 'enabled':
...
export async function buildOwnerLegalValuesAggregate(ctx: RequestContext): Promise<Record<string, unknown>> {
  ...
  const allRows = (values.data ?? []).map((lv) => {
    ...
    const activeCurrent = lvVersions.find((item) => {
      const raw = item as { status: string; effective_from: string; effective_to: string | null };
    ...
    return {
      ...
      current_active_value: activeCurrent ? (activeCurrent as { value_payload_json: unknown }).value_payload_json : null,
      versions: lvVersions,
    };
  });
...
export async function buildOwnerPlatformPricingAggregate(ctx: RequestContext): Promise<Record<string, unknown>> {
  assertPlatformOwner(ctx);
    ...
    actions: [
      {
        ...
        button_label: '+New',
        note:
          'Новый тариф: укажите ровно один из module_id, module_code или пару new_module_code + new_module_name. Условия (лимиты) — в limits_json.',
        payload: {
          ...
          limits_json:
            'optional JSON array of {limit_code, limit_value?, is_unlimited?} e.g. [{"limit_code":"max_clients","limit_value":100,"is_unlimited":false}]',
        },
      },
      {
...
export async function buildOrganizationCountrySettingsAggregate(
  ctx: RequestContext,
  ...
  if (!organization.data) throw notFound('Organization not found');
  ...
  let activeRuleset: Record<string, unknown> | null = null;
  if (settings.data?.active_ruleset_id) {
    const ruleset = await supabaseAdmin
      ...
      .maybeSingle();
    if (ruleset.error) throw ruleset.error;
    activeRuleset = ruleset.data ?? null;
  }

  const diagnostics: string[] = [];
  if (!settings.data) diagnostics.push('country_settings_not_configured');
  if (settings.data && !settings.data.active_country_pack_id) diagnostics.push('no_active_country_pack');
  if (settings.data && settings.data.active_country_pack_id && !settings.data.active_ruleset_id) diagnostics.push('no_active_ruleset');
...
export async function buildCountryPackDiagnosticsAggregate(
  ctx: RequestContext,
  organizationId: string
): Promise<Record<string, unknown>> {
  assertOrgAggregateAccess(ctx, organizationId);
  ...
  const errors: string[] = [];
  if (!context.country_code) errors.push('missing_country_code');
  if (!context.country_pack_id) warnings.push('missing_active_pack');
  if (!context.ruleset_id) warnings.push('missing_active_ruleset');
...
export async function buildActiveRulesetContextAggregate(
  ctx: RequestContext,
  organizationId: string,
  date: string
): Promise<Record<string, unknown>> {
...
function buildDocflowCommunicationTemplatesFromLegalTable(
  ...
  for (const lv of legalTable) {
    ...
    for (const ver of versions) {
      ...
      const allowedActions: DocflowTemplateRowAction[] = [
        {
        ...
        },
        {
          ...
          note:
            'Update this version in place (payload and optional effective_from / effective_to). Use Create legal value version from Legal Values when you need a separate timeline row with a non-overlapping effective_from.',
        },
        {
      ...
      out.push({
        ...
        condition_config_summary:
          parseError === null ? JSON.stringify(normalized.condition_config ?? {}) : '—',
        schedule_config_summary:
          parseError === null ? JSON.stringify(normalized.schedule_config ?? {}) : '—',
...
function isOperationalCommunicationLegalValueRow(lv: Record<string, unknown>): boolean {
  return String(lv.category ?? '') === OPERATIONAL_COMMUNICATION_POLICIES_CATEGORY;
}

const REMINDER_WORKFLOW_TYPE_LABELS: Record<string, string> = {
  waiting_client: 'Waiting for client',
  response_sla: 'Response SLA',
  review_sla: 'Review SLA',
};

const REMINDER_CHANNEL_LABELS: Record<string, string> = {
  docflow: 'DocFlow',
  email: 'Email',
  portal: 'Portal',
};

type ReminderWorkflowRowAction = {
  action_key: string;
  label: string;
  enabled: boolean;
...
function buildTemplateBodiesIndexForRuleset(
  legalTable: Array<Record<string, unknown>>,
  rulesetId: string,
): Map<string, OperationalReminderTemplatePayload> {
  const index = new Map<string, OperationalReminderTemplatePayload>();
  for (const lv of legalTable) {
    if (!isOperationalCommunicationLegalValueRow(lv)) continue;
    const versions = Array.isArray(lv.versions) ? (lv.versions as Record<string, unknown>[]) : [];
    for (const ver of versions) {
      if (String(ver.country_pack_ruleset_id ?? '') !== rulesetId) continue;
      const vpj = ver.value_payload_json;
      if (!isOperationalReminderTemplatePayload(vpj)) continue;
      try {
...
function resolvePackIdForRuleset(
  rulesetId: string,
  packContext?: { rulesets?: unknown[] },
): string {
...
function computeReminderWorkflowRowStatus(params: {
  ...
  if (params.versionStatus !== 'active') return 'disabled';
  if (!params.workflowEnabled) return 'disabled';
  if (params.effectiveFrom > params.today) return 'disabled';
  if (params.effectiveTo && params.effectiveTo < params.today) return 'disabled';
  return 'active';
}

function buildReminderWorkflowAllowedActions(params: {
  rowStatus: 'active' | 'disabled';
  ...
  editableForm: Record<string, unknown> | null;
}): ReminderWorkflowRowAction[] {
  const basePayload = {
...
function buildOperationalReminderPoliciesFromLegalTable(
  legalTable: Array<Record<string, unknown>>,
  packContext?: {
    countries?: unknown[];
  ...
  for (const lv of legalTable) {
    if (!isOperationalCommunicationLegalValueRow(lv)) continue;
    const valueKey = String(lv.value_key ?? '');
    if (valueKey !== 'comm.reminder.policy') continue;

    const versions = Array.isArray(lv.versions) ? (lv.versions as Record<string, unknown>[]) : [];
    for (const ver of versions) {
      const vpj = ver.value_payload_json;
      if (!isOperationalReminderPolicyPayload(vpj)) continue;

      let policy: ReturnType<typeof assertValidOperationalReminderPolicyPayload>;
      let parseError: string | null = null;
      try {
      ...
      for (const wf of policy.workflows as OperationalReminderWorkflow[]) {
        ...
        let editableForm: Record<string, unknown> | null = null;
        if (parseError === null) {
          try {
...
function buildOperationalReminderTemplatesFromLegalTable(
  legalTable: Array<Record<string, unknown>>,
): Array<Record<string, unknown>> {
  const out: Array<Record<string, unknown>> = [];
  for (const lv of legalTable) {
    ...
    for (const ver of versions) {
      ...
      let parseError: string | null = null;
      try {
        normalized = assertValidOperationalReminderTemplatePayload(vpj) as unknown as Record<string, unknown>;
      } catch (e) {
        parseError = e instanceof Error ? e.message : 'invalid_payload';
        normalized = {};
      }
...
function buildCommunicationPoliciesSlice(
  ...
  for (const row of operationalReminderTemplates) {
    if (row.parse_error) validationErrors.push(`template:${row.template_key}:${row.parse_error}`);
  }
  return {
...
async function fetchOwnerLegalControlPanelAuditSummary(): Promise<{ recent: OwnerLegalControlAuditRow[] }> {
...
export async function buildOwnerLegalControlPanelAggregate(
  ...
  opts?: {
    ...
    tax_knowledge_trainer_proposal_id?: string | null;
  }
): Promise<Record<string, unknown>> {
  const actor = await requireOwnerLegalWorkspaceActor(ctx);
  const allowedCountries = actorAssignedCountryCodes(actor);
  ...
  const trainerSlice = selectedCountryCode
    ? await buildKnowledgeTrainerSlice(selectedCountryCode, {
        ...
        tax_knowledge_proposal_id: opts?.tax_knowledge_trainer_proposal_id,
      })
  ...
  return {
    ...
    owner_panel_sections: isPlatformOwner
      ...
      },
      {
        ...
        description:
          'INV-13A Owner global invoice layout (versioned). Structure only — VAT rates stay in Legal Values / Country Pack.',
    ...
    docflow_communication_quick_actions: [
      ...
      },
      {
        ...
        payload: {
          ...
          status: 'optional draft|active|deprecated|disabled',
          value_payload_json:
            'string JSON object docflow_communication template see DocFlow Phase 11 owner contract',
        },
      },
    ],
