 ...
 */
export async function getLegalValueByKey(countryCode: string, key: string): Promise<LegalValue | null> {
  ...
  return (data as LegalValue | null) ?? null;
}

export async function resolveLegalValue(
  countryCode: string,
  key: string,
  date: string,
  rulesetId: string
): Promise<LegalValueVersion | null> {
  ...
  const rows = (data ?? []) as LegalValueVersion[];
  if (rows.length > 1) {
...
export async function assertNoOverlapLegalValueVersions(input: {
  ...
  const overlaps = (data ?? []).some((row: { id: string; effective_from: string; effective_to: string | null }) => {
    if (input.excludeVersionId && row.id === input.excludeVersionId) return false;
...
export async function assertLegalValueExists(countryCode: string, key: string): Promise<LegalValue> {
  const legalValue = await getLegalValueByKey(countryCode, key);
