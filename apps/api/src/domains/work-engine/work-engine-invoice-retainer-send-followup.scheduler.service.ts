...
export type RecurringSendFollowupScanSummary = {
  cycles_scanned: number;
  followups_emitted: number;
};

type ApprovedCycleRow = {
  id: string;
...
function resolveCycleProfile(
  row: ApprovedCycleRow,
): { represented_client_id: string; end_customer_id: string } {
...
export async function scanRecurringDocumentSendFollowupsForOrg(params: {
  ...
  batchSize?: number;
  dryRun?: boolean;
  schedulerCtx: RequestContext;
}): Promise<RecurringSendFollowupScanSummary> {
  const nowIso = params.nowIso ?? new Date().toISOString();
  const batchSize = params.batchSize ?? DEFAULT_BATCH_SIZE;
  const summary: RecurringSendFollowupScanSummary = {
    cycles_scanned: 0,
    followups_emitted: 0,
  };
  ...
  const rows = (data ?? []) as unknown as ApprovedCycleRow[];
  ...
  for (const row of rows) {
    ...
    if (!row.generated_draft_id) continue;
    ...
    if (intake?.work_item_id) summary.followups_emitted += 1;
  }
