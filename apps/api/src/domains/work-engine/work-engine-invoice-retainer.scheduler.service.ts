...
} from './work-engine-invoice-retainer.scheduler.pure.js';

const DEFAULT_BATCH_SIZE = 25;

export type RecurringDocumentSchedulerRunParams = {
  org_id?: string;
  today_iso?: string;
  batch_size?: number;
  dry_run?: boolean;
  scheduler_ctx?: RequestContext;
};

export type RecurringDocumentSchedulerRunSummary = {
  status: typeof RECURRING_SCHEDULER_STATUS_ACTIVE;
...
type SchedulerProfileRow = {
  ...
  last_scheduler_cycle_key: string | null;
};
...
function buildSchedulerIssuerScope(
  orgId: string,
  actorUserId: string,
...
async function listRecurringSchedulerOrgIds(singleOrgId?: string): Promise<string[]> {
  ...
  for (const row of data ?? []) {
    const id = String((row as { organization_id?: string }).organization_id ?? '').trim();
    if (id) seen.add(id);
  }
  return [...seen].sort();
}

async function loadActiveProfilesForOrg(orgId: string, limit: number): Promise<SchedulerProfileRow[]> {
  const { data, error } = await supabaseAdmin
    .from('income_recurring_document_profiles')
  ...
  return (data ?? []) as SchedulerProfileRow[];
}

function parseSnapshot(raw: unknown): RecurringDocumentTemplateSnapshot | null {
  if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;
  const o = raw as RecurringDocumentTemplateSnapshot;
  if (o.snapshot_kind !== 'document_template_snapshot') return null;
  return o;
}
...
export async function runWorkEngineRecurringDocumentScheduler(
  params?: RecurringDocumentSchedulerRunParams,
): Promise<RecurringDocumentSchedulerRunSummary> {
  ...
  const summary: RecurringDocumentSchedulerRunSummary = {
  ...
  for (const orgId of orgIds) {
    ...
    for (const profile of profiles) {
      try {
        ...
        const cycleKey = buildRecurringSchedulerCycleKey(profile.id, scheduledDocumentDate);
        const alreadyProcessed = isRecurringSchedulerPeriodProcessed(
          profile.last_scheduler_cycle_key,
          cycleKey,
        );
        if (!due || alreadyProcessed) continue;
        ...
        if (outcome === 'created') summary.recurring_drafts_created += 1;
        if (outcome === 'failed') summary.recurring_failures += 1;
      } catch (err) {
