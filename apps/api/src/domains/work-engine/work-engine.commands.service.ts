...
import {
  ...
  approveSendReminderCandidate,
  cancelReminderCandidate,
  editReminderCandidate,
  loadReminderCandidate,
  parseReminderSnoozePreset,
  snoozeReminderCandidate,
} from './work-engine.reminder-review.service.js';
import {
  assertCollectionReminderApprovable,
  buildCollectionReminderReviewAggregate,
} from './work-engine-collection-reminder-review.read-model.service.js';
...
function parseRefreshAggregateKey(
  payload: WorkEngineCommandPayload,
): 'foundation' | 'queue' | 'collection_reminder_review' | 'invoice_collection_control' {
  ...
  if (raw === REFRESH_COLLECTION_REVIEW) return 'collection_reminder_review';
  if (raw === REFRESH_COLLECTION_CONTROL) return 'invoice_collection_control';
  throw badRequest(
    `Invalid refresh_aggregate: use '${REFRESH_FOUNDATION}', '${REFRESH_QUEUE}', '${REFRESH_COLLECTION_REVIEW}', '${REFRESH_COLLECTION_CONTROL}', or omit`,
...
function reminderViewerContext(ctx: RequestContext) {
  if (!ctx.membership) return null;
  return {
    userId: ctx.user.id,
    permissions: ctx.membership.permissions ?? [],
...
async function resolveReminderCommandRefreshPayload(
  orgId: string,
  payload: WorkEngineCommandPayload,
  candidateId: string,
): Promise<WorkEngineCommandPayload> {
 ...
 */
function assertEscalationCommandEnabled(
  ctx: RequestContext,
  current: WorkItemRow,
  command: QueueEscalationCommandKind,
): void {
  const viewer = viewerQueueContext(ctx);
  if (!viewer) throw forbidden('Organization membership required');
  const row = computeEscalationCommands({ row: current, viewer }).find((c) => c.command === command);
  if (!row?.enabled) {
    throw badRequest(
      row?.reason ?? `Escalation command '${command}' is not allowed for this work item`,
      'escalation_command_not_allowed',
    );
  }
}
...
export async function executeWorkEngineCommand(
  ...
  switch (command) {
    ...
    case 'edit_reminder_candidate': {
      const candidateId = reqString(payload, 'reminder_candidate_id');
      const refreshPayload = await resolveReminderCommandRefreshPayload(
        orgId,
        payload,
        candidateId,
      );
    ...
    case 'approve_reminder_candidate': {
      ...
      const refreshPayload = await resolveReminderCommandRefreshPayload(
        orgId,
        {
          ...payload,
          refresh_aggregate:
            asOptionalString(payload.refresh_aggregate) ?? REFRESH_COLLECTION_REVIEW,
        },
        candidateId,
      );
      return executeWithCommandIdempotency(ctx, orgId, command, refreshPayload, async () => {
