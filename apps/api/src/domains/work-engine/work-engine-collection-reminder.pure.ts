...
export const INVOICE_COLLECTION_FOLLOWUP_WORK_TYPE = 'invoice_collection_followup' as const;
...
export type CollectionReminderInvoiceTruth = {
  ...
  payment_state_key: 'unpaid' | 'partial' | 'paid' | string;
...
export function shouldCreateCollectionReminderCandidate(params: {
  workType: string;
  workState: string;
  stillOpenOverdue: boolean;
}): boolean {
  if (params.workType !== INVOICE_COLLECTION_FOLLOWUP_WORK_TYPE) return false;
  if (params.workState === 'done' || params.workState === 'archived') return false;
...
}): CollectionReminderInvoiceTruth {
  return {
...
export const COLLECTION_REMINDER_REVIEW_AGGREGATE_KEY =
  'collection_reminder_review_aggregate' as const;

/** INV-4C — approve stops at approved; send is INV-4D. */
export const COLLECTION_REMINDER_APPROVE_COMMAND = 'approve_reminder_candidate' as const;

export type CollectionReminderApproveBlockReasonKey =
  | 'invoice_paid'
  | 'no_remaining_balance'
  | 'not_overdue'
  | 'not_collection_work_item'
  | 'candidate_terminal'
  | 'candidate_not_approvable_status'
...
export function resolveCollectionReminderApproveGate(params: {
  ...
  allowed: boolean;
  reason_key: CollectionReminderApproveBlockReasonKey | null;
  ...
  if (params.paymentStateKey === 'paid' || params.remainingBalance <= 0) {
    return {
      allowed: false,
      reason_key:
        params.paymentStateKey === 'paid' ? 'invoice_paid' : 'no_remaining_balance',
...
export function isCollectionReminderMutatableStatus(status: string): boolean {
  return (
...
export const COLLECTION_REMINDER_SEND_COMMAND = 'send_collection_reminder' as const;

export const COLLECTION_REMINDER_DELIVERY_SOURCE_MODULE = 'work_engine' as const;
export const COLLECTION_REMINDER_DELIVERY_ENTITY_TYPE = 'work_reminder_candidate' as const;

export type CollectionReminderSendBlockReasonKey =
  | CollectionReminderApproveBlockReasonKey
  | 'candidate_not_sendable_status'
  | 'missing_recipient'
  | 'already_sent';
...
export function resolveCollectionReminderSendGate(params: {
  ...
  allowed: boolean;
  reason_key: CollectionReminderSendBlockReasonKey | null;
  disabled_reason: string | null;
} {
  ...
  const approveGate = resolveCollectionReminderApproveGate({
    ...
    candidateStatus:
      params.candidateStatus === 'approved' || params.candidateStatus === 'delivery_failed'
        ? 'pending_review'
...
export type InvoiceCollectionControlStatusKey =
  | 'waiting_review'
  | 'approved'
  | 'sending'
  | 'sent'
...
export function resolveInvoiceCollectionControlStatus(params: {
  ...
  collectionWorkState: string | null;
  paymentStateKey: string;
  remainingBalance: number;
  latestCandidateStatus: string | null;
  wasReopened?: boolean;
}): InvoiceCollectionControlStatusKey {
  if (params.paymentStateKey === 'paid' && params.remainingBalance <= 0) {
  ...
  if (params.wasReopened) return 'reopened';
  if (!params.collectionActive) return 'no_active_collection';

  const cs = params.latestCandidateStatus;
  if (cs === 'pending_review' || cs === 'edited' || cs === 'snoozed') return 'waiting_review';
  if (cs === 'approved') return 'approved';
  if (cs === 'sending') return 'sending';
  if (cs === 'sent') return 'waiting_payment';
  if (cs === 'delivery_failed') return 'delivery_failed';
  return 'waiting_payment';
}

export const INVOICE_COLLECTION_CONTROL_AGGREGATE_KEY =
  'invoice_collection_control_aggregate' as const;

export const INCOME_WORK_EVENT_INVOICE_PAID = 'income.invoice_paid' as const;
export const INCOME_WORK_EVENT_INVOICE_PARTIALLY_PAID = 'income.invoice_partially_paid' as const;

export function isIncomeInvoicePaidFactEventType(eventType: string): boolean {
  return (
    eventType === INCOME_WORK_EVENT_INVOICE_PAID ||
