...
export async function beginWorkEngineCommandIdempotency(args: {
  ...
  const row = existing as { command_type: string; completed_at: string | null } | null;
  ...
  if (row.completed_at) return { kind: 'replay' };
...
export async function abortWorkEngineCommandIdempotency(leaseRowId: string): Promise<void> {
  const { error } = await supabaseAdmin
    .from('work_engine_command_idempotency')
    .delete()
    .eq('id', leaseRowId);
