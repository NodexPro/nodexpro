...
export const AUTO_ESCALATION_SOURCE: EscalationSource = 'sla_breached';

export const AUTO_ESCALATION_REASON =
  'SLA breached and requires manager attention.';

export type AutoEscalationOwnerCandidate = {
  user_id: string;
  role_code: string;
};
...
export function shouldAutoEscalateForSla(params: {
  ...
  sla_status: string;
  has_breached_obligation: boolean;
}): boolean {
  if (params.work_state === 'escalated' || params.work_state === 'done' || params.work_state === 'archived') {
    return false;
  }
  if (params.sla_status === 'breached') return true;
  if (params.has_breached_obligation) return true;
  return false;
}

/** Backend-only owner pick order for system_rule escalation (MVP). */
export function resolveAutoEscalationOwnerId(
  workItem: Pick<
    WorkItemRow,
    'escalation_owner_id' | 'reviewer_user_id' | 'assigned_user_id' | 'owner_user_id'
  >,
  members: AutoEscalationOwnerCandidate[],
): string | null {
  const byUser = new Map(members.map((m) => [m.user_id, m.role_code]));

  const tryId = (id: string | null): string | null => {
    if (!id || !byUser.has(id)) return null;
    return id;
  };
  ...
  if (assigneeId && byUser.has(assigneeId)) {
    ...
    if (role === 'owner' || role === 'admin') return assigneeId;
  }
...
export function buildEscalationPriorStateTooltip(priorState: WorkState | null): string | null {
  if (!priorState) return null;
  return `Was ${workStateLabelForTooltip(priorState)}`;
}

function workStateLabelForTooltip(state: WorkState): string {
  switch (state) {
    case 'new':
      return 'New';
    case 'assigned':
