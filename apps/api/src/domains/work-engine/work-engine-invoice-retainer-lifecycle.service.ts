...
import {
  emitRecurringDocumentApprovedWorkEvent,
} from './work-engine-invoice-retainer-bridge.js';

type CycleApprovalRow = {
  id: string;
...
async function loadCycleForApproval(params: {
  ...
  draftId?: string | null;
  scheduledDocumentDate?: string | null;
  cycleId?: string | null;
}): Promise<CycleApprovalRow> {
  let q = supabaseAdmin
    .from('income_recurring_document_cycles')
    .select(
      'id, organization_id, recurring_profile_id, scheduled_document_date, generated_draft_id, generated_document_id, status, approved_at',
    )
    .eq('organization_id', params.orgId)
    .eq('recurring_profile_id', params.profileId);

  if (params.cycleId) q = q.eq('id', params.cycleId);
  else if (params.draftId) q = q.eq('generated_draft_id', params.draftId);
  ...
  const row = data as CycleApprovalRow | null;
  if (!row) throw notFound('Recurring cycle not found');
...
export async function findActiveRecurringInvoiceReviewWorkItem(params: {
  ...
  const row = (data as WorkItemRow | null) ?? null;
  if (
    row &&
