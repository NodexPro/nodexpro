...
export type IncomeDocumentSentFactConsumption = {
...
async function loadRetainerCyclesForIncomeDocument(params: {
  ...
  const cycles = new Map<string, RetainerCycleRow>();
  for (const row of (byDocument.data ?? []) as RetainerCycleRow[]) {
  ...
  const sourceDraftId = (documentRow as { source_draft_id: string | null } | null)
    ?.source_draft_id;
  if (sourceDraftId) {
    ...
    for (const row of (byDraft.data ?? []) as RetainerCycleRow[]) {
      cycles.set(row.id, row);
    }
  }
...
export async function consumeIncomeDocumentSentFact(params: {
  ...
  actorUserId: string | null;
}): Promise<IncomeDocumentSentFactConsumption> {
