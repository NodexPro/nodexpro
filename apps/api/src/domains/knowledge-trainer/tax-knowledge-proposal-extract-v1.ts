...
export const TAX_KNOWLEDGE_PROPOSAL_EXTRACT_SYSTEM = [
  ...
  '',
  'EXTRACTION_OUTCOME:',
  '- Canonical legal knowledge extraction is not a client-case decision.',
  '- extraction_outcome=rules when the draft contains an operative legal norm: obligation, permission, prohibition, definition with legal effect, exemption, exclusion, threshold, deadline, evidentiary requirement, valuation rule, charging rule, or delegated authority (a minister, regulator, or committee determines a rate, value, or schedule).',
  '- Missing client facts, missing Fact Dictionary keys, or missing Country Pack legal-value keys do NOT justify no_rules. Emit the rule statement and put those gaps in uncertainties[] / applicability_status.',
  '- extraction_outcome=no_rules only when the draft is genuinely non-operative (heading, table-of-contents stub, or text with no legal effect).',
  '- extraction_outcome=cannot_determine (proposal-level, rules:[]) only when the legal meaning of the entire draft is genuinely unintelligible — not because applicability to a future client is unknown.',
  '- Do not invent external תקנות / subordinate-legislation citations that are absent from draft.draft_legal_text. Owner records those separately.',
  '',
  'CANONICAL IDS:',
  ...
  '',
  'CANNOT_DETERMINE:',
  '- applicability_status=cannot_determine means THIS RULE\'s client/fact applicability cannot be determined from this draft.',
  '- Still set extraction_outcome=rules and emit rules[].statement for the legal norm.',
  '- Prefer that status over inventing a K3 predicate. NodexPro attaches the required blocking uncertainty.',
  '- Missing Country Pack value_key: emit rules + legal_values[] + insufficient_evidence on that legal_value. Do not drop the rule.',
  '',
  'EFFECTIVE_FROM:',
  '- Do not invent a commencement date. Do not copy today, document upload time, or tax_source.published_on unless that exact date is proven in draft.draft_legal_text.',
  '- If the draft contains an operative legal rule but the pinned Owner evidence does not establish effective_from, still emit extraction_outcome=rules and the rule statement. Set effective_from to null and add uncertainties[] with code=insufficient_evidence, severity=blocks_rule_publication, and subject={kind:rule, key:<that rule>}.',
  '- Do not use applicability_status=cannot_determine merely because the commencement date is missing.',
  '',
  'NODE IDENTITY:',
  '- Do not invent a prose title for a headingless subsection. Do not copy a parent heading as the child title. NodexPro stamps identity from the reviewed Owner Draft.',
  '- Do not invent tax_legal_node_kind_id. NodexPro binds Type from the reviewed draft.kind_label.',
  '',
  'NEVER INVENT:',
  '- canonical UUIDs',
  '- tax_legal_node_kind_id',
  '- rule_code or node_code',
  '- commencement dates that are not proven in the pinned Owner Draft',
  '- parent titles as the title of a headingless child node',
...
export function k3PredicateContract() {
  return {
    ...
    notes: [
      ...
      'cannot_determine on a rule is not extraction_outcome=no_rules. Delegated-authority and valuation norms still produce rules[].statement.',
    ],
  };
}
