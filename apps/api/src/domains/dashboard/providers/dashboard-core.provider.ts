...
export const dashboardCoreProvider: DashboardProvider = {
  code: CORE_PROVIDER_CODE,
  required: true,
  supports: () => true,
  async getOverviewPart(ctx: RequestContext): Promise<DashboardOverviewPartial> {
    ...
    const createdRows = (createdRowsRes.data ?? []) as Array<{ created_at: string }>;
    for (const row of createdRows) {
      const d = new Date(row.created_at);
      const k = monthKeyUTC(d);
      if (countsByMonth.has(k)) countsByMonth.set(k, (countsByMonth.get(k) ?? 0) + 1);
    }
