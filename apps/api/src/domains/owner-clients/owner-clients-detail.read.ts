...
import {
  ...
  buildCountLabel,
  computeOrgSubscriptionStatus,
  NO_DATA_LABEL,
  NOT_MEASURED_LABEL,
  resolveHealthFromIssue,
  type OwnerClientDetailAggregate,
...
export async function loadOwnerClientDetailData(organizationId: string): Promise<OwnerClientDetailAggregate> {
  const organizations = await loadOrganizations();
  const org = organizations.find((o) => o.id === organizationId);
  if (!org) throw notFound('Organization not found', 'ORGANIZATION_NOT_FOUND');
  ...
  const subscription = computeOrgSubscriptionStatus(
    moduleRows.map((m) => ({ entitlement_status: m.entitlement_status })),
  );
  const planNames = [...new Set(subscriptions.map((s) => s.plan_name).filter(Boolean) as string[])];
