...
import {
  ...
  buildUsageLabel,
  type OwnerClientActiveModule,
  type OwnerClientFilters,
  type OwnerClientsListAggregate,
} from './owner-clients.pure.js';
import {
  computeEntitlementStatus,
  loadCommercialModules,
  loadCountByOrg,
  loadCountryLabels,
  loadLastActivityByOrg,
  loadOrgHealthIssuesByOrg,
  loadOrgModuleActivations,
  loadOrgOwners,
  loadOrganizations,
  loadOrgSubscriptions,
  loadOrgValidTrials,
  type OrgModuleActivation,
  type OrgSubscription,
} from './owner-clients.shared-read.js';

function buildOrgMrr(subscriptions: OrgSubscription[]): {
  value: number | null;
  ...
  const currencies = new Set(active.map((s) => s.currency).filter(Boolean) as string[]);
  const planNames = [...new Set(active.map((s) => s.plan_name).filter(Boolean) as string[])];
...
function buildActiveModules(
  orgId: string,
  activations: Map<string, OrgModuleActivation[]>,
  subscriptions: OrgSubscription[],
  orgHasValidTrial: boolean,
  commercialModules: Awaited<ReturnType<typeof loadCommercialModules>>,
): OwnerClientActiveModule[] {
...
export async function loadOwnerClientsListData(
  filters?: Partial<OwnerClientFilters> | null,
): Promise<OwnerClientsListAggregate> {
  const [
    organizations,
    countryLabels,
    commercialModules,
  ...
  const rows = organizations.map((org) => {
    ...
    const active_modules = buildActiveModules(
      org.id,
      activationsByOrg,
      subscriptions,
      validTrials.has(org.id),
      commercialModules,
    );
