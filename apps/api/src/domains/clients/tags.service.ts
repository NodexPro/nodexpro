...
export async function listTags(ctx: RequestContext, orgId: string) {
  assertOrg(ctx, orgId);
...
export async function createTag(ctx: RequestContext, orgId: string, body: { name: string; code?: string | null; color?: string | null }) {
  assertOrg(ctx, orgId);
...
export async function listTagsForClient(ctx: RequestContext, orgId: string, clientId: string) {
  assertOrg(ctx, orgId);
  assertPermission(ctx, 'clients:read');
  ...
  const rows = (links ?? []) as unknown as { tags: { id: string; name: string; code: string | null; color: string | null } | null }[];
  return rows.map((r) => r.tags).filter((t): t is NonNullable<typeof t> => t != null);
}

export async function addTagToClient(ctx: RequestContext, orgId: string, clientId: string, tagId: string) {
  assertOrg(ctx, orgId);
  assertPermission(ctx, 'clients:write');
...
export async function removeTagFromClient(ctx: RequestContext, orgId: string, clientId: string, tagId: string) {
  assertOrg(ctx, orgId);
  assertPermission(ctx, 'clients:write');
  ...
  const { data: link } = await supabaseAdmin
    ...
    .eq('tag_id', tagId)
    .single();
  if (!link) throw forbidden('Tag link not found');
