...
export interface PostIncomeDocumentToAccountingInput {
  ...
  totals_snapshot_json: Record<string, unknown>;
  lines_snapshot_json: unknown[];
  description_note?: string | null;
}

export interface PostIncomeDocumentToAccountingResult {
  requires_posting: boolean;
  accounting_posting_status: 'posted' | 'not_required';
...
async function findExistingPosting(
...
): Promise<{ entry_id: string; link_id: string } | null> {
  const { data: links } = await supabaseAdmin
    ...
    .limit(1);
  const link = (links ?? [])[0] as { id: string; accounting_entry_id: string } | undefined;
  if (!link) return null;
  return { entry_id: link.accounting_entry_id, link_id: link.id };
}

async function resolvePeriodForIssueDate(
  ctx: RequestContext,
  organizationId: string,
  issueDate: string,
  currency: string,
  ...
  const existing = (periods ?? [])[0] as { id: string } | undefined;
...
async function resolveIncomeCategoryId(ctx: RequestContext, organizationId: string): Promise<string> {
...
async function finalizeEntry(ctx: RequestContext, organizationId: string, entryId: string): Promise<void> {
...
async function auditAccountingEntry(
  ctx: RequestContext,
  organizationId: string,
  entryId: string,
...
async function postRequiredEntries(
  ctx: RequestContext,
  ...
  const periodId = await resolvePeriodForIssueDate(
    ctx,
  ...
  for (const spec of plan.entries) {
    ...
    if (!primaryLinkId) primaryLinkId = link.id;
 ...
 */
export async function postIncomeDocumentToAccountingBase(
  ctx: RequestContext,
  input: PostIncomeDocumentToAccountingInput,
): Promise<PostIncomeDocumentToAccountingResult> {
