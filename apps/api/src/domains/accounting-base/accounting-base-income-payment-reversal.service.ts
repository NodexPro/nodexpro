...
import {
  ...
  sumPostedAllocationsForIncomeDocument,
...
export const ACCOUNTING_BASE_REVERSE_INCOME_PAYMENT_ALLOCATION_RPC =
  'accounting_base_reverse_income_payment_allocation' as const;

export type AccountsReceivableAffectedRefresh = {
  ...
  aging_bucket_key: 'current' | '1_30' | '31_60' | '61_90' | '90_plus';
  aging_label: string;
  aging_label_he: string;
  notes: string[];
};

export type ReverseIncomePaymentAllocationResponse = {
  ok: true;
  ...
  additional_refreshed: Array<
    | {
        aggregate_key: 'invoice_lifecycle_aggregate';
        aggregate: InvoiceLifecycleAggregate;
      }
    | {
        aggregate_key: 'accounts_receivable_affected';
        aggregate: AccountsReceivableAffectedRefresh;
      }
  >;
};

type AtomicReverseRpcResult = {
  replay: boolean;
  original_allocation_id: string;
  reversal_allocation_id: string;
  payment_id: string;
...
function reqReason(value: unknown): string {
  const s = String(value ?? '').trim();
  if (!s) throw badRequest('reason required');
  if (s.length > 500) throw badRequest('reason must be at most 500 characters');
  return s;
}

function mapReverseRpcError(err: { message?: string; code?: string }): never {
  const message = String(err.message ?? 'Payment allocation reversal failed');
  if (/Allocation not found/i.test(message)) throw notFound('Allocation not found');
  if (/Income document not found/i.test(message)) throw notFound('Income document not found');
  ...
  if (/reason required/i.test(message)) throw badRequest('reason required');
...
export async function callReverseIncomePaymentAllocationRpc(input: {
  ...
  createdBy: string;
  reason: string;
}): Promise<AtomicReverseRpcResult> {
  const { data, error } = await supabaseAdmin.rpc(
    ACCOUNTING_BASE_REVERSE_INCOME_PAYMENT_ALLOCATION_RPC,
    {
  ...
  if (error) mapReverseRpcError(error);
...
async function buildAccountsReceivableAffectedRefresh(params: {
  ...
  todayIso?: string;
}): Promise<AccountsReceivableAffectedRefresh> {
  const { data, error } = await supabaseAdmin
    .from('income_documents')
    .select(
      'id, document_status, document_type, due_date, currency, totals_snapshot_json',
    )
...
export async function executeReverseIncomePaymentAllocation(
  ctx: RequestContext,
  organizationId: string,
  payload: Record<string, unknown>,
): Promise<ReverseIncomePaymentAllocationResponse> {
  assertOrgInContext(ctx, organizationId);
  requirePaymentWrite(ctx);

  const allocationId = String(payload.allocation_id ?? '').trim();
  if (!allocationId) throw badRequest('allocation_id required');
  ...
  const { data: allocRaw, error: allocErr } = await supabaseAdmin
    ...
    .select(
      'id, organization_id, payment_id, source_module, source_entity_id, allocated_amount, currency, status, reversal_of_allocation_id',
    )
    .eq('organization_id', organizationId)
    .eq('id', allocationId)
    .maybeSingle();
  throwIfSupabaseError(allocErr, 'reverseAllocationLoad');
  if (!allocRaw) throw notFound('Allocation not found');

  const alloc = allocRaw as {
    id: string;
  ...
  try {
    ...
    const paymentCase = await buildIncomeInvoicePaymentCaseAggregate(
      ctx,
      organizationId,
      rpc.income_document_id,
    );
