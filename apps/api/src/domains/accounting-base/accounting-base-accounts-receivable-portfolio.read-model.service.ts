...
import {
  ...
  clampArPagination,
  parseArCurrencyFilter,
  parseArOptionalIsoDate,
  parseArOverdueFilter,
  parseArPaymentStateFilter,
} from './accounting-base-accounts-receivable.pure.js';
import {
...
import {
  ...
  parseOptionalUuid,
} from './accounting-base-accounts-receivable-portfolio.pure.js';
import {
  ACCOUNTING_BASE_AR_PORTFOLIO_GRAIN_RPC,
  ACCOUNTING_BASE_AR_PORTFOLIO_ROWS_RPC,
  ACCOUNTS_RECEIVABLE_PORTFOLIO_AGGREGATE_KEY,
  type AccountsReceivablePortfolioAggregate,
  type AccountsReceivablePortfolioGrain,
  type AccountsReceivablePortfolioRow,
} from './accounting-base-accounts-receivable-portfolio.types.js';
import { assertOrgInContext } from './accounting-base.guards.js';
import type { AccountsReceivableAgingBucketKey } from './accounting-base-accounts-receivable.types.js';

function hasPerm(ctx: RequestContext, code: string): boolean {
  return (ctx.membership?.permissions ?? []).includes(code);
}

function assertPortfolioPermissions(ctx: RequestContext): void {
  const canFinance =
    hasPerm(ctx, ACCOUNTING_BASE_VIEW_PERMISSION) ||
  ...
  if (!canFinance) throw forbidden('accounting_base.view required');
  if (!canOfficeClients) throw forbidden('clients:read or income.view required for office portfolio');
}

const AGING_KEYS = new Set(['current', '1_30', '31_60', '61_90', '90_plus']);

function mapGrainRow(raw: Record<string, unknown>): AccountsReceivablePortfolioGrain | null {
  ...
  if (!clientId || !AGING_KEYS.has(aging)) return null;
  if (payment !== 'unpaid' && payment !== 'partial') return null;
  return {
    represented_client_id: clientId,
    currency: String(raw.currency ?? 'ILS').trim().toUpperCase() || 'ILS',
    aging_bucket_key: aging as AccountsReceivableAgingBucketKey,
...
async function batchLoadClientDisplayNames(
  orgId: string,
  clientIds: string[],
...
function toPortfolioRow(
...
): AccountsReceivablePortfolioRow | null {
  const id = String(raw.income_document_id ?? '').trim();
  const clientId = String(raw.represented_client_id ?? '').trim();
  const aging = String(raw.aging_bucket_key ?? '').trim();
  const payment = String(raw.payment_state_key ?? '').trim();
  if (!id || !clientId || !AGING_KEYS.has(aging)) return null;
  if (payment !== 'unpaid' && payment !== 'partial') return null;
  ...
  return {
    ...
    due_state_key:
      dueState === 'overdue' || dueState === 'not_due' || dueState === 'not_applicable'
        ? dueState
        : 'not_applicable',
    overdue: Boolean(raw.overdue),
    ...
    days_overdue: days,
    aging_bucket_key: aging as AccountsReceivableAgingBucketKey,
    aging_label: accountsReceivableAgingLabel(aging as AccountsReceivableAgingBucketKey),
    aging_label_he: accountsReceivableAgingLabelHe(aging as AccountsReceivableAgingBucketKey),
  };
}
...
export async function buildAccountsReceivablePortfolioAggregate(params: {
  ...
  todayIso?: string;
}): Promise<AccountsReceivablePortfolioAggregate> {
  ...
  const grains = ((grainRes.data ?? []) as Record<string, unknown>[])
    .map(mapGrainRow)
    .filter((g): g is AccountsReceivablePortfolioGrain => g != null);
  ...
  const resolvedTotal =
    rawRows.length > 0 ? Number(rawRows[0]?.total_count ?? 0) || 0 : totalFromGrain;
  ...
  const rows = rawRows
    .map((r) =>
      toPortfolioRow(r, displayNames.get(String(r.represented_client_id ?? '')) ?? null),
    )
    .filter((r): r is AccountsReceivablePortfolioRow => r != null);
  ...
  return {
    ...
    meta: {
      ...
      notes: [
        ...
        ...unsupported.map(
          (t) => `Collectible type "${t}" excluded — AB payment not supported.`,
        ),
