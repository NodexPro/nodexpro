...
import {
  ACCOUNTS_RECEIVABLE_AGING_BUCKET_KEYS,
...
import type {
  AccountsReceivablePortfolioCurrencyTotal,
...
export function resolveAccountsReceivableAgingBucketFromDueDate(params: {
  ...
  const days = Math.round(
    (Date.parse(`${today}T00:00:00.000Z`) - Date.parse(`${due}T00:00:00.000Z`)) / 86_400_000,
  );
...
export function buildPortfolioSummaryFromGrain(grains: AccountsReceivablePortfolioGrain[]): {
  ...
  let partial_count = 0;
  let overdue_count = 0;
  const byCurrency = new Map<string, AccountsReceivablePortfolioCurrencyTotal>();

  for (const g of grains) {
    open_invoice_count += g.invoice_count;
    if (g.payment_state_key === 'unpaid') unpaid_count += g.invoice_count;
    if (g.payment_state_key === 'partial') partial_count += g.invoice_count;
    if (g.overdue) overdue_count += g.invoice_count;
    ...
    cur.overdue_remaining_balance = roundMoney2(
      cur.overdue_remaining_balance + g.overdue_remaining_balance,
    );
...
export function buildPortfolioAgingFromGrain(
  grains: AccountsReceivablePortfolioGrain[],
): { buckets: AccountsReceivableAgingBucketSummary[] } {
  const counts = new Map<AccountsReceivableAgingBucketKey, number>();
  const money = new Map<AccountsReceivableAgingBucketKey, Map<string, number>>();
...
export function buildPortfolioClientsFromGrain(
  grains: AccountsReceivablePortfolioGrain[],
  displayNameByClientId: ReadonlyMap<string, string | null>,
): AccountsReceivableClientOutstanding[] {
  type Acc = {
    client_id: string;
    open_invoice_count: number;
    ...
    >;
    agingGrains: AccountsReceivablePortfolioGrain[];
  };
  ...
  for (const g of grains) {
    ...
    if (g.payment_state_key === 'unpaid') acc.unpaid_count += g.invoice_count;
    if (g.payment_state_key === 'partial') acc.partial_count += g.invoice_count;
    if (g.overdue) acc.overdue_count += g.invoice_count;
  ...
  const clients: AccountsReceivableClientOutstanding[] = Array.from(byClient.values()).map(
    (acc) => ({
      ...
      totals_by_currency: Array.from(acc.currencyMap.values()).sort((a, b) =>
        a.currency.localeCompare(b.currency),
      ),
