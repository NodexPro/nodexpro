...
} from './accounting-base-accounts-receivable-clients.pure.js';
import {
  AR_CANDIDATE_MAX,
...
}): AccountsReceivableAggregate['rows'][number]['allowed_actions'] {
  const canView = hasPerm(params.ctx, ACCOUNTING_BASE_VIEW_PERMISSION) || hasPerm(params.ctx, 'income.view');
  const canPay =
    hasPerm(params.ctx, ACCOUNTING_BASE_PAYMENT_WRITE_PERMISSION) && params.remainingBalance > 0;
...
export async function buildAccountsReceivableAggregate(params: {
  ...
  let q = supabaseAdmin
    ...
    .select(
      'id, organization_id, issuer_business_id, represented_client_id, income_customer_id, document_type, document_number, document_status, issue_date, due_date, currency, totals_snapshot_json, customer_snapshot_json',
    )
  ...
  const filtered = sortAccountsReceivableCandidates(
    filterAccountsReceivableCandidates(composed, filters),
  );
  ...
  const notes = [
    ...
    ...unsupportedCollectible.map(
      (t) => `Collectible type "${t}" excluded from open A/R V1 — no AB payment orchestration.`,
    ),
...
function emptyAggregate(params: {
  ...
  filters: AccountsReceivableAggregate['filters'];
  limit: number;
  offset: number;
  notes: string[];
