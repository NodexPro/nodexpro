...
import type {
  ...
  AccountsReceivableClientCurrencyTotal,
  AccountsReceivableClientOutstanding,
} from './accounting-base-accounts-receivable.types.js';

/** Minimal row shape for client grouping (avoids circular import with AR pure). */
export type ArClientGroupRow = {
...
export function accountsReceivableClientGroupKey(
  representedClientId: string | null | undefined,
): string {
  return representedClientId ?? '__self__';
}

export function resolveAccountsReceivableClientId(
  representedClientId: string | null | undefined,
): string | null {
...
type ClientAcc = {
  ...
  currencyMap: Map<string, AccountsReceivableClientCurrencyTotal>;
  rows: ArClientGroupRow[];
};

function emptyCurrency(currency: string): AccountsReceivableClientCurrencyTotal {
  return {
    currency,
    original_amount: 0,
    paid_amount: 0,
 ...
 */
export function buildAccountsReceivableClients(
  rows: ArClientGroupRow[],
  displayNameByClientKey: ReadonlyMap<string, string | null>,
): AccountsReceivableClientOutstanding[] {
  ...
  for (const row of rows) {
    ...
    if (row.payment_state_key === 'unpaid') acc.unpaid_count += 1;
    if (row.payment_state_key === 'partial') acc.partial_count += 1;
    if (row.overdue) acc.overdue_count += 1;
    acc.rows.push(row);
    ...
    if (row.overdue) {
      cur.overdue_remaining_balance = roundMoney2(
        cur.overdue_remaining_balance + row.remaining_balance,
      );
    }
    acc.currencyMap.set(currency, cur);
  }

  const clients: AccountsReceivableClientOutstanding[] = Array.from(byKey.entries()).map(
    ([key, acc]) => ({
...
export function sortAccountsReceivableClients(
...
export function buildArClientDisplayNameMapFromScope(scope: {
  ...
  if (scope.represented_client_id) {
    map.set(
      accountsReceivableClientGroupKey(scope.represented_client_id),
      scope.represented_client_label,
    );
  }
