...
import {
  accountsReceivableAgingLabel,
  accountsReceivableAgingLabelHe,
  resolveAccountsReceivableAgingBucket,
} from './accounting-base-accounts-receivable-aging.pure.js';
import type {
  AccountsReceivableAgingBucketFilter,
  AccountsReceivableAgingBucketKey,
  AccountsReceivableCurrencyTotal,
...
export function accountsReceivableSupportedDocumentTypes(): string[] {
...
export function accountsReceivableUnsupportedCollectibleTypes(): string[] {
  return COLLECTIBLE_CANDIDATE_TYPES.filter((t) => !isSupportedIncomePaymentDocumentType(t));
}
...
export function clampArPagination(rawLimit: unknown, rawOffset: unknown): {
  limit: number;
  offset: number;
} {
  let limit = Number(rawLimit ?? AR_DEFAULT_LIMIT);
  if (!Number.isFinite(limit) || limit <= 0) limit = AR_DEFAULT_LIMIT;
  if (limit > AR_MAX_LIMIT) limit = AR_MAX_LIMIT;
...
export function parseArPaymentStateFilter(raw: unknown): AccountsReceivablePaymentStateFilter {
  const v = String(raw ?? 'all_open').trim();
  if (v === 'unpaid' || v === 'partial' || v === 'all_open') return v;
  return 'all_open';
}

export function parseArOverdueFilter(raw: unknown): AccountsReceivableOverdueFilter {
...
export function parseArOptionalIsoDate(raw: unknown): string | null {
  if (raw == null || String(raw).trim() === '') return null;
  return parseIsoDate(raw);
}

export function parseArCurrencyFilter(raw: unknown): string | null {
...
export type ArComposedCandidate = {
  income_document_id: string;
  document_number: string;
  document_type: string;
  issue_date: string | null;
  due_date: string | null;
  /** Canonical office client identity; null in self mode (not income_customer). */
...
export function isOpenAccountsReceivableRow(row: Pick<ArComposedCandidate, 'remaining_balance'>): boolean {
  return roundMoney2(row.remaining_balance) > 0;
}

/** Attach aging_bucket_key from INV-2 overdue fields (pure; no DB). */
export function withAccountsReceivableAgingBucket<
  T extends Pick<ArComposedCandidate, 'overdue' | 'days_overdue'>,
>(row: T): T & { aging_bucket_key: AccountsReceivableAgingBucketKey } {
  return {
    ...row,
...
export function filterAccountsReceivableCandidates(
  rows: ArComposedCandidate[],
  filters: {
  ...
  return rows.filter((row) => {
    if (!isOpenAccountsReceivableRow(row)) return false;
    if (filters.payment_state === 'unpaid' && row.payment_state_key !== 'unpaid') return false;
    if (filters.payment_state === 'partial' && row.payment_state_key !== 'partial') return false;
    if (filters.overdue === 'true' && !row.overdue) return false;
    if (filters.overdue === 'false' && row.overdue) return false;
    if (filters.aging_bucket !== 'all' && row.aging_bucket_key !== filters.aging_bucket) return false;
    if (filters.currency && row.currency !== filters.currency) return false;
    if (filters.due_date_from && (!row.due_date || row.due_date < filters.due_date_from)) return false;
    if (filters.due_date_to && (!row.due_date || row.due_date > filters.due_date_to)) return false;
...
export function sortAccountsReceivableCandidates(rows: ArComposedCandidate[]): ArComposedCandidate[] {
  return [...rows].sort((a, b) => {
    ...
    if (issA !== issB) return issA.localeCompare(issB);
    return a.document_number.localeCompare(b.document_number, 'he');
  });
}

export function paginateAccountsReceivable(
  rows: ArComposedCandidate[],
  limit: number,
  offset: number,
): { page: ArComposedCandidate[]; total_count: number; has_more: boolean } {
...
export function buildAccountsReceivableTotalsByCurrency(
  rows: ArComposedCandidate[],
): AccountsReceivableCurrencyTotal[] {
  const map = new Map<string, AccountsReceivableCurrencyTotal>();
...
export function buildAccountsReceivableSummary(rows: ArComposedCandidate[]): {
...
export function toAccountsReceivableRow(
  candidate: ArComposedCandidate,
  allowed_actions: AccountsReceivableRow['allowed_actions'],
): AccountsReceivableRow {
  return {
