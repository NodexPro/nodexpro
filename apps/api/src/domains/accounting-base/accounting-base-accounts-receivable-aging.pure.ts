...
import type {
  ...
  AccountsReceivableAgingCurrencyTotal,
...
export const ACCOUNTS_RECEIVABLE_AGING_BUCKET_KEYS = [
  ...
  '61_90',
  '90_plus',
] as const satisfies readonly AccountsReceivableAgingBucketKey[];

const AGING_LABEL_HE: Record<AccountsReceivableAgingBucketKey, string> = {
  current: 'שוטף',
  '1_30': '1–30',
  '31_60': '31–60',
  '61_90': '61–90',
...
const AGING_LABEL_EN: Record<AccountsReceivableAgingBucketKey, string> = {
  current: 'Current',
  '1_30': '1–30 days',
  '31_60': '31–60 days',
  '61_90': '61–90 days',
...
export function resolveAccountsReceivableAgingBucket(params: {
  ...
  if (!params.overdue) return 'current';
...
export function accountsReceivableAgingLabelHe(key: AccountsReceivableAgingBucketKey): string {
  return AGING_LABEL_HE[key];
}

export function accountsReceivableAgingLabel(key: AccountsReceivableAgingBucketKey): string {
  return AGING_LABEL_EN[key];
}

export function parseArAgingBucketFilter(
  raw: unknown,
): AccountsReceivableAgingBucketKey | 'all' {
  const v = String(raw ?? 'all').trim();
  if (v === 'all') return 'all';
  if ((ACCOUNTS_RECEIVABLE_AGING_BUCKET_KEYS as readonly string[]).includes(v)) {
    return v as AccountsReceivableAgingBucketKey;
  }
  return 'all';
}

/** Aging summary over filtered open rows — remaining_balance by bucket × currency only. */
export function buildAccountsReceivableAgingSummary(
  rows: Array<{
  ...
  const byBucket = new Map<
    AccountsReceivableAgingBucketKey,
    Map<string, AccountsReceivableAgingCurrencyTotal>
  >();
  ...
  return {
    buckets: ACCOUNTS_RECEIVABLE_AGING_BUCKET_KEYS.map((bucket_key) => ({
      ...
      totals_by_currency: Array.from(byBucket.get(bucket_key)!.values()).sort((a, b) =>
