...
export const ANNUAL_SYSTEM_DOCUMENT_TEMPLATES: ReadonlyArray<{ system_key: string; document_name_he: string; sort_order: number }> = [
...
export type AnnualTabScope = 'annual_report' | 'capital_declaration';

const CAPITAL_DECLARATION_SYSTEM_DOCUMENT_TEMPLATES: ReadonlyArray<{
  system_key: string;
...
/** שורות מוכנות לרינדור בבלוק הבקרה העליון (ללא לוגיקה בצד לקוח) */
export type AnnualControlBlockDto = {
  days_left_caption_he: string;
  progress_caption_he: string;
  /** מחרוזת מוכנה, למשל "5 / 12" */
  progress_received_total_he: string;
};

export type AnnualTabResponse = {
  tab_key: AnnualTabScope;
  tab_title_he: string;
  read_model_version: number;
  permissions: { can_view: boolean; can_edit: boolean };
  /** מצב בקרה — נקבע בשרת בלבד */
  status: AnnualControlStatusDto;
  missing_documents: string[];
  completion_percent: number;
  deadline_info: AnnualDeadlineInfoDto;
  control_block: AnnualControlBlockDto;
  risk_indicator: { label_he: string };
  missing_documents_section_title_he: string;
  /** כותרת כרטיס סטטוס בלוח הבקרה */
  ...
  documents_table: {
    ...
    /** Ready-to-render סיכום — נגזר מהשורות באותו aggregate (בשרת בלבד) */
    summary: {
...
export type AnnualTabCommandBody = {
  type: string;
  payload?: Record<string, unknown>;
  expected_version: number;
  tab_scope?: AnnualTabScope;
};

export type AnnualAnnualCommandType =
  | 'add_annual_document_row'
  | 'update_annual_document_row'
  | 'remove_annual_document_row'
  | 'toggle_annual_document_received'
  | 'attach_annual_document_file'
  | 'remove_annual_document_file'
  | 'add_annual_submission_date'
  | 'update_annual_submission_date'
  | 'remove_annual_submission_date'
  | 'attach_annual_submission_file'
  | 'update_annual_notes'
  | 'copy_previous_capital_declaration';
...
function canViewAnnualTab(ctx: RequestContext): boolean {
  return hasPerm(ctx, 'annual_report_tab.view') || hasPerm(ctx, 'client_operations.view');
}

function canEditAnnualTab(ctx: RequestContext): boolean {
  return hasPerm(ctx, 'annual_report_tab.edit') || hasPerm(ctx, 'client_operations.edit');
}

function parseAnnualTabScope(v: unknown): AnnualTabScope {
  return v === 'capital_declaration' ? 'capital_declaration' : 'annual_report';
}

const LEGACY_SCOPE_MARKER: Record<AnnualTabScope, string> = {
  annual_report: '[scope:annual_report]',
  capital_declaration: '[scope:capital_declaration]',
};

function hasLegacyScopeMarker(note: string | null | undefined, scope: AnnualTabScope): boolean {
  return String(note ?? '').startsWith(LEGACY_SCOPE_MARKER[scope]);
}

function stripLegacyScopeMarker(note: string | null | undefined): string | null {
  const s = String(note ?? '');
  if (s.startsWith(LEGACY_SCOPE_MARKER.annual_report)) return s.slice(LEGACY_SCOPE_MARKER.annual_report.length).trim() || null;
  if (s.startsWith(LEGACY_SCOPE_MARKER.capital_declaration)) return s.slice(LEGACY_SCOPE_MARKER.capital_declaration.length).trim() || null;
  return note ?? null;
}

function attachLegacyScopeMarker(note: string | null, scope: AnnualTabScope, scoped: boolean): string | null {
  if (scoped) return note;
  const payload = note == null || note === '' ? '' : ` ${note}`;
  return `${LEGACY_SCOPE_MARKER[scope]}${payload}`.trim();
}

let annualScopeSupportCache: boolean | null = null;
async function supportsAnnualScopeColumns(): Promise<boolean> {
  if (annualScopeSupportCache != null) return annualScopeSupportCache;
  try {
...
function systemTemplatesForScope(
  scope: AnnualTabScope
): ReadonlyArray<{ system_key: string; document_name_he: string; description_he: string | null; sort_order: number; required: boolean }> {
  if (scope === 'capital_declaration') return CAPITAL_DECLARATION_SYSTEM_DOCUMENT_TEMPLATES;
...
function computeDocumentRowPresentation(
  received: boolean,
  fileAssetId: string | null
): { status: AnnualDocumentRowStatus; status_label_he: string; row_style: AnnualDocumentRowStyle } {
...
function submissionStatusLabelHe(code: string): string {
  const c = String(code ?? '').trim();
  if (c === 'submitted') return 'הוגש';
  if (c === 'late') return 'באיחור';
  if (c === 'extension') return 'בהארכה';
  if (c === 'draft') return 'טיוטה';
  return c || '—';
}
...
function buildAnnualDeadlineInfo(serverNow: Date): AnnualDeadlineInfoDto {
...
function buildAnnualControlState(
  rows: AnnualDocumentRowDto[],
  serverNow: Date
): {
  status: AnnualControlStatusDto;
  ...
  let status: AnnualControlStatusDto;
  let risk_label_he: string;
...
function buildCapitalFallbackReadModel(canEdit: boolean): AnnualTabResponse {
  const rows: AnnualDocumentRowDto[] = CAPITAL_DECLARATION_SYSTEM_DOCUMENT_TEMPLATES.map((t) => ({
...
async function ensureAnnualBucket(): Promise<void> {
  if (annualBucketEnsured) return;
...
async function ensureProfile(orgId: string, clientId: string, scope: AnnualTabScope): Promise<{ id: string; read_model_version: number }> {
  ...
  if (scoped) existingQuery = existingQuery.eq('tab_scope', scope);
  const { data: existing } = await existingQuery.maybeSingle();
  if (existing) {
    return { id: String((existing as { id: string }).id), read_model_version: Number((existing as { read_model_version: number }).read_model_version ?? 1) };
  }
  const { data: ins, error } = await supabaseAdmin
    ...
    .select('id, read_model_version')
    .single();
  if (error || !ins) throw new AppError(500, error?.message ?? 'annual profile insert failed', 'SUPABASE_ERROR');
  return { id: String((ins as { id: string }).id), read_model_version: Number((ins as { read_model_version: number }).read_model_version ?? 1) };
}

async function ensureSystemDocumentRows(orgId: string, clientId: string, scope: AnnualTabScope): Promise<void> {
  const scoped = await supportsAnnualScopeColumns();
  ...
  if (scoped) existingQ = existingQ.eq('tab_scope', scope);
  const { data: existingRows, error: readErr } = await existingQ;
  if (readErr) throw new AppError(500, readErr.message ?? 'annual system rows read failed', 'SUPABASE_ERROR');
  const existingKeys = new Set(
    ((existingRows ?? []) as Array<{ system_key: string | null }>)
  ...
  if (insertErr) throw new AppError(500, insertErr.message ?? 'annual system rows insert failed', 'SUPABASE_ERROR');
}

async function assertExpectedVersion(orgId: string, clientId: string, expected: number, scope: AnnualTabScope): Promise<void> {
  const scoped = await supportsAnnualScopeColumns();
  let q = supabaseAdmin
    .from('client_annual_report_profiles')
    ...
    .eq('organization_id', orgId)
    .eq('client_id', clientId);
  if (scoped) q = q.eq('tab_scope', scope);
...
async function bumpReadModelVersion(orgId: string, clientId: string, expected: number, userId: string, scope: AnnualTabScope): Promise<void> {
  const scoped = await supportsAnnualScopeColumns();
  let q = supabaseAdmin
    .from('client_annual_report_profiles')
    .update({
...
async function auditAnnual(ctx: RequestContext, orgId: string, clientId: string, command: string, payload: Record<string, unknown>): Promise<void> {
  await writeAudit({
...
export async function getAnnualTabReadModel(
  ctx: RequestContext,
  clientId: string,
  scopeInput?: AnnualTabScope
): Promise<AnnualTabResponse | null> {
  const orgId = assertOrg(ctx);
  const scope = parseAnnualTabScope(scopeInput);
  const scoped = await supportsAnnualScopeColumns();
  if (!canViewAnnualTab(ctx)) return null;
  ...
  let profileQ = supabaseAdmin
    ...
    .eq('organization_id', orgId)
    .eq('client_id', clientId);
  if (scoped) profileQ = profileQ.eq('tab_scope', scope);
  ...
  let docQ = supabaseAdmin
    ...
    .eq('organization_id', orgId)
    .eq('client_id', clientId);
  if (scoped) docQ = docQ.eq('tab_scope', scope);
  ...
  let subQ = supabaseAdmin
    ...
    .eq('organization_id', orgId)
    .eq('client_id', clientId);
  if (scoped) subQ = subQ.eq('tab_scope', scope);
  const { data: subRows } = await subQ.order('sort_order', { ascending: true }).order('tax_year', { ascending: false });

  const fileIds = [
    ...new Set(
      [...(docRows ?? []), ...(subRows ?? [])].map((r) => (r as { file_asset_id?: string | null }).file_asset_id).filter(Boolean)
    ),
  ] as string[];
  ...
  if (fileIds.length > 0) {
    ...
    for (const a of (assets ?? []) as Array<{ id: string; file_name: string | null }>) {
      fileNameById.set(a.id, a.file_name ?? '');
    }
  }

  const rows: AnnualDocumentRowDto[] = (docRows ?? []).map((raw) => {
    const r = raw as {
      id: string;
      source_type: string;
      system_key: string | null;
  ...
  const scopedRows: AnnualDocumentRowDto[] = scoped
    ? rows
    : rows.filter((r) => (r.source_type === 'custom' ? true : Boolean(r.system_key && allowedSystemKeys.has(r.system_key))));

  const scopedSubRows = ((subRows ?? []) as Array<Record<string, unknown>>).filter((raw) => {
    if (scoped) return true;
    const note = (raw.note as string | null | undefined) ?? null;
    if (scope === 'capital_declaration') return hasLegacyScopeMarker(note, 'capital_declaration');
    return !hasLegacyScopeMarker(note, 'capital_declaration');
  });
  const submissions: AnnualSubmissionRowDto[] = scopedSubRows.map((raw) => {
    const s = raw as { id: string; tax_year: number; submitted_on: string; status: string; note: string | null; file_asset_id: string | null };
  ...
  const control_block: AnnualControlBlockDto = {
    days_left_caption_he: 'ימים נותרו להגשה',
  ...
  const hasPreviousDeclarationFile =
    scope === 'capital_declaration' && submissions.some((s) => s.file.state === 'attached' && Boolean(s.file.file_asset_id));
...
async function fetchDocumentRow(orgId: string, clientId: string, rowId: string, scope: AnnualTabScope): Promise<Record<string, unknown> | null> {
  const scoped = await supportsAnnualScopeColumns();
  let q = supabaseAdmin
    .from('client_annual_document_rows')
...
export async function assertAnnualReportFileOpenAllowed(
  orgId: string,
  clientId: string,
  fileAssetId: string,
  scope: AnnualTabScope = 'annual_report'
): Promise<void> {
  const scoped = await supportsAnnualScopeColumns();
  let linkedQ = supabaseAdmin
    ...
    .limit(1);
  if (scoped) linkedQ = linkedQ.eq('tab_scope', scope);
  ...
  let subQ = supabaseAdmin
    ...
    .limit(1);
  if (scoped) subQ = subQ.eq('tab_scope', scope);
  if (!scoped && scope === 'capital_declaration') subQ = subQ.like('note', `${LEGACY_SCOPE_MARKER.capital_declaration}%`);
  ...
  if (!String(fa.storage_key).startsWith(expectedPrefix)) throw forbidden('File not linked to this client annual report');
}

async function fetchSubmissionRow(orgId: string, clientId: string, submissionId: string, scope: AnnualTabScope): Promise<Record<string, unknown> | null> {
  const scoped = await supportsAnnualScopeColumns();
  let q = supabaseAdmin
    .from('client_annual_submission_rows')
    .select('id, note')
  ...
  if (!row) return null;
  if (!scoped) {
    const note = (row.note as string | null | undefined) ?? null;
    if (scope === 'capital_declaration' && !hasLegacyScopeMarker(note, 'capital_declaration')) return null;
    if (scope === 'annual_report' && hasLegacyScopeMarker(note, 'capital_declaration')) return null;
  }
  return row;
}

export async function uploadAnnualReportDocument(
  ctx: RequestContext,
  orgId: string,
  clientId: string,
  body: { file_base64?: string; file_name?: string; mime_type?: string | null; tab_scope?: AnnualTabScope }
  ...
  await writeAudit({
    ...
    payload: { client_id: clientId, file_asset_id: (asset as { id: string }).id, context: 'annual_report_upload' },
  });
...
export async function getAnnualReportDocumentFileOpenUrl(
  ctx: RequestContext,
  orgId: string,
  clientId: string,
  fileAssetId: string,
...
export async function executeAnnualTabCommand(ctx: RequestContext, clientId: string, body: AnnualTabCommandBody): Promise<void> {
  const orgId = assertOrg(ctx);
  const scope = parseAnnualTabScope(body?.tab_scope);
  const scoped = await supportsAnnualScopeColumns();
  if (!canEditAnnualTab(ctx)) throw forbidden('Insufficient permission');
  await ensureClientInOrg(orgId, clientId);
  ...
  const cmd = body.type as AnnualAnnualCommandType;

  switch (cmd) {
    case 'add_annual_document_row': {
      const name = String(payload.document_name_he ?? '').trim();
      if (!name) throw badRequest('שם מסמך נדרש');
      ...
      if (error) throw new AppError(500, error.message ?? 'insert failed', 'SUPABASE_ERROR');
    ...
    case 'update_annual_document_row': {
      ...
      if (!rowId) throw badRequest('row_id נדרש');
      const row = await fetchDocumentRow(orgId, clientId, rowId, scope);
      if (!row) throw badRequest('שורה לא נמצאה');
      ...
      if (payload.document_name_he !== undefined) {
        ...
        if (!n) throw badRequest('שם מסמך ריק');
        patch.document_name_he = n;
      }
      if (payload.row_note !== undefined) patch.row_note = payload.row_note == null || payload.row_note === '' ? null : String(payload.row_note);
      ...
      if (Object.keys(patch).length === 0) throw badRequest('אין שדות לעדכון');
      ...
      if (error) throw new AppError(500, error.message ?? 'update failed', 'SUPABASE_ERROR');
    ...
    case 'remove_annual_document_row': {
      ...
      const row = await fetchDocumentRow(orgId, clientId, rowId, scope);
      if (!row) throw badRequest('שורה לא נמצאה');
      if (String(row.source_type) !== 'custom') throw badRequest('לא ניתן למחוק שורת מערכת');
      const { error } = await supabaseAdmin
        .from('client_annual_document_rows')
        .delete()
        .eq('id', rowId)
        ...
        .eq('client_id', clientId)
        .match(scoped ? { tab_scope: scope } : {});
      if (error) throw new AppError(500, error.message ?? 'delete failed', 'SUPABASE_ERROR');
    ...
    case 'attach_annual_document_file': {
      ...
      if (!rowId || !fileAssetId) throw badRequest('row_id וקובץ נדרשים');
      await assertAnnualReportFileOpenAllowed(orgId, clientId, fileAssetId, scope);
      const { error } = await supabaseAdmin
        .from('client_annual_document_rows')
        ...
        .eq('client_id', clientId)
        .match(scoped ? { tab_scope: scope } : {});
      if (error) throw new AppError(500, error.message ?? 'attach failed', 'SUPABASE_ERROR');
    ...
    case 'remove_annual_document_file': {
      ...
      const { error } = await supabaseAdmin
        ...
        .eq('client_id', clientId)
        .match(scoped ? { tab_scope: scope } : {});
      if (error) throw new AppError(500, error.message ?? 'remove file failed', 'SUPABASE_ERROR');
      await auditAnnual(ctx, orgId, clientId, cmd, { row_id: rowId });
      break;
    }
    case 'add_annual_submission_date': {
      const taxYear = Number(payload.tax_year);
      if (!Number.isFinite(taxYear) || taxYear < 1900 || taxYear > 2200) throw badRequest('שנת מס לא תקינה');
      const submittedOn = String(payload.submitted_at ?? payload.submitted_on ?? '').trim().slice(0, 10);
      if (!submittedOn) throw badRequest('תאריך הגשה נדרש');
      ...
      if (error) throw new AppError(500, error.message ?? 'insert submission failed', 'SUPABASE_ERROR');
    ...
    case 'update_annual_submission_date': {
      ...
      if (!submissionId) throw badRequest('submission_id נדרש');
      const patch: Record<string, unknown> = {};
      if (payload.tax_year !== undefined) {
        const ty = Number(payload.tax_year);
        if (!Number.isFinite(ty)) throw badRequest('שנת מס לא תקינה');
        patch.tax_year = ty;
      }
      if (payload.submitted_at !== undefined || payload.submitted_on !== undefined) {
        ...
        if (!d) throw badRequest('תאריך ריק');
        patch.submitted_on = d;
      }
      if (payload.status !== undefined) patch.status = String(payload.status ?? 'submitted').trim() || 'submitted';
      ...
      const { error } = await supabaseAdmin
        ...
        .eq('client_id', clientId)
        .match(scoped ? { tab_scope: scope } : {});
      if (error) throw new AppError(500, error.message ?? 'update submission failed', 'SUPABASE_ERROR');
    ...
    case 'remove_annual_submission_date': {
      ...
      const { error } = await supabaseAdmin
        ...
        .eq('client_id', clientId)
        .match(scoped ? { tab_scope: scope } : {});
      if (error) throw new AppError(500, error.message ?? 'delete submission failed', 'SUPABASE_ERROR');
    ...
    case 'attach_annual_submission_file': {
      const submissionId = String(payload.submission_id ?? '');
      const fileAssetId = String(payload.file_asset_id ?? '');
      if (!submissionId || !fileAssetId) throw badRequest('submission_id וקובץ נדרשים');
      const row = await fetchSubmissionRow(orgId, clientId, submissionId, scope);
      if (!row) throw badRequest('רשומת הגשה לא נמצאה');
      await assertAnnualReportFileOpenAllowed(orgId, clientId, fileAssetId, scope);
      const { error } = await supabaseAdmin
        .from('client_annual_submission_rows')
        .update({ file_asset_id: fileAssetId })
        ...
        .eq('client_id', clientId)
        .match(scoped ? { tab_scope: scope } : {});
      if (error) throw new AppError(500, error.message ?? 'attach submission file failed', 'SUPABASE_ERROR');
    ...
    case 'update_annual_notes': {
      ...
      const { error } = await supabaseAdmin
        ...
        .eq('client_id', clientId)
        .match(scoped ? { tab_scope: scope } : {});
      if (error) throw new AppError(500, error.message ?? 'notes update failed', 'SUPABASE_ERROR');
      await auditAnnual(ctx, orgId, clientId, cmd, { notes_len: notes?.length ?? 0 });
      break;
    }
    case 'copy_previous_capital_declaration': {
      if (scope !== 'capital_declaration') throw badRequest('פקודה זו זמינה רק בהצהרת הון');
      ...
      if (scoped) prevQ = prevQ.eq('tab_scope', scope);
      if (!scoped) prevQ = prevQ.like('note', `${LEGACY_SCOPE_MARKER.capital_declaration}%`);
      const { data: prevRows, error: prevErr } = await prevQ.order('tax_year', { ascending: false }).order('submitted_on', { ascending: false }).limit(1);
      if (prevErr) throw new AppError(500, prevErr.message ?? 'previous declaration read failed', 'SUPABASE_ERROR');
      const prev = (prevRows?.[0] ?? null) as { id: string; file_asset_id: string | null; tax_year: number; submitted_on: string } | null;
      if (!prev?.file_asset_id) throw badRequest('לא נמצאה הצהרת הון קודמת עם מסמך');
      ...
      const { data: targetRows, error: targetErr } = await supabaseAdmin
        ...
        .match(scoped ? { tab_scope: scope } : {})
        .limit(1);
      if (targetErr) throw new AppError(500, targetErr.message ?? 'target document row read failed', 'SUPABASE_ERROR');
      const targetRowId = String((targetRows?.[0] as { id?: string } | undefined)?.id ?? '');
      if (!targetRowId) throw badRequest('שורת יעד לא נמצאה');
      ...
      const { error: updateErr } = await supabaseAdmin
        ...
        .eq('client_id', clientId)
        .match(scoped ? { tab_scope: scope } : {});
      if (updateErr) throw new AppError(500, updateErr.message ?? 'copy previous declaration failed', 'SUPABASE_ERROR');
