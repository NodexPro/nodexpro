/**
 * Accounting workspace commands — reuse domain saves from client-accounting-tab.service + vehicle fleet, then full case.
 */
...
import {
  saveClientBusinessProfileSection,
  type ClientBusinessProfileSaveInput,
} from './client-business-profile.service.js';
...
export type AccountingCommandType =
  ...
  | 'save_accounting_business_profile'
  | 'add_expense_management_custom_field'
  | 'remove_expense_management_custom_field'
  | 'create_vehicle_fleet_item'
  | 'update_vehicle_fleet_item'
  | 'delete_vehicle_fleet_item';

const BLOCK_SAVE_COMMANDS: Partial<Record<AccountingCommandType, 'income' | 'expense_management' | 'expenses' | 'vehicles'>> =
  {
...
function parsePayload(body: { payload?: Record<string, unknown> }): Record<string, unknown> {
  const p = body.payload;
  return p && typeof p === 'object' && !Array.isArray(p) ? { ...p } : {};
}

function hasBusinessProfileEdit(ctx: RequestContext): boolean {
  const p = ctx.membership?.permissions ?? [];
  return p.includes('business_profile.edit') || p.includes('client_operations.edit');
}

function parseBusinessProfileCommandPayload(p: Record<string, unknown>): ClientBusinessProfileSaveInput {
  const ev = p.expected_version;
  const expected_version = typeof ev === 'number' && Number.isFinite(ev) ? ev : Number(ev);
  if (!Number.isFinite(expected_version)) throw badRequest('expected_version required');
  ...
  const business_operation_mode =
    modeRaw == null || modeRaw === '' ? null : (modeRaw as ClientBusinessProfileSaveInput['business_operation_mode']);

  const custRaw = p.primary_customer_type;
  const primary_customer_type =
    custRaw == null || custRaw === '' ? null : (custRaw as ClientBusinessProfileSaveInput['primary_customer_type']);

  let has_business_vehicles: boolean | undefined;
  ...
  return {
    ...
    business_close_date:
      p.business_close_date == null || p.business_close_date === '' ? null : String(p.business_close_date),
    ...(has_business_vehicles !== undefined ? { has_business_vehicles } : {}),
  };
}

export async function executeAccountingCommand(
  ctx: RequestContext,
  clientId: string,
  body: {
    type?: string;
  ...
  const t = String(body.type ?? '').trim() as AccountingCommandType;
  ...
  if (t === 'save_accounting_business_profile') {
    if (!hasBusinessProfileEdit(ctx)) throw forbidden('Insufficient permission');
  ...
  if (t === 'remove_expense_management_custom_field') {
    ...
    if (!fieldId) throw badRequest('field_id required');
    await removeExpenseManagementCustomFieldApi(ctx, clientId, fieldId);
    return getClientOperationsCase(ctx, clientId, opts);
  }

  if (t === 'create_vehicle_fleet_item') {
    if (!canEditBlock(ctx, 'vehicles')) throw forbidden('Insufficient permission');
  ...
  if (t === 'update_vehicle_fleet_item') {
    ...
    if (!vid) throw badRequest('vehicle_id required');
    const { vehicle_id: _omit, ...rest } = payload;
    void _omit;
    await updateVehicleFleetItem(ctx, clientId, vid, rest as Record<string, unknown>);
    return getClientOperationsCase(ctx, clientId, opts);
  }
  ...
  if (t === 'delete_vehicle_fleet_item') {
    ...
    const { vehicle_id: _omit, ...rest } = payload;
    void _omit;
    await deleteVehicleFleetItem(ctx, clientId, vid, rest as Record<string, unknown>);
    return getClientOperationsCase(ctx, clientId, opts);
  }
