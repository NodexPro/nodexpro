...
export type OsekPaturDeclarationUi = {
  label_he: string;
  /** לתצוגה בשדה, לדוגמה 31.03.26 */
...
export function getOsekPaturDeadlineAndTaxYear(now: Date): { deadlineYear: number; taxYear: number } {
 ...
 */
const TOOLTIP_POSTPONEMENT_2026_HE =
  'מועד הגשת הצהרה שנתית לשנת 2025 על ידי עוסק פטור יידחה עד ליום 30.4.2026 ומועד הגשת דיווח שנתי מסכם על ידי בעל עסק זעיר לשנת 2025, אשר כבר נדחה ליום 31.5.2026 ימשיך לחול ביום זה.';

export function computeOsekPaturDeclarationUi(now: Date): OsekPaturDeclarationUi {
  ...
  let tooltip_title_he: string;
  let tooltip_body_he: string;
