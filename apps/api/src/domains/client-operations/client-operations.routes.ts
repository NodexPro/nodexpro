...
import {
  ...
  type ClientHistoryExportBody,
  type ClientHistoryTabCommandBody,
...
import {
  ...
  getDueReminders,
} from './client-operations-notes.service.js';
import {
  getClientTaxSettings,
  revealClientPaymentSecret,
  type PaymentSecretKind,
...
import {
  requestPaymentCardAccessCode,
  verifyPaymentCardAccessCode,
  type PaymentContext,
} from './payment-card-access.service.js';
import {
  replaceClientAccountingVehicles,
  updateClientAccountingGeneral,
} from './client-accounting.service.js';
import { saveClientBusinessProfileSection } from './client-business-profile.service.js';
import {
  addExpenseManagementCustomFieldApi,
  evaluateAccountingModalFieldVisibility,
  getAccountingBlockModal,
  getAccountingSettingsTab,
  normalizeAccountingBlockDraft,
  removeExpenseManagementCustomFieldApi,
  saveAccountingBlock,
} from './client-accounting-tab.service.js';
import {
  createVehicleFleetItem,
  deleteVehicleFleetItem,
  evaluateVehicleItemModalVisibility,
  getVehicleFleetFileOpenUrl,
  getVehicleFleetItemModal,
  updateVehicleFleetItem,
  uploadVehicleFleetDocument,
} from './client-vehicle-fleet.service.js';
import { executeFeesTabCommand, parseFeesPriceChartView, type FeesTabCommandBody } from './client-fees-tab.service.js';
import { executePayrollTabCommand, type PayrollTabCommandBody } from './client-payroll-tab.service.js';
import {
  executeAnnualTabCommand,
  getAnnualReportDocumentFileOpenUrl,
  uploadAnnualReportDocument,
  type AnnualTabScope,
  type AnnualTabCommandBody,
} from './client-annual-report-tab.service.js';
import {
  executeClientDocumentsTabCommand,
  getClientDocumentFileOpenUrl,
  uploadClientWorkspaceDocument,
  type ClientDocumentsTabCommandBody,
} from './client-documents-tab.service.js';
import {
  executeClientObligationsCommand,
  executeClientTasksCommand,
  type ClientObligationsCommandBody,
  type ClientTasksCommandBody,
...
const withAccountingBlockEdit = [
  requirePermission(
    'accounting_settings_expenses.edit',
    'accounting_settings_income.edit',
    'accounting_settings_expense_management.edit',
    'accounting_settings_documents.edit',
    'accounting_settings_vehicles.edit',
...
router.get('/clients/:clientId', ...withView, async (req, res, next) => {
  try {
    ...
    const feesPv = parseFeesPriceChartView((req.query as { fees_price_chart_view?: unknown }).fees_price_chart_view);
...
router.post('/clients/:clientId/history/commands', ...withView, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const body = (req.body ?? {}) as ClientHistoryTabCommandBody;
    const feesPv = parseFeesPriceChartView((req.body as { fees_price_chart_view?: unknown })?.fees_price_chart_view);
...
router.post('/clients/:clientId/history/export', ...withView, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const body = (req.body ?? {}) as ClientHistoryExportBody;
...
router.post('/clients/:clientId/fees/commands', ...withFeesEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const body = (req.body ?? {}) as FeesTabCommandBody;
...
router.post('/clients/:clientId/payroll/commands', ...withPayrollEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const body = (req.body ?? {}) as PayrollTabCommandBody;
...
router.post('/clients/:clientId/annual/commands', ...withAnnualEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const body = (req.body ?? {}) as AnnualTabCommandBody;
    const feesPv = parseFeesPriceChartView((req.body as { fees_price_chart_view?: unknown })?.fees_price_chart_view);
...
router.post('/clients/:clientId/annual/upload', ...withAnnualEdit, async (req, res, next) => {
  try {
    ...
    const out = await uploadAnnualReportDocument(ctx, orgId, clientId, (req.body ?? {}) as Record<string, unknown>);
    return res.json(out);
  } catch (e) {
    next(e);
  }
...
router.get('/clients/:clientId/annual/files/:fileAssetId/open', ...withAnnualView, async (req, res, next) => {
  try {
    ...
    const orgId = ctx.organizationId!;
    const scope = parseAnnualTabScope((req.query as { tab_scope?: unknown } | undefined)?.tab_scope);
...
router.post('/clients/:clientId/documents/commands', ...withClientDocumentsEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const body = (req.body ?? {}) as ClientDocumentsTabCommandBody;
    const feesPv = parseFeesPriceChartView((req.body as { fees_price_chart_view?: unknown })?.fees_price_chart_view);
...
router.post('/clients/:clientId/obligations/commands', ...withEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const body = (req.body ?? {}) as ClientObligationsCommandBody;
...
router.post('/clients/:clientId/tasks/commands', ...withEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const body = (req.body ?? {}) as ClientTasksCommandBody;
...
router.post('/clients/:clientId/documents/upload', ...withClientDocumentsEdit, async (req, res, next) => {
  try {
    ...
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const orgId = ctx.organizationId!;
    const out = await uploadClientWorkspaceDocument(ctx, orgId, clientId, (req.body ?? {}) as Record<string, unknown>);
    return res.json(out);
  } catch (e) {
    next(e);
  }
...
router.post('/clients/:clientId/tax-settings/commands/update_tax_settings', ...withEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const out = await executeClientOperationsTaxSettingsCommand(
      ctx,
      clientId,
      'update_tax_settings',
      req.body ?? {},
      { fees_price_chart_view: (req.body as { fees_price_chart_view?: unknown })?.fees_price_chart_view }
    );
    return res.json(out);
  } catch (e) {
    next(e);
...
router.post('/clients/:clientId/accounting/vehicles/commands/replace_accounting_vehicles', ...withEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const body = (req.body ?? {}) as { vehicles?: unknown };
...
router.post('/clients/:clientId/accounting/business-profile/commands/save_business_profile', ...withBusinessProfileEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const out = await saveClientBusinessProfileSection(ctx, clientId, req.body ?? ({} as any));
    return res.json(out);
  } catch (e) {
    next(e);
  }
...
router.get('/clients/:clientId/accounting-settings/vehicle-fleet/item-modal', ...withAccountingTabView, async (req, res, next) => {
  try {
    ...
    const vehicleId =
      typeof q === 'string' && /^[0-9a-f-]{36}$/i.test(q) ? q : null;
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const orgId = ctx.organizationId!;
...
router.post('/clients/:clientId/accounting-settings/vehicle-fleet/item-modal-visibility', ...withAccountingTabView, async (req, res, next) => {
  try {
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const out = await evaluateVehicleItemModalVisibility((req.body ?? {}) as Record<string, unknown>);
    return res.json(out);
  } catch (e) {
    next(e);
  }
...
router.post('/clients/:clientId/accounting-settings/vehicle-fleet/items', ...withAccountingBlockEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    await createVehicleFleetItem(ctx, clientId, (req.body ?? {}) as Record<string, unknown>);
...
router.post('/clients/:clientId/accounting-settings/vehicle-fleet/items/:vehicleId/commands/update_vehicle_fleet_item', ...withAccountingBlockEdit, async (req, res, next) => {
  try {
    ...
    await updateVehicleFleetItem(ctx, clientId, vehicleId, (req.body ?? {}) as Record<string, unknown>);
    const tab = await getAccountingSettingsTab(ctx, clientId);
    return res.json(tab);
  } catch (e) {
    next(e);
...
router.delete('/clients/:clientId/accounting-settings/vehicle-fleet/items/:vehicleId', ...withAccountingBlockEdit, async (req, res, next) => {
  try {
    ...
    await deleteVehicleFleetItem(ctx, clientId, vehicleId, (req.body ?? {}) as Record<string, unknown>);
    const tab = await getAccountingSettingsTab(ctx, clientId);
    return res.json(tab);
  } catch (e) {
    next(e);
...
router.get('/clients/:clientId/accounting-settings/blocks/:blockKey/modal', ...withAccountingTabView, async (req, res, next) => {
  try {
    ...
    const out = await getAccountingBlockModal(ctx, clientId, blockKey as any);
    return res.json(out);
  } catch (e) {
    next(e);
  }
...
router.post('/clients/:clientId/accounting-settings/blocks/:blockKey/modal-visibility', ...withAccountingTabView, async (req, res, next) => {
  try {
    ...
    const out = await evaluateAccountingModalFieldVisibility(
      ctx,
      clientId,
      blockKey as any,
...
router.post('/clients/:clientId/accounting-settings/blocks/:blockKey/normalize-draft', ...withAccountingTabView, async (req, res, next) => {
  try {
    ...
    const out = await normalizeAccountingBlockDraft(ctx, clientId, blockKey as any, (req.body ?? {}) as Record<string, unknown>);
    return res.json(out);
  } catch (e) {
    next(e);
  }
...
router.post('/clients/:clientId/accounting-settings/blocks/:blockKey/commands/save_block', ...withAccountingBlockEdit, async (req, res, next) => {
  try {
    ...
    const out = await saveAccountingBlock(ctx, clientId, blockKey as any, (req.body ?? {}) as Record<string, unknown>);
    return res.json(out);
  } catch (e) {
    next(e);
  }
...
router.post(
  '/clients/:clientId/accounting-settings/blocks/expense_management/custom-fields',
  ...withAccountingBlockEdit,
  async (req, res, next) => {
    try {
      const ctx = req.context as RequestContext;
      const clientId = String(req.params.clientId ?? '');
      if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
      const out = await addExpenseManagementCustomFieldApi(ctx, clientId, (req.body ?? {}) as Record<string, unknown>);
      return res.json(out);
    } catch (e) {
      next(e);
    }
...
router.delete(
  '/clients/:clientId/accounting-settings/blocks/expense_management/custom-fields/:fieldId',
  ...withAccountingBlockEdit,
  async (req, res, next) => {
    try {
      const ctx = req.context as RequestContext;
...
router.post('/clients/:clientId/accounting-settings/vehicle-fleet/upload', ...withAccountingBlockEdit, async (req, res, next) => {
  try {
    ...
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const orgId = ctx.organizationId!;
    const uploaded = await uploadVehicleFleetDocument(ctx, orgId, clientId, (req.body ?? {}) as any);
    const feesPv = parseFeesPriceChartView((req.query as { fees_price_chart_view?: unknown }).fees_price_chart_view);
...
router.post('/clients/:clientId/tax-settings/reveal-payment-secret', ...withEdit, async (req, res, next) => {
  try {
    ...
    const secretKind = (req.body as { secret_kind?: unknown })?.secret_kind;
    if (type !== 'vat' && type !== 'income_tax') {
      return res.status(400).json({ code: 'BAD_REQUEST', message: 'type must be vat or income_tax' });
    }
    const allowed = new Set<PaymentSecretKind>(['card_number', 'expiry']);
    if (typeof secretKind !== 'string' || !allowed.has(secretKind as PaymentSecretKind)) {
      return res.status(400).json({
        code: 'BAD_REQUEST',
        message: 'secret_kind must be card_number or expiry (CVV is never stored)',
      });
    }
    const payload = await revealClientPaymentSecret(ctx, clientId, type, secretKind as PaymentSecretKind);
...
router.post('/clients/:clientId/tax-settings/payment-card/request-code', ...withEdit, async (req, res, next) => {
  try {
    ...
    const clientId = String(req.params.clientId ?? '');
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const t = (req.body as { type?: unknown })?.type;
    if (t !== 'vat' && t !== 'income_tax') {
      return res.status(400).json({ code: 'BAD_REQUEST', message: 'type must be vat or income_tax' });
    }
    const out = await requestPaymentCardAccessCode(ctx, clientId, t as PaymentContext);
    return res.json(out);
  } catch (e) {
    next(e);
  }
...
router.post('/clients/:clientId/tax-settings/payment-card/verify-code', ...withEdit, async (req, res, next) => {
  try {
    ...
    if (!clientId) return res.status(400).json({ code: 'BAD_REQUEST', message: 'clientId required' });
    const t = (req.body as { type?: unknown })?.type;
    const challengeId = (req.body as { challenge_id?: unknown })?.challenge_id;
    ...
    const out = await verifyPaymentCardAccessCode(ctx, clientId, challengeId.trim(), code.trim(), t as PaymentContext);
    return res.json(out);
  } catch (e) {
    next(e);
  }
