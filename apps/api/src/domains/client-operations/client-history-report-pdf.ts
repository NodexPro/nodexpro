...
export type HistoryReportPdfRow = {
...
function wrapLines(text: string, size: number, maxWidth: number, he: PDFFont, lat: PDFFont): string[] {
  ...
  for (const w of words) {
    ...
    if (widthOfMixedText(trial, size, he, lat) <= maxWidth) cur = trial;
    else {
...
export async function buildHistoryReportPdfBuffer(params: {
  ...
  const drawRtlMixedLine = (line: string, size: number, color: RGB) => {
  ...
  const drawRtlParagraph = (text: string, size: number, color: RGB, gapAfter = 8) => {
