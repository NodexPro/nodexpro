...
export type ObligationDocumentsMissingSignal = {
  ctx: RequestContext;
  orgId: string;
  clientId: string;
  obligationType: string;
 ...
 */
export async function emitObligationDocumentsMissingIfMapped(
  signal: ObligationDocumentsMissingSignal,
): Promise<void> {
  const eventType = mapObligationTypeToWorkEngineEvent(signal.obligationType);
  if (!eventType) return;
  if (!signal.periodKey) return;
  ...
  try {
    ...
    console.warn(
      '[client_obligations → work_engine] intake failed for obligation event',
      {
        org_id: signal.orgId,
        client_id: signal.clientId,
        obligation_type: signal.obligationType,
