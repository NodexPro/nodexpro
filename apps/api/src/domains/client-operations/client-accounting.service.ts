...
export type ClientAccountingVehiclePublic = {
  ...
  /** Effective value: profession rule default when active, else stored DB value */
  recognized_vat_percent: number | null;
  recognized_expense_percent: number | null;
};

export type ClientAccountingSettingsPublic = {
...
export type ClientAccountingBundle = {
  settings: ClientAccountingSettingsPublic;
  vehicles: ClientAccountingVehiclePublic[];
  /** Profession names from backend rules — UI may suggest; save still sends free-text occupation_field */
  profession_rule_names: string[];
};

export type AccountingVehicleInput = {
  vehicle_kind: VehicleKind;
  license_plate?: string | null;
  manufacture_year?: number | null;
  engine_type: EngineType;
...
function parseDateOnlyOptional(v: unknown): string | null {
  if (v === undefined || v === null) return null;
  if (typeof v !== 'string') throw badRequest('תאריך לא חוקי');
  const s = v.trim();
  if (!s) return null;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(s)) throw badRequest('תאריך לא חוקי');
  const d = new Date(`${s}T00:00:00.000Z`);
  if (Number.isNaN(d.getTime())) throw badRequest('תאריך לא חוקי');
  return s;
}

function normalizeOptString(v: unknown): string | null {
  if (v === undefined || v === null) return null;
...
async function loadProfessionRules(): Promise<ProfessionRuleRow[]> {
  ...
  if (error) throw new AppError(500, error.message ?? 'accounting_vehicle_profession_rules read failed', 'SUPABASE_ERROR');
  return (data ?? []) as ProfessionRuleRow[];
}

function matchProfessionRule(occupationField: string | null, rules: ProfessionRuleRow[]): ProfessionRuleRow | null {
  const o = (occupationField ?? '').trim();
  if (!o) return null;
  for (const r of rules) {
    if (String(r.profession_name).trim() === o) return r;
  }
  return null;
}

function effectiveVatPercent(stored: number | null, rule: ProfessionRuleRow | null): number | null {
...
function vatPercentToStore(
  clientSent: number | null | undefined,
  rule: ProfessionRuleRow | null
): number | null {
  if (rule?.applies_automatic_default) {
    return Number(rule.vehicle_vat_percent_default);
  }
  if (clientSent === undefined || clientSent === null) return null;
  const n = Number(clientSent);
  if (Number.isNaN(n)) return null;
  return n;
}

async function getOrCreateSettingsRow(orgId: string, clientId: string): Promise<{
  occupation_field: string | null;
  business_opened_on: string | null;
  business_closed_on: string | null;
  has_vehicles: boolean;
  ...
  if (readErr) throw new AppError(500, readErr.message ?? 'client_accounting_settings read failed', 'SUPABASE_ERROR');

  if (existing) {
    const r = existing as Record<string, unknown>;
    return {
      occupation_field: (r.occupation_field as string | null) ?? null,
      business_opened_on: (r.business_opened_on as string | null) ?? null,
      business_closed_on: (r.business_closed_on as string | null) ?? null,
  ...
  if (insErr) throw new AppError(500, insErr.message ?? 'client_accounting_settings insert failed', 'SUPABASE_ERROR');
...
function mapVehicleRow(
  ...
  rule: ProfessionRuleRow | null
): ClientAccountingVehiclePublic {
  const storedVat =
    v.recognized_vat_percent != null && v.recognized_vat_percent !== ''
      ? Number(v.recognized_vat_percent)
      : null;
  const storedExp =
    v.recognized_expense_percent != null && v.recognized_expense_percent !== ''
  ...
  return {
    ...
    vehicle_kind: v.vehicle_kind as VehicleKind,
    license_plate: (v.license_plate as string | null) ?? null,
    manufacture_year:
      v.manufacture_year != null && v.manufacture_year !== '' ? Number(v.manufacture_year) : null,
    engine_type: v.engine_type as EngineType,
    compulsory_insurance_from: (v.compulsory_insurance_from as string | null) ?? null,
    compulsory_insurance_to: (v.compulsory_insurance_to as string | null) ?? null,
    comprehensive_insurance_from: (v.comprehensive_insurance_from as string | null) ?? null,
    comprehensive_insurance_to: (v.comprehensive_insurance_to as string | null) ?? null,
    recognized_vat_percent: effectiveVatPercent(
      storedVat != null && !Number.isNaN(storedVat) ? storedVat : null,
      rule
    ),
    recognized_expense_percent:
      storedExp != null && !Number.isNaN(storedExp) ? storedExp : null,
  };
}

export async function getClientAccountingBundle(
  ctx: RequestContext,
  clientId: string
): Promise<ClientAccountingBundle> {
  ...
  if (vErr) throw new AppError(500, vErr.message ?? 'client_accounting_vehicles read failed', 'SUPABASE_ERROR');

  const vehicles = (vehiclesRaw ?? []).map((v) => mapVehicleRow(v as Record<string, unknown>, matchedRule));
...
export type UpdateAccountingGeneralBody = Partial<{
  occupation_field: string | null;
  business_opened_on: string | null;
  business_closed_on: string | null;
  has_vehicles: boolean;
...
export async function updateClientAccountingGeneral(
  ctx: RequestContext,
  clientId: string,
  body: UpdateAccountingGeneralBody
): Promise<ClientAccountingBundle> {
  const orgId = assertOrg(ctx);
  ...
  if (body.has_vehicles !== undefined) {
    ...
    if (!hv) {
      const { error: delErr } = await supabaseAdmin
        ...
        .eq('organization_id', orgId)
        .eq('client_id', clientId);
      if (delErr) throw new AppError(500, delErr.message ?? 'vehicles delete failed', 'SUPABASE_ERROR');
    }
  }
  ...
  if (error) throw new AppError(500, error.message ?? 'client_accounting_settings update failed', 'SUPABASE_ERROR');
...
const ALLOW_KIND = new Set<string>(['business', 'private']);
const ALLOW_ENGINE = new Set<string>(['diesel', 'gasoline', 'electric']);

export async function replaceClientAccountingVehicles(
  ctx: RequestContext,
  clientId: string,
  body: { vehicles: AccountingVehicleInput[] }
): Promise<ClientAccountingBundle> {
  const orgId = assertOrg(ctx);
  const userId = ctx.user.id;
  ...
  for (let idx = 0; idx < vehicles.length; idx++) {
    ...
    if (!ALLOW_KIND.has(v.vehicle_kind)) throw badRequest('סוג רכב לא חוקי');
    if (!ALLOW_ENGINE.has(v.engine_type)) throw badRequest('סוג הנעה לא חוקי');

    const year =
      v.manufacture_year === undefined || v.manufacture_year === null
        ? null
    ...
    if (exp !== undefined && exp !== null) {
      const n = Number(exp);
      if (Number.isNaN(n) || n < 0 || n > 100) throw badRequest('אחוז הוצאה מוכרת לא חוקי');
    } else {
      exp = null;
    }

    const vatStored = vatPercentToStore(
      v.recognized_vat_percent === undefined ? null : v.recognized_vat_percent === null ? null : Number(v.recognized_vat_percent),
      matchedRule
    );
    if (!matchedRule?.applies_automatic_default && v.recognized_vat_percent != null) {
      const n = Number(v.recognized_vat_percent);
      if (Number.isNaN(n) || n < 0 || n > 100) throw badRequest('אחוז מע״מ מוכר לא חוקי');
    }
  ...
  if (rows.length > 0) {
    ...
    if (insErr) throw new AppError(500, insErr.message ?? 'vehicles insert failed', 'SUPABASE_ERROR');
  }
