...
export type BusinessOperationMode = 'services' | 'trade' | 'online' | 'freelancer' | 'manufacturing' | 'mixed';
export type PrimaryCustomerType = 'private' | 'business' | 'mixed';
...
export type ClientBusinessProfileSectionField = {
  key:
    | 'business_domain'
  ...
  group_key: 'business_scope' | 'address' | 'general_info';
  group_label: string;
  group_order: number;
  row_order: number;
};
...
export type ClientBusinessProfileSectionResponse = {
  ...
  version: number;
  fields: ClientBusinessProfileSectionField[];
};

export type ClientBusinessProfileSaveInput = {
  expected_version: number;
  business_domain: string | null;
  business_activity_description: string | null;
  business_address: string | null;
  private_address: string | null;
  business_operation_mode: BusinessOperationMode | null;
  primary_customer_type: PrimaryCustomerType | null;
  is_seasonal_business: boolean;
  peak_months: number[]; // empty when not seasonal
  business_open_date: string | null; // YYYY-MM-DD
  business_close_date: string | null; // YYYY-MM-DD
  /** נשמר רכבים בלוק נפרד; אופציונלי לתאימות לאחור — אם חסר, הערך ב-DB לא משתנה. */
  has_business_vehicles?: boolean;
};

type FieldError = { key: ClientBusinessProfileSectionField['key']; message_he: string };

export type ClientBusinessProfileSaveResponse = {
  ok: true;
...
const BUSINESS_OPERATION_MODE_OPTIONS: Array<{ value: BusinessOperationMode; label: string }> = [
...
const PRIMARY_CUSTOMER_TYPE_OPTIONS: Array<{ value: PrimaryCustomerType; label: string }> = [
...
const PEAK_MONTHS_OPTIONS: Array<{ value: number; label: string }> = [
  { value: 1, label: 'ינואר' },
...
const BOOL_OPTIONS: Array<{ value: boolean; label: string }> = [
  { value: true, label: 'כן' },
...
function parseDateOnlyOptional(v: unknown): string | null {
  ...
  if (typeof v !== 'string') throw badRequest('Invalid date');
  const s = v.trim();
  if (!s) return null;
...
function normalizeTextOptional(v: unknown, maxLen: number): string | null {
  if (v === undefined || v === null) return null;
  if (typeof v !== 'string') return null;
  const s = v.trim();
  if (!s) return null;
  if (s.length > maxLen) throw badRequest(`Text too long (max ${maxLen})`);
  return s;
}
...
export async function getClientBusinessProfileSection(
  ctx: RequestContext,
  clientId: string
): Promise<ClientBusinessProfileSectionResponse> {
  ...
  const { data: row, error } = await supabaseAdmin
    ...
    .select(
      'business_domain, business_activity_description, business_address, private_address, business_operation_mode, primary_customer_type, is_seasonal_business, peak_months, business_open_date, business_close_date, has_business_vehicles, version'
    )
  ...
  const isSeasonal = Boolean((r.is_seasonal_business as boolean | null) ?? false);
  const peakMonthsRaw = (r.peak_months as number[] | null) ?? [];
  ...
  const section: ClientBusinessProfileSectionResponse = {
    section_key: 'business_profile',
    ...
    fields: [
      {
        ...
        label: 'תחום עיסוק',
        type: 'text',
        value: (r.business_domain as string | null) ?? null,
        options: undefined,
        required: false,
        editable: canEdit,
        visible: true,
      ...
      },
      {
        ...
        type: 'textarea',
        value: (r.business_activity_description as string | null) ?? null,
        required: false,
        editable: canEdit,
        visible: true,
        max_length: 1000,
      ...
      },
      {
        ...
        label: 'כתובת עסקי',
        type: 'text',
        value: (r.business_address as string | null) ?? null,
        required: false,
        editable: canEdit,
        visible: true,
        max_length: 500,
      ...
      },
      {
        ...
        label: 'כתובת פרטי',
        type: 'text',
        value: (r.private_address as string | null) ?? null,
        required: false,
        editable: canEdit,
        visible: true,
        max_length: 500,
      ...
      },
      {
        ...
        type: 'enum_single',
        value: (r.business_operation_mode as BusinessOperationMode | null) ?? null,
      ...
      },
      {
        ...
        label: 'קהל עיקרי',
        type: 'enum_single',
        value: (r.primary_customer_type as PrimaryCustomerType | null) ?? null,
      ...
      },
      {
        ...
        label: 'עסק עונתי',
        type: 'boolean',
        value: Boolean((r.is_seasonal_business as boolean | null) ?? false),
      ...
      },
      {
        ...
        label: 'פתיחת עסק',
        type: 'date',
        value: (r.business_open_date as string | null) ?? null,
        required: false,
        editable: canEdit,
        visible: true,
        group_key: 'general_info',
      ...
      },
      {
        ...
        label: 'סגירת עסק',
        type: 'date',
        value: (r.business_close_date as string | null) ?? null,
        required: false,
        editable: canEdit,
        visible: true,
        validation: [
...
export async function saveClientBusinessProfileSection(
  ctx: RequestContext,
  clientId: string,
  body: ClientBusinessProfileSaveInput
): Promise<ClientBusinessProfileSaveResponse> {
  const orgId = assertOrg(ctx);
  await ensureClientInOrg(orgId, clientId);
  ...
  let business_domain: string | null = null;
  let business_activity_description: string | null = null;
  let business_address: string | null = null;
  let private_address: string | null = null;
  let business_operation_mode: BusinessOperationMode | null = null;
  let primary_customer_type: PrimaryCustomerType | null = null;
  ...
  let business_open_date: string | null = null;
  let business_close_date: string | null = null;
  ...
  let has_business_vehicles: boolean;
  if (body.has_business_vehicles !== undefined) {
    has_business_vehicles = Boolean(body.has_business_vehicles);
  } else if (existing) {
    has_business_vehicles = Boolean((existing as { has_business_vehicles?: boolean | null }).has_business_vehicles);
  } else {
  ...
  const { data: oldRow } = await supabaseAdmin
    .from('client_accounting_settings')
    .select(
      'business_domain, business_activity_description, business_address, private_address, business_operation_mode, primary_customer_type, is_seasonal_business, peak_months, business_open_date, business_close_date, has_business_vehicles'
    )
    .eq('organization_id', orgId)
    .eq('client_id', clientId)
    .maybeSingle();

  for (const fieldKey of fieldsToCompare as readonly string[]) {
    const oldVal = (oldRow as Record<string, unknown> | null)?.[fieldKey];
    const newVal = (normalizedNew as Record<string, unknown>)[fieldKey];
    ...
    if (JSON.stringify(normOld) !== JSON.stringify(normNew)) changedFields.push(fieldKey);
  }
  ...
  if (updErr) throw new AppError(500, updErr.message ?? 'client_accounting_settings update failed', 'SUPABASE_ERROR');
