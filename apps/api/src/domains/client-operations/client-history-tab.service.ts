...
export type ClientHistoryTabResponse = {
  tab_key: 'history';
  /** Derived from latest matching audit `created_at` in the retention window (for workspace merge / staleness hints). */
  ...
  ui: {
    ...
    /** Product: archival/cleanup job — not implemented in this repo. */
    retention_archival_todo_he: string;
  };
  sections: Array<{
    section_key: HistorySectionKey;
    title_he: string;
    latest_events: Array<{
      event_id: string;
      occurred_at: string;
...
export type ClientHistoryTabCommandBody = {
  type: 'open_history_section' | 'close_history_section';
  payload?: Record<string, unknown>;
};

/** User-facing export: PDF (pdf-lib) and Excel-compatible UTF-8 CSV. JSON is not a product format. */
export type ClientHistoryExportBody = {
  section_key?: HistorySectionKey | string | null;
  from_date?: string | null;
  to_date?: string | null;
  format: 'pdf' | 'excel';
};

export type ClientHistoryExportFileResult = {
  contentType: string;
...
function canViewHistoryTab(ctx: RequestContext): boolean {
...
const SECTION_ORDER: Array<{ key: HistorySectionKey; title_he: string }> = [
...
const KNOWN_SECTION_KEYS = new Set<HistorySectionKey>(SECTION_ORDER.map((s) => s.key));

function sectionTitleHe(key: HistorySectionKey): string {
...
function formatExportPeriodHe(fromD: string | null, toD: string | null): string {
  if (fromD && toD) return `מ-${fromD} עד ${toD}`;
  if (fromD) return `מ-${fromD}`;
  if (toD) return `עד ${toD}`;
  return `${RETENTION_MONTHS} החודשים האחרונים (לפי תצוגת ההיסטוריה במערכת)`;
}

type HumanHistoryExportRow = {
  occurred_display_he: string;
  actor_display_name: string | null;
  summary_he: string;
...
function buildHistoryReportCsv(params: {
  ...
  sectionScopeHe: string;
  periodHe: string;
  rows: HumanHistoryExportRow[];
}): string {
  const lines: string[] = [
...
export function mapAuditRowToHistoryEvent(row: AuditRow): MappedEvent | null {
  ...
  if (action === AUDIT_ACTIONS.CLIENT_OPERATIONS_WORKSPACE_PROFILE_UPDATED) {
    ...
    return {
      ...
      summary_he:
        keys.length > 0 ? `עודכנו פרטי לקוח (${keys.length} שדות)` : 'עודכנו פרטי לקוח',
  ...
  if (action === AUDIT_ACTIONS.CLIENT_PAYROLL_UPDATED) {
    ...
    const keys = Array.isArray(payload.changed_keys) ? (payload.changed_keys as unknown[]).map(String) : [];
    return {
      section_key: 'payroll',
      action_type: cmd,
  ...
  if (action === AUDIT_ACTIONS.CLIENT_ANNUAL_REPORT_UPDATED) {
    const cmd =
      payload.workspace_command != null ? String(payload.workspace_command) : String(payload.command ?? 'annual_update');
    return {
      section_key: 'annual',
      action_type: cmd,
  ...
  if (action === AUDIT_ACTIONS.CLIENT_OBLIGATION_COMMAND_EXECUTED) {
    ...
    const niDeductionsCmd =
      cmd.startsWith('set_ni_deductions') || cmd.startsWith('mark_ni_deductions') || cmd.startsWith('touch_ni_deductions');
    ...
    const cmdToHuman: Record<string, { button_he: string; result_he: string }> = {
...
async function fetchClientAuditRows(orgId: string, clientId: string, cutoffIso: string): Promise<AuditRow[]> {
  ...
  const { data, error } = await supabaseAdmin
    ...
    .limit(4000);
  if (error) throw new AppError(500, error.message ?? 'audit query failed', 'SUPABASE_ERROR');
...
export type ClientHistoryOpenSectionOptions = {
  section_key: HistorySectionKey;
  from_date?: string | null;
  to_date?: string | null;
};

export async function getClientHistoryTabReadModel(
  ctx: RequestContext,
  clientId: string,
  openSection?: ClientHistoryOpenSectionOptions | null
): Promise<ClientHistoryTabResponse | null> {
  const orgId = assertOrg(ctx);
  if (!canViewHistoryTab(ctx)) return null;

  const cutoffIso = retentionCutoffIso();
  const rows = await fetchClientAuditRows(orgId, clientId, cutoffIso);

  const mappedRows: Array<ReturnType<typeof rowToEnvelope>> = [];
  const actorIds = rows.map((r) => r.actor_user_id).filter((x): x is string => Boolean(x));
  const labels = await loadActorLabels(orgId, actorIds);

  let maxTs = 0;
  for (const row of rows) {
    const mapped = mapAuditRowToHistoryEvent(row as AuditRow);
    if (!mapped) continue;
    const label = row.actor_user_id ? labels.get(row.actor_user_id) ?? null : null;
    const env = rowToEnvelope(row as AuditRow, mapped, label);
    mappedRows.push(env);
    const t = Date.parse(row.created_at);
    if (Number.isFinite(t) && t > maxTs) maxTs = t;
  }

  const bySection = new Map<HistorySectionKey, typeof mappedRows>();
  ...
  const sections: ClientHistoryTabResponse['sections'] = SECTION_ORDER.map(({ key, title_he }) => {
  ...
  let open_section: ClientHistoryTabResponse['open_section'] = null;
...
export async function executeClientHistoryTabCommand(
  ctx: RequestContext,
  clientId: string,
  body: ClientHistoryTabCommandBody
): Promise<ClientHistoryOpenSectionOptions | null> {
  const orgId = assertOrg(ctx);
  ...
  if (!c) throw forbidden('Client not found');

  if (!body?.type) throw badRequest('פקודה לא תקינה');
  ...
  if (body.type === 'open_history_section') {
    ...
    const sk = String(p.section_key ?? '').trim() as HistorySectionKey;
    if (!sk || !KNOWN_SECTION_KEYS.has(sk)) throw badRequest('section_key לא חוקי');
 ...
 */
export async function exportClientHistoryReport(
  ctx: RequestContext,
  clientId: string,
  body: ClientHistoryExportBody
  ...
  const { data: clientRow } = await supabaseAdmin
    ...
    .eq('id', clientId)
    .maybeSingle();
  if (!clientRow) throw forbidden('Client not found');

  const clientDisplayName = String(clientRow.display_name ?? '').trim() || '—';

  const skFilter = body.section_key && String(body.section_key).trim() ? (String(body.section_key).trim() as HistorySectionKey) : null;
  if (skFilter && !KNOWN_SECTION_KEYS.has(skFilter)) throw badRequest('section_key לא חוקי');
  ...
  const humanRows: HumanHistoryExportRow[] = [];
  for (const row of rows) {
    const mapped = mapAuditRowToHistoryEvent(row as AuditRow);
    if (!mapped) continue;
    const t = Date.parse(row.created_at);
    ...
    if (skFilter && mapped.section_key !== skFilter) continue;
    const label = row.actor_user_id ? labels.get(row.actor_user_id) ?? null : null;
    const env = rowToEnvelope(row as AuditRow, mapped, label);
    humanRows.push({
  ...
  if (fmt === 'pdf') {
    try {
      ...
      throw new AppError(
        500,
        'לא ניתן להפיק את דוח ה-PDF כרגע. נסה שוב או השתמש בייצוא לאקסל.',
        'PDF_EXPORT_FAILED'
      );
    }
  }
