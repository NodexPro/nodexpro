 ...
 * Fixed contract:
 * 1) PayrollProcessStatus is the single enum for שכר cell + modal status_label.
 * 2) mark_payroll_period_not_relevant: not_relevant=true and all other flags false.
 * 3) sent_to_employer=true requires salary_data_received=true; no_salaries=true forces salary_data_received & sent_to_employer false;
 *    clearing salary_data_received clears sent_to_employer.
 * 4) All period keys / month arithmetic use Asia/Jerusalem (business-time helpers).
 * 5) טופס 102/100 on payroll profile auto-sync to client_ni_deductions_period for the same payroll_period_key.
 */
export type PayrollProcessStatus =
  | 'payroll_not_relevant'
  | 'payroll_no_salaries'
  | 'payroll_ok'
  | 'payroll_ready_to_process'
  | 'payroll_missing_data';
...
type IncomeAdvancesState =
  | 'not_relevant'
  | 'waiting_for_income_data'
  | 'waiting_client_response'
  | 'missing_data'
  | 'ready_to_report'
  | 'reported'
...
type NationalInsuranceSelfEmployedState = 'not_relevant' | 'not_paid' | 'paid_on_time' | 'paid_late';

/**
 * Two different columns — do not mix:
 * · ביטוח לאומי (עצמאי): client_obligations national_insurance_self_employed_payment; modal “תשלום לביטוח לאומי” → שולם = mark_ni_paid. Column label שולם when paid_on_time / paid_late; timing text in period_summary.
 * · ביטוח לאומי ניכויים: client_ni_deductions_period; modals / set_ni_deductions_* / mark_ni_deductions_* only.
 *
 * NI deductions (ניכויים) — policy:
 * 1) client_ni_deductions_period.paid → column שולם; day 15 vs all_completed_at for internal state; סיכום נוכחית + במועד / עם איחור.
 * 2) Auto reminder modal: days 11–15 every visit; 16–31 once per business day (touch_ni_deductions_auto_reminder_shown).
 * 3) payroll_ready gates the flow unless row.paid (then column still שולם from DB truth).
 * 4) mark_ni_deductions_reported_and_paid / paid step requires sent_to_employer + 102 + 100.
 */
export type NiDeductionsState =
  | 'not_relevant'
  | 'not_started'
  | 'in_progress'
  | 'ready'
...
/** מס הכנסה ניכויים — aggregate-only cell states (not client_obligations enum). */
export type IncomeTaxDeductionsAggregateState =
  | 'itd_not_required'
  | 'itd_blocked'
  | 'itd_paid'
  | 'itd_reported'
...
type NiDeductionsPeriodRow = {
  reported_102: boolean;
  reported_100: boolean;
  paid: boolean;
  /** User-marked "משכורות בטיפול"; cleared when all NI-deductions steps complete. */
...
type PayrollPeriodStateRow = {
  salary_data_received: boolean;
  sent_to_employer: boolean;
  no_salaries_this_month: boolean;
  not_relevant: boolean;
};

type ObligationStatus = VatState | PayrollState | IncomeAdvancesState | NationalInsuranceSelfEmployedState;

type PriorityLevel = 'high' | 'medium' | 'low';
type PriorityCode = 'high_today' | 'warning' | 'ok';

type ClientObligationsTabResponse = {
  ...
  payroll_manual_modal: null | {
    ...
    checkboxes: Array<{
      key: 'salary_data_received' | 'sent_to_employer' | 'no_salaries_this_month';
      label_he: string;
      is_checked: boolean;
      interaction: { type: 'command'; command_type: ClientObligationsCommandBody['type']; payload: Record<string, unknown> };
    }>;
    actions: Array<{
      action_key: string;
      ...
      interaction:
        | { type: 'close_modal' }
        | { type: 'command'; command_type: ClientObligationsCommandBody['type']; payload: Record<string, unknown> };
    }>;
  };
  ni_auto_reminder_modal: null | {
    ...
    standing_order_checkbox: {
      ...
      interaction: {
        type: 'command';
        action_key: string;
        command_type: ClientObligationsCommandBody['type'];
    ...
    actions: Array<{
      ...
      action_label_he: string;
      interaction:
        | { type: 'command'; action_key: string; command_type: ClientObligationsCommandBody['type']; payload: Record<string, unknown> }
        | { type: 'open_module'; action_key: string; payload?: Record<string, unknown> };
    }>;
  };
  ...
  ni_deductions_auto_reminder_modal: null | {
    ...
    period_key: string;
    checkboxes: Array<{
      key: 'sent_to_employer' | 'reported_102' | 'reported_100' | 'paid';
      label_he: string;
      is_checked: boolean;
      interaction:
        | { type: 'command'; command_type: ClientObligationsCommandBody['type']; payload: Record<string, unknown> }
    ...
    suppress_checkbox: {
      is_visible: boolean;
      is_checked: boolean;
      interaction: { type: 'command'; command_type: 'set_ni_deductions_reminder_suppressed'; payload: Record<string, unknown> };
    };
    actions: Array<{
      action_key: string;
      action_label_he: string;
  ...
  table: {
    ...
    rows: Array<{
      ...
      cells: Array<{
        ...
        status_code?: ObligationStatus | NiDeductionsState | PayrollProcessStatus | IncomeTaxDeductionsAggregateState;
        status_label_he?: string;
        ...
        interaction?: {
          ...
          modal_key?:
            | 'ni_manual_modal'
            | 'ni_deductions_manual_modal'
            | 'payroll_manual_modal'
            | 'income_tax_deductions_manual_modal';
          command_type?: ClientObligationsCommandBody['type'];
          payload?: Record<string, unknown>;
        };
...
type ClientTasksTabResponse = {
  ...
  tasks: Array<{
    ...
    task_type: string;
    status: string;
    suggested_action: { action_key: string; label_he: string } | null;
    due_at: string | null;
    due_display_he: string | null;
  }>;
  labels: { tab_title_he: string };
};

export type ClientObligationsCommandBody = {
  type:
...
export type ClientTasksCommandBody = {
  type: 'resolve_task' | 'snooze_task' | 'reopen_task';
  payload?: Record<string, unknown>;
};
...
function fmtDateHe(dateLike: string | null): string | null {
...
function periodKeyToDisplayHe(periodKey: string): string {
  const m = /^(\d{4})-(\d{2})$/.exec(periodKey);
  if (!m) return periodKey;
  return `${m[2]}/${m[1]}`;
}

function incomeTaxDeductionsPeriodDisplayHe(periodKey: string): string {
...
type IncomeTaxDeductionsFrequency = 'monthly' | 'bi_monthly' | 'semi_annual';

type IncomeTaxDeductionsRequirement = {
  is_required: boolean;
  due_date: string;
  period_key: string;
  period_label: string;
...
type IncomeTaxDeductionsPeriodRow = {
  reported: boolean;
  paid: boolean;
  not_relevant: boolean;
};

function normalizeIncomeTaxDeductionsFrequency(raw: string | null | undefined): IncomeTaxDeductionsFrequency | null {
  const s = String(raw ?? '')
    .trim()
    .toLowerCase()
    .replace(/\s+/g, '_');
  if (!s) return null;
  if (s === 'monthly' || s === 'חד_חודשי') return 'monthly';
  if (s === 'bi_monthly' || s === 'דו_חודשי') return 'bi_monthly';
  if (s === 'semi_annual' || s === 'חצי_שנתי') return 'semi_annual';
  return null;
}

function joinIncomeTaxDeductionsPeriodKey(a: string, b: string): string {
  return a <= b ? `${a}_${b}` : `${b}_${a}`;
}

/** Asia/Jerusalem; payroll_period_key for monthly = previous month; composite keys for bi-monthly / semi-annual. */
function resolveIncomeTaxDeductionsRequirement(now: Date, frequency: IncomeTaxDeductionsFrequency): IncomeTaxDeductionsRequirement {
...
function defaultIncomeTaxDeductionsPeriodRow(): IncomeTaxDeductionsPeriodRow {
  return { reported: false, paid: false, not_relevant: false };
}

function incomeTaxDeductionsRowFromDb(r: Record<string, unknown> | null): IncomeTaxDeductionsPeriodRow {
  if (!r) return defaultIncomeTaxDeductionsPeriodRow();
  return {
...
async function fetchIncomeTaxDeductionsPeriodRow(
  orgId: string,
  clientId: string,
  periodKey: string
): Promise<IncomeTaxDeductionsPeriodRow & { id?: string }> {
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) throw new AppError(500, error.message ?? 'client_income_tax_deductions_period read failed', 'SUPABASE_ERROR');
  const base = incomeTaxDeductionsRowFromDb((data ?? null) as Record<string, unknown> | null);
  return data ? { ...base, id: String((data as Record<string, unknown>).id) } : base;
}

async function persistIncomeTaxDeductionsRow(
  orgId: string,
  clientId: string,
  periodKey: string,
  next: IncomeTaxDeductionsPeriodRow,
  nowIso: string
): Promise<void> {
  ...
  if (error) throw new AppError(500, error.message ?? 'client_income_tax_deductions_period upsert failed', 'SUPABASE_ERROR');
}

function resolveIncomeTaxDeductionsCell(args: {
  requirement: IncomeTaxDeductionsRequirement;
  payrollReady: boolean;
  row: IncomeTaxDeductionsPeriodRow;
}): {
  state: IncomeTaxDeductionsAggregateState;
  display_value: string;
  status_label_he: string;
  status_tone: 'critical' | 'warning' | 'ok' | 'blocked' | 'neutral';
  icon_key: string;
...
function buildIncomeTaxDeductionsManualModal(
  req: IncomeTaxDeductionsRequirement,
  row: IncomeTaxDeductionsPeriodRow,
  cell: ReturnType<typeof resolveIncomeTaxDeductionsCell>,
  payrollReady: boolean
): NonNullable<ClientObligationsTabResponse['income_tax_deductions_manual_modal']> {
  const p = { period_key: req.period_key };
  const actions: NonNullable<ClientObligationsTabResponse['income_tax_deductions_manual_modal']>['actions'] = [];
...
async function loadIncomeTaxDeductionsCommandContext(
  orgId: string,
  clientId: string,
  commandNow: Date,
  periodKey: string
): Promise<{ freq: IncomeTaxDeductionsFrequency; req: IncomeTaxDeductionsRequirement }> {
  const { data: taxRow, error } = await supabaseAdmin
    ...
    .eq('client_id', clientId)
    .maybeSingle();
  if (error) throw new AppError(500, error.message ?? 'client_tax_settings read failed', 'SUPABASE_ERROR');
  const t = (taxRow ?? null) as Record<string, unknown> | null;
  if (!Boolean(t?.income_tax_deductions_enabled)) throw badRequest('מס הכנסה ניכויים לא מופעל');
  if (!String(t?.income_tax_deductions_file_number ?? '').trim()) throw badRequest('חסר מספר תיק ניכויים');
  const freq = normalizeIncomeTaxDeductionsFrequency(String(t?.income_tax_deductions_frequency ?? ''));
  if (!freq) throw badRequest('חסרה תדירות דיווח ניכויים');
  const req = resolveIncomeTaxDeductionsRequirement(commandNow, freq);
  if (!req.is_required) throw badRequest('דיווח ניכויים לא נדרש לתקופה זו');
  if (periodKey !== req.period_key) throw badRequest('period_key mismatch');
  return { freq, req };
}

async function patchIncomeTaxDeductionsComputedStatuses(
  orgId: string,
  clientId: string,
  now: Date,
  relevant: RelevantObligationInput[]
): Promise<RelevantObligationInput[]> {
  if (!relevant.some((r) => r.obligation_type === 'income_tax_deductions')) return relevant;
  const { data: taxRow, error: taxErr } = await supabaseAdmin
    ...
    .eq('client_id', clientId)
    .maybeSingle();
  if (taxErr) throw new AppError(500, taxErr.message ?? 'client_tax_settings read failed', 'SUPABASE_ERROR');
  const t = (taxRow ?? null) as Record<string, unknown> | null;
  const freq = normalizeIncomeTaxDeductionsFrequency(String(t?.income_tax_deductions_frequency ?? ''));
  ...
  const { data: itdRows, error: itdErr } = await supabaseAdmin
    ...
    .eq('organization_id', orgId)
    .eq('client_id', clientId);
  if (itdErr) throw new AppError(500, itdErr.message ?? 'client_income_tax_deductions_period read failed', 'SUPABASE_ERROR');
  const byKey = new Map<string, IncomeTaxDeductionsPeriodRow>();
  for (const r of (itdRows ?? []) as Array<Record<string, unknown>>) {
    byKey.set(String(r.period_key), incomeTaxDeductionsRowFromDb(r));
  }
  return relevant.map((rel) => {
    if (rel.obligation_type !== 'income_tax_deductions') return rel;
    const req = resolveIncomeTaxDeductionsRequirement(now, freq);
    if (!req.is_required || rel.period_key !== req.period_key) return rel;
    const row = byKey.get(rel.period_key) ?? defaultIncomeTaxDeductionsPeriodRow();
    let st: ObligationStatus = 'ready_to_process';
    if (!payrollReady) st = 'waiting_for_salary_data';
    else if (row.not_relevant) st = 'not_relevant';
    else if (row.paid) st = 'paid_on_time';
    else if (row.reported) st = 'reported';
    else st = 'ready_to_process';
    return { ...rel, computed_status: st };
  });
}

function defaultPayrollPeriodStateRow(): PayrollPeriodStateRow {
  return {
...
function payrollPeriodStateFromDb(r: Record<string, unknown> | null): PayrollPeriodStateRow {
  if (!r) return defaultPayrollPeriodStateRow();
  return {
...
function computePayrollProcessStatus(row: PayrollPeriodStateRow): PayrollProcessStatus {
  if (row.not_relevant) return 'payroll_not_relevant';
  if (row.no_salaries_this_month) return 'payroll_no_salaries';
  if (row.sent_to_employer) return 'payroll_ok';
  if (row.salary_data_received) return 'payroll_ready_to_process';
  return 'payroll_missing_data';
}

function payrollProcessPresentation(s: PayrollProcessStatus): {
  display_value: string;
  status_label_he: string;
  status_tone: 'critical' | 'warning' | 'ok' | 'blocked' | 'neutral';
  icon_key: string;
...
function payrollProcessToPriorityStatus(s: PayrollProcessStatus): ObligationStatus {
  switch (s) {
    case 'payroll_missing_data':
      return 'missing_data';
    case 'payroll_ready_to_process':
...
function isPayrollReadyForNiDeductionsFlow(payrollProcess: PayrollProcessStatus): boolean {
  return payrollProcess === 'payroll_ok' || payrollProcess === 'payroll_no_salaries';
}

async function fetchPayrollPeriodState(orgId: string, clientId: string, payrollPeriodKey: string): Promise<PayrollPeriodStateRow> {
  const { data, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) throw new AppError(500, error.message ?? 'client_payroll_period_state read failed', 'SUPABASE_ERROR');
  return payrollPeriodStateFromDb((data ?? null) as Record<string, unknown> | null);
}

async function persistPayrollPeriodState(
  orgId: string,
  clientId: string,
  payrollPeriodKey: string,
  row: PayrollPeriodStateRow,
  nowIso: string
): Promise<void> {
  ...
  if (error) throw new AppError(500, error.message ?? 'client_payroll_period_state upsert failed', 'SUPABASE_ERROR');
}
 ...
 */
function operationalProfileSalaryReceivedFlagFromPayrollRow(row: PayrollPeriodStateRow): boolean {
  if (row.not_relevant) return true;
  if (row.no_salaries_this_month) return true;
  if (row.salary_data_received) return true;
  return false;
}

async function syncSalaryReceivedProfileFlagWithPayrollPeriodIfCurrent(
  orgId: string,
  clientId: string,
  payrollPeriodKey: string,
  row: PayrollPeriodStateRow,
  ...
  if (payrollPeriodKey !== businessPreviousMonthKey(now)) return;
  ...
  if (error) throw new AppError(500, error.message ?? 'client_operational_profiles upsert failed', 'SUPABASE_ERROR');
}

/** הביא חומר · שכר sets profile flag; aggregate read keeps client_payroll_period_state aligned (single DB truth). */
function shouldPersistPayrollSalaryReceivedFromProfileFlag(
  profileSalaryReceived: boolean,
  row: PayrollPeriodStateRow
): boolean {
  if (!profileSalaryReceived) return false;
  if (row.not_relevant || row.no_salaries_this_month) return false;
  if (row.salary_data_received) return false;
  return true;
}

async function persistPayrollSalaryReceivedWhenProfileSaysSo(
  orgId: string,
  clientId: string,
  payrollPeriodKey: string,
  prev: PayrollPeriodStateRow,
  now: Date
): Promise<PayrollPeriodStateRow> {
  const nowIso = now.toISOString();
  const next: PayrollPeriodStateRow = {
    ...prev,
    not_relevant: false,
...
function buildPayrollManualModal(
  payrollPeriodKey: string,
  row: PayrollPeriodStateRow
): ClientObligationsTabResponse['payroll_manual_modal'] {
...
function defaultNiDeductionsRow(): NiDeductionsPeriodRow {
  return {
    reported_102: false,
    reported_100: false,
    paid: false,
...
function niDeductionsRowFromDb(r: Record<string, unknown> | null): NiDeductionsPeriodRow {
  if (!r) return defaultNiDeductionsRow();
  return {
    ...
    not_relevant: Boolean(r.not_relevant),
    auto_reminder_last_shown_at: (r.auto_reminder_last_shown_at as string | null) ?? null,
    all_completed_at: (r.all_completed_at as string | null) ?? null,
  };
}

function computeNiDeductionsState(
  row: NiDeductionsPeriodRow,
  periodKey: string,
  now: Date,
  sentToEmployer: boolean
): NiDeductionsState {
  if (row.not_relevant) return 'not_relevant';
  ...
  if (closed) return 'not_paid';
  if (S && A && B && !P) return 'ready';
  if (row.payroll_in_progress) return 'in_progress';
  const c = Number(S) + Number(A) + Number(B) + Number(P);
  if (c === 0) return 'not_started';
  return 'in_progress';
}

function niDeductionsPresentation(state: NiDeductionsState): {
  display_value: string;
  status_label_he: string;
  status_tone: 'critical' | 'warning' | 'ok' | 'blocked' | 'neutral';
  icon_key: string;
...
async function fetchNiDeductionsPeriodRow(
  orgId: string,
  clientId: string,
  periodKey: string
): Promise<NiDeductionsPeriodRow & { id?: string }> {
  const { data, error } = await supabaseAdmin
    ...
    .eq('period_key', periodKey)
    .maybeSingle();
  if (error) throw new AppError(500, error.message ?? 'client_ni_deductions_period read failed', 'SUPABASE_ERROR');
  const base = niDeductionsRowFromDb((data ?? null) as Record<string, unknown> | null);
  return data ? { ...base, id: String((data as Record<string, unknown>).id) } : base;
}

async function persistNiDeductionsRow(
  orgId: string,
  clientId: string,
  periodKey: string,
  next: NiDeductionsPeriodRow,
  nowIso: string
): Promise<void> {
  ...
  if (error) throw new AppError(500, error.message ?? 'client_ni_deductions_period upsert failed', 'SUPABASE_ERROR');
}

/** After payroll profile flags טופס 102/100 — mirror into ניכויים flags (same payroll_period_key = previous month, Asia/Jerusalem). */
export async function syncNiDeductionsReportingFromPayrollProfile(orgId: string, clientId: string): Promise<void> {
  const now = new Date();
  const nowIso = now.toISOString();
  const mm = businessPreviousMonthKey(now);
  const { data: taxRow } = await supabaseAdmin
    ...
    .maybeSingle();
  const t = (taxRow ?? null) as Record<string, unknown> | null;
  const niTaxApplicable =
    Boolean(t?.income_tax_deductions_enabled) && String(t?.national_insurance_deductions_file_number ?? '').trim() !== '';
  if (!niTaxApplicable) return;
  ...
  if (cur.not_relevant) return;

  const next: NiDeductionsPeriodRow = { ...cur };
  let changed = false;
  ...
  if (!allFour) {
    ...
    if (!wasAllFour) next.all_completed_at = nowIso;
  }
  await persistNiDeductionsRow(orgId, clientId, mm, next, nowIso);
}

async function touchNiDeductionsAutoReminderShown(orgId: string, clientId: string, periodKey: string, now: Date): Promise<void> {
  ...
  const { data: ex, error: rErr } = await supabaseAdmin
    ...
    .eq('period_key', periodKey)
    .maybeSingle();
  if (rErr) throw new AppError(500, rErr.message ?? 'client_ni_deductions_period read failed', 'SUPABASE_ERROR');
  if (ex) {
    const { error: uErr } = await supabaseAdmin
      .from('client_ni_deductions_period')
      .update({ auto_reminder_last_shown_at: nowIso, updated_at: nowIso })
      .eq('id', String((ex as Record<string, unknown>).id));
    if (uErr) throw new AppError(500, uErr.message ?? 'client_ni_deductions_period update failed', 'SUPABASE_ERROR');
  } else {
    ...
    if (iErr) throw new AppError(500, iErr.message ?? 'client_ni_deductions_period insert failed', 'SUPABASE_ERROR');
  }
}

function niDeductionsCheckboxDefs(
  periodKey: string,
  row: NiDeductionsPeriodRow,
  payrollRow: PayrollPeriodStateRow
): Array<{
  key: 'sent_to_employer' | 'reported_102' | 'reported_100' | 'paid';
  label_he: string;
  is_checked: boolean;
  interaction:
    | { type: 'command'; command_type: 'set_ni_deductions_step'; payload: Record<string, unknown> }
    | { type: 'open_modal'; modal_key: 'payroll_manual_modal' };
}> {
  const p = { period_key: periodKey };
  return [
    {
      key: 'sent_to_employer' as const,
      label_he: 'משכורות נשלחו למעסיק',
      is_checked: payrollRow.sent_to_employer,
      interaction: { type: 'open_modal' as const, modal_key: 'payroll_manual_modal' as const },
    },
    {
      key: 'reported_102' as const,
      label_he: 'טופס 102 דווח',
      is_checked: row.reported_102,
      interaction: {
        type: 'command' as const,
        command_type: 'set_ni_deductions_step' as const,
        payload: { ...p, step: 'reported_102', enabled: !row.reported_102 },
      },
    },
    {
      key: 'reported_100' as const,
...
function niDeductionsModalActions(
  periodKey: string,
  ndState: NiDeductionsState
): Array<{
  action_key: string;
  action_label_he: string;
  interaction: { type: 'command'; command_type: ClientObligationsCommandBody['type']; payload: Record<string, unknown> };
}> {
  if (ndState === 'paid_on_time' || ndState === 'paid_late') return [];
  const p = { period_key: periodKey };
  return [
    {
...
function shiftUtcMonth(year: number, month1to12: number, delta: number): { year: number; month: number } {
...
function isNiDeductionsPeriodClosed(now: Date, periodKey: string): boolean {
  return businessMonthKey(now) > periodKey;
}

function vatReportingPeriodMonthly(now: Date): { period_key: string; reporting_end_year: number; reporting_end_month: number } {
...
function vatReportingPeriodBiMonthly(now: Date): {
  period_key: string;
  reporting_end_year: number;
  reporting_end_month: number;
} {
  const curYear = now.getUTCFullYear();
  const curMonth = now.getUTCMonth() + 1;
  const prev = shiftUtcMonth(curYear, curMonth, -1);
...
function vatPeriodDisplayHe(periodKey: string): string {
...
function statusLabelHe(status: ObligationStatus): string {
  switch (status) {
    case 'not_relevant':
      return 'לא רלוונטי';
...
function computePriorityScore(status: ObligationStatus, obligationType: string, hasMissingData: boolean): number {
  void obligationType;
  if (status === 'not_reported' || status === 'reported_late') return 100;
  if (status === 'not_paid') return 92;
  if (status === 'missing_data') return hasMissingData ? 95 : 80;
  if (status === 'paid_late') return 98;
  if (status === 'paid_on_time') return 20;
  if (status === 'pending_payment_confirmation') return 75;
  if (status === 'waiting_client_response' || status.startsWith('waiting_for_')) return 65;
  if (status === 'ready_to_report' || status === 'ready_to_process') return 55;
  if (status === 'reported' || status === 'reported_and_paid') return 20;
  return 10;
}
...
function statusTone(status: ObligationStatus): 'critical' | 'warning' | 'ok' | 'blocked' | 'neutral' {
  if (status === 'missing_data') return 'blocked';
  if (status === 'not_reported' || status === 'reported_late' || status === 'not_paid' || status === 'paid_late') return 'critical';
  if (status === 'waiting_client_response' || status.startsWith('waiting_for_') || status === 'pending_payment_confirmation') return 'warning';
  if (status === 'reported' || status === 'reported_and_paid' || status === 'ready_to_report' || status === 'ready_to_process' || status === 'paid_on_time') return 'ok';
  return 'neutral';
}

function statusIconKey(status: ObligationStatus): string {
  if (status === 'missing_data') return 'black_dot';
  if (status === 'not_reported' || status === 'reported_late' || status === 'not_paid' || status === 'paid_late') return 'red_dot';
  if (status === 'waiting_client_response' || status.startsWith('waiting_for_') || status === 'pending_payment_confirmation') return 'yellow_dot';
  if (status === 'reported' || status === 'reported_and_paid' || status === 'ready_to_report' || status === 'ready_to_process' || status === 'paid_on_time')
    return 'green_dot';
  return 'gray_dot';
}

function cellsToPriorityState(states: ObligationStatus[]): {
  code: PriorityCode;
  label_he: string;
...
function isVatBiMonthlyReportingMonth(now: Date): boolean {
  const m = now.getUTCMonth() + 1;
  return m === 1 || m === 3 || m === 5 || m === 7 || m === 9 || m === 11;
}

function requestPriorityForMissingData(
  checkboxKey: 'vat' | 'income_advances' | 'payroll',
  dayOfMonth: number
): 'info' | 'warning' | 'critical' | null {
  if (dayOfMonth < 1) return null;
  if (checkboxKey === 'payroll') {
    if (dayOfMonth >= 10) return 'critical';
    if (dayOfMonth >= 4) return 'warning';
    return 'info';
  }
  if (checkboxKey === 'vat') {
    if (dayOfMonth >= 10) return 'critical';
  ...
  if (dayOfMonth >= 10) return 'critical';
  if (dayOfMonth >= 8) return 'warning';
  return 'info';
}

function requestStageForMissingData(dayOfMonth: number): 'day_1' | 'day_4' | 'day_8' | 'day_10_plus' | null {
  if (dayOfMonth >= 10) return 'day_10_plus';
  if (dayOfMonth >= 8) return 'day_8';
  if (dayOfMonth >= 4) return 'day_4';
  if (dayOfMonth >= 1) return 'day_1';
  return null;
}

function requestTextHe(
  checkboxKey: 'vat' | 'income_advances' | 'payroll',
  stage: 'day_1' | 'day_4' | 'day_8' | 'day_10_plus'
): string {
  if (checkboxKey === 'payroll') {
    if (stage === 'day_1') return 'נא לשלוח נתוני שכר';
    if (stage === 'day_4') return 'תזכורת: חסרים נתוני שכר';
    if (stage === 'day_8') return 'דחוף: יש לשלוח נתוני שכר היום';
    return 'קריטי: נתוני שכר באיחור';
  }
  if (checkboxKey === 'vat') {
    if (stage === 'day_1') return 'נא להעביר חומר למע"מ';
    if (stage === 'day_4') return 'תזכורת: חסר חומר למע"מ';
    if (stage === 'day_8') return 'דחוף: חסר חומר למע"מ';
    return 'סיכון איחור: מע"מ ללא חומר';
  }
  if (stage === 'day_1') return 'נא לעדכן האם היו הכנסות';
  if (stage === 'day_4') return 'תזכורת: עדכון הכנסות נדרש';
  if (stage === 'day_8') return 'בדיקה: היו הכנסות החודש?';
  return 'דחוף: חסרים נתוני הכנסות';
}

function isVatReportingPipelineStatus(status: ObligationStatus): boolean {
  return (
...
function isVatBlockedByMaterialPrerequisite(status: ObligationStatus): boolean {
  return status === 'waiting_for_material' || status === 'waiting_client_response' || status === 'missing_data';
}

function isNationalInsuranceSelfEmployedObligation(obligationType: string): boolean {
  return obligationType === 'national_insurance_self_employed_payment';
}

type ObligationRowView = {
  obligation_id: string;
  obligation_type: string;
  period_key: string;
  period_display_he: string;
...
function cellInteractionForObligation(
  obligation: ObligationRowView,
  columnKey: string
): {
  type: 'command' | 'open_module' | 'open_modal' | 'none';
  action_key?: string;
  modal_key?: 'ni_manual_modal' | 'ni_deductions_manual_modal' | 'income_tax_deductions_manual_modal';
  command_type?: ClientObligationsCommandBody['type'];
  payload?: Record<string, unknown>;
} {
  if (columnKey === 'payroll') return { type: 'open_module', action_key: 'open_payroll' };
  ...
  if (columnKey === 'material') return { type: 'open_module', action_key: 'request_documents' };
  if (columnKey === 'vat') return { type: 'open_module', action_key: 'report_vat' };
  if (columnKey === 'income_advances') return { type: 'open_module', action_key: 'open_income_tax' };
  if (columnKey === 'ni') return { type: 'open_module', action_key: 'open_national_insurance' };
  if (columnKey === 'income_tax_deductions') return { type: 'open_module', action_key: 'open_income_tax_deductions' };
  return { type: 'none' };
}

function availableActionsForCell(
  columnKey: string,
  obligation: ObligationRowView | null
): NonNullable<ClientObligationsTabResponse['table']['rows'][number]['cells'][number]['available_actions']> {
  const out: NonNullable<ClientObligationsTabResponse['table']['rows'][number]['cells'][number]['available_actions']> = [];
  if (!obligation) return out;
  if (columnKey === 'ni' && isNationalInsuranceSelfEmployedObligation(obligation.obligation_type)) return out;
  ...
  if (columnKey === 'material') {
    ...
    if (isVatBlockedByMaterialPrerequisite(obligation.status_code)) return out;
    out.push(
      {
        action_key: 'mark_reported',
...
function niDeductionsStateToPriorityStatus(s: NiDeductionsState): ObligationStatus {
  switch (s) {
    case 'not_paid':
      return s;
    case 'paid_late':
...
function niSelfEmployedPriorityStatus(ob: ObligationRowView, todayYmd: string): ObligationStatus {
  if (ob.status_code === 'not_paid' && ob.due_date && todayYmd > ob.due_date) return 'paid_late';
  return ob.status_code;
}

function nationalInsuranceSelfEmployedCellPresentation(
  ob: ObligationRowView,
  todayYmd: string
): { display_value: string; status_label_he: string; status_tone: ReturnType<typeof statusTone>; icon_key: string } {
...
function pickNiSelfEmployedObligation(
  obligations: ObligationRowView[],
  now: Date,
  preferredPeriodKey: string | null
): ObligationRowView | null {
  const pk = (preferredPeriodKey ?? '').trim() || monthKey(now);
  return (
    obligations.find((o) => o.obligation_type === 'national_insurance_self_employed_payment' && o.period_key === pk) ??
    obligations.find((o) => o.obligation_type === 'national_insurance_self_employed_payment') ??
    null
  );
}

function buildObligationsTable(
  obligations: ObligationRowView[],
  taxSnapshot:
    | {
        ...
        national_insurance_standing_order_until: string | null;
      }
    | null,
  profileSnapshot:
    | {
    ...
    | null,
  now: Date,
  niDeductionsCtx: { periodKey: string; payrollReady: boolean; row: NiDeductionsPeriodRow; sentToEmployer: boolean } | null,
  payrollProcessCtx: { payrollPeriodKey: string; payrollProcess: PayrollProcessStatus } | null,
  niSelfEmployedPeriodKey: string | null,
  incomeTaxDeductionsCtx: {
    requirement: IncomeTaxDeductionsRequirement;
    payrollReady: boolean;
    row: IncomeTaxDeductionsPeriodRow;
  } | null
): ClientObligationsTabResponse['table'] {
  const columns: ClientObligationsTabResponse['table']['columns'] = [
  ...
  const pick = (pred: (o: ObligationRowView) => boolean): ObligationRowView | null => obligations.find(pred) ?? null;
  ...
  const fallbackDisplayByColumn = (columnKey: string): string => {
    if (columnKey === 'payroll') return 'אין עובדים';
    if (columnKey === 'material') return 'לא נדרש';
    return 'לא רלוונטי';
  };
  const statusCell = (columnKey: string, ob: ObligationRowView | null): ClientObligationsTabResponse['table']['rows'][number]['cells'][number] => {
    if (!ob) {
      const interactionByColumn: Record<string, { type: 'command' | 'open_module' | 'none'; action_key?: string }> = {
  ...
  const dueDateCell = (columnKey: string, ob: ObligationRowView | null): ClientObligationsTabResponse['table']['rows'][number]['cells'][number] => ({
    column_key: columnKey,
    kind: 'date',
  ...
  const materialCell = (): ClientObligationsTabResponse['table']['rows'][number]['cells'][number] => {
    ...
    const payrollActive =
      Boolean(taxSnapshot?.income_tax_deductions_enabled) || String(taxSnapshot?.national_insurance_deductions_file_number ?? '').trim() !== '';
    ...
    const checkbox = (
      ...
      const markReceivedCommand =
        key === 'vat'
          ? 'mark_material_received'
          : key === 'income_advances'
            ? 'mark_income_data_received'
            : 'mark_salary_data_received';
      const markNotRelevantCommand =
        key === 'vat'
          ? 'mark_material_not_relevant'
          : key === 'income_advances'
            ? 'mark_income_data_not_relevant'
      ...
      return {
        ...
        request_interaction: shouldShowRequestIcon
          ? {
              type: 'command' as const,
              action_key: 'request_data',
              command_type: 'send_docflow_request' as const,
        ...
        interaction: !is_active
          ...
          : !is_checked
            ? {
                type: 'command' as const,
                action_key: 'mark_received',
                command_type: markReceivedCommand as ClientObligationsCommandBody['type'],
                payload: payloadBase,
              }
            : { type: 'none' as const },
          ...
          : [
              ...
              },
              {
                ...
                interaction: {
                  type: 'command' as const,
                  action_key: 'mark_not_relevant',
                  command_type: markNotRelevantCommand as ClientObligationsCommandBody['type'],
                  payload: payloadBase,
                },
              },
            ],
  ...
  const payrollProcessCell = (): ClientObligationsTabResponse['table']['rows'][number]['cells'][number] => {
    ...
    (() => {
      ...
      if (!incomeTaxDeductionsEnabled) {
        return {
          ...
          display_value: 'לא רלוונטי',
          status_code: 'not_relevant' as const,
          status_label_he: 'לא רלוונטי',
          status_tone: 'neutral' as const,
          icon_key: 'gray_dot',
          action_label_he: 'פתח מיסים',
          interaction: { type: 'open_module' as const, action_key: 'open_national_insurance_deductions' },
          available_actions: [],
        };
      }
      ...
      return {
        ...
        kind: 'status' as const,
          display_value: 'ממתין',
          status_code: 'missing_data' as const,
          status_label_he: 'חסר נתונים',
          status_tone: 'blocked' as const,
          icon_key: 'yellow_dot',
        action_label_he: 'פתח מיסים',
        interaction: { type: 'open_module' as const, action_key: 'open_national_insurance_deductions' },
          available_actions: [],
      ...
      if (!niDeductionsCtx) {
        return {
          ...
          status_code: 'waiting_for_salary_data' as const,
      ...
      if (niDeductionsCtx.row.paid) {
        const ndStatePaid = computeNiDeductionsState(
          niDeductionsCtx.row,
          niDeductionsCtx.periodKey,
          now,
          niDeductionsCtx.sentToEmployer
        );
        ...
        return {
          ...
          icon_key: presPaid.icon_key,
          action_label_he: 'עדכון',
          interaction: { type: 'open_modal' as const, modal_key: 'ni_deductions_manual_modal' as const },
          available_actions: [],
        };
      }
      ...
      const ndState = computeNiDeductionsState(
        niDeductionsCtx.row,
        niDeductionsCtx.periodKey,
        now,
        niDeductionsCtx.sentToEmployer
    ...
    (() => {
      ...
      if (!enabled) {
        return {
          ...
          icon_key: 'gray_dot',
          action_label_he: 'פתח מיסים',
          interaction: { type: 'open_module' as const, action_key: 'open_income_tax_deductions' },
          available_actions: [],
        };
      }
      ...
      const openItdModal =
        incomeTaxDeductionsCtx.requirement.is_required && cell.state !== 'itd_not_required';
      return {
        column_key: 'income_tax_deductions',
        kind: 'status' as const,
        ...
        interaction: openItdModal
          ? { type: 'open_modal' as const, modal_key: 'income_tax_deductions_manual_modal' as const }
          : { type: 'open_module' as const, action_key: 'open_income_tax_deductions' },
        available_actions: [],
      };
    })(),
    (() => {
      const ndState =
        niDeductionsCtx && (niDeductionsCtx.row.paid || niDeductionsCtx.payrollReady)
          ? computeNiDeductionsState(niDeductionsCtx.row, niDeductionsCtx.periodKey, now, niDeductionsCtx.sentToEmployer)
          : null;
      const priorityInputs: ObligationStatus[] = [
      ...
      if (niSelf) priorityInputs.push(niSelfEmployedPriorityStatus(niSelf, todayYmd));
      if (ndState) priorityInputs.push(niDeductionsStateToPriorityStatus(ndState));
      ...
      return {
        ...
        interaction: { type: 'open_module' as const, action_key: 'open_tasks' },
        available_actions: [],
      };
    })(),
  ];
...
function obligationActionList(obligationType: string): Array<{ action_key: string; label_he: string }> {
...
function taskSuggestedActionByType(taskType: string): { action_key: string; label_he: string } | null {
  switch (taskType) {
...
type RelevantObligationInput = {
  obligation_type: string;
  period_key: string;
  due_date: string;
  related_module: string;
  computed_status: ObligationStatus;
};

function vatDueDateFromPeriod(
  reportingEndYear: number,
  reportingEndMonth: number,
  vatDueType: string | null
): string {
...
function annualDueDate(now: Date): string {
  const y = Number(businessYmd(now).slice(0, 4));
  return `${y}-05-31`;
}

async function buildRelevantObligations(orgId: string, clientId: string, now: Date): Promise<RelevantObligationInput[]> {
  const [{ data: profRow }, { data: taxRow }] = await Promise.all([
    ...
    supabaseAdmin
      .from('client_tax_settings')
      .select(
        'vat_type,vat_frequency,vat_due_type,income_tax_advance_enabled,income_tax_deductions_enabled,income_tax_deductions_file_number,income_tax_deductions_frequency,national_insurance_type,national_insurance_monthly_amount,national_insurance_standing_order_until'
      )
      .eq('organization_id', orgId)
      .eq('client_id', clientId)
      .maybeSingle(),
  ...
  const p = (profRow ?? {}) as Record<string, unknown>;
  const t = (taxRow ?? {}) as Record<string, unknown>;

  const out: RelevantObligationInput[] = [];
  ...
  if (String(t.vat_type ?? '') === 'yes') {
    ...
    const due = vatDueDateFromPeriod(vatPeriod.reporting_end_year, vatPeriod.reporting_end_month, (t.vat_due_type as string | null) ?? null);
    const periodKey = vatPeriod.period_key;
    const missingMaterialState: ObligationStatus = day > 6 ? 'missing_data' : 'waiting_for_material';
    const status: ObligationStatus = materialBrought ? 'ready_to_report' : missingMaterialState;
    out.push({
  ...
  if (hasDeductionsFile || Boolean(p.payroll_flag)) {
    const due = `${mm}-10`;
    const status: ObligationStatus = salaryReceived ? 'ready_to_process' : day >= 10 ? 'missing_data' : 'waiting_for_salary_data';
    out.push({
  ...
  if (Boolean(t.income_tax_advance_enabled)) {
    const due = `${mm}-10`;
    const status: ObligationStatus = incomeReceived ? 'ready_to_report' : day >= 10 ? 'missing_data' : 'waiting_for_income_data';
    out.push({
  ...
  if (niEnabled) {
    ...
    const standingOrderActive = Boolean(standingOrderUntil) && standingOrderUntil >= ymd(now);
    const status: ObligationStatus = standingOrderActive ? 'paid_on_time' : 'not_paid';
    out.push({
...
async function loadNiSelfEmployedRelevantObligation(
  orgId: string,
  clientId: string,
  now: Date
): Promise<RelevantObligationInput | null> {
...
async function upsertNiSelfEmployedRowFromRelevant(
  orgId: string,
  clientId: string,
  niRel: RelevantObligationInput,
  ...
  const { error: upErr } = await supabaseAdmin.from('client_obligations').upsert(
    {
      ...
      docflow_message_sent_at: (r?.docflow_message_sent_at as string | null) ?? null,
      last_reported_at: (r?.last_reported_at as string | null) ?? null,
...
export async function recomputeClientObligationsAndTasks(
  ctx: RequestContext,
  clientId: string,
  now: Date = new Date()
): Promise<void> {
...
async function recomputeClientObligationsAndTasksForOrg(ctx: RequestContext, orgId: string, clientId: string, now: Date): Promise<void> {
  ...
  const { data: profileRow } = await supabaseAdmin
    ...
    .eq('client_id', clientId)
    .maybeSingle();
  const materialBrought = Boolean((profileRow as Record<string, unknown> | null)?.material_brought_flag);
  const { data: currentRows, error } = await supabaseAdmin
    .from('client_obligations')
    .select('*')
    .eq('organization_id', orgId)
    .eq('client_id', clientId);
  if (error) throw new AppError(500, error.message ?? 'client_obligations read failed', 'SUPABASE_ERROR');
  const current = (currentRows ?? []) as Array<Record<string, unknown>>;
  const keyOf = (o: { obligation_type: string; period_key: string }) => `${o.obligation_type}::${o.period_key}`;
  const currentByKey = new Map<string, Record<string, unknown>>(current.map((r) => [keyOf({ obligation_type: String(r.obligation_type), period_key: String(r.period_key) }), r]));
  const nextKeys = new Set(relevant.map((r) => keyOf(r)));

  for (const rel of relevant) {
    const row = currentByKey.get(keyOf(rel));
    const existingStatus = row ? (String(row.status) as ObligationStatus) : null;
    const niStandingOrderOverridesNotPaid =
      rel.obligation_type === 'national_insurance_self_employed_payment' && rel.computed_status === 'paid_on_time';
    const commandOwned =
    ...
    const keepCommandOwned =
      commandOwned && existingStatus ? !(vatPrerequisiteMissing && isVatReportingPipelineStatus(existingStatus)) : false;
    let status = keepCommandOwned && existingStatus ? existingStatus : rel.computed_status;
    const docflowSentAt = (row?.docflow_message_sent_at as string | null) ?? null;
    if (
      !keepCommandOwned &&
...
function taskTypeByState(
  status: ObligationStatus,
  obligationType: string
): { task_type: string; title_he: string; action: string } {
  if (status === 'missing_data') {
    if (obligationType === 'payroll_data') return { task_type: 'missing_salary_data', title_he: 'חסרים נתוני שכר', action: 'פתח שכר' };
    if (obligationType === 'income_tax_advance') return { task_type: 'missing_income_data', title_he: 'חסרות הכנסות למקדמות', action: 'בדוק הכנסות' };
    return { task_type: 'missing_material', title_he: 'חסר חומר מהלקוח', action: 'פתח מסמכים' };
  }
  if (status === 'not_reported') return { task_type: 'not_reported', title_he: 'לא דווח', action: 'פתח התחייבויות' };
  if (status === 'reported_late') return { task_type: 'reported_late', title_he: 'הוגש באיחור', action: 'פתח התחייבויות' };
  if (status === 'not_paid') return { task_type: 'ni_not_paid', title_he: 'ביטוח לאומי לא שולם', action: 'בדוק עם הלקוח' };
  if (status === 'paid_late') return { task_type: 'ni_not_paid', title_he: 'ביטוח לאומי לא שולם (באיחור)', action: 'בדוק עם הלקוח' };
  if (status === 'pending_payment_confirmation') return { task_type: 'payment_pending_confirmation', title_he: 'ממתין לאישור תשלום', action: 'בדוק תשלום' };
...
async function syncTasksFromObligations(orgId: string, clientId: string, now: Date): Promise<void> {
  ...
  const existingTasks = (tasks ?? []) as Array<{ id: string; source_id: string | null; task_type: string; status: string }>;
  const obligationsRows = (obligations ?? []) as Array<Record<string, unknown>>;
  const taskByProblem = new Map<string, { id: string; status: string }>();
  for (const t of existingTasks) {
    if (!t.source_id) continue;
    const k = `${t.source_id}::${t.task_type}`;
    if (!taskByProblem.has(k)) taskByProblem.set(k, { id: t.id, status: t.status });
  }

  for (const ob of obligationsRows) {
    const obId = String(ob.id);
    const status = String(ob.status) as ObligationStatus;
    const actionable =
      status === 'missing_data' ||
      status === 'not_reported' ||
      status === 'reported_late' ||
    ...
    const hasMissingData =
      !Boolean(p.material_brought_flag) || !Boolean(p.salary_data_received_flag) || !Boolean(p.income_data_received_flag);
    ...
    if (!actionable) {
      for (const [k, ex] of taskByProblem.entries()) {
        if (!k.startsWith(`${obId}::`)) continue;
        if (ex.status !== 'open' && ex.status !== 'snoozed') continue;
        await supabaseAdmin
          .from('client_tasks')
    ...
    if (existing && (existing.status === 'open' || existing.status === 'snoozed')) {
      await supabaseAdmin
        ...
        .update({
          title_he,
          description_he: (ob.blocking_reason as string | null) ?? null,
    ...
    const { error: insErr } = await supabaseAdmin.from('client_tasks').insert({
      organization_id: orgId,
      assignee_user_id: (p.assigned_handler_user_id as string | null) ?? null,
      client_id: clientId,
    ...
    if (insErr) throw new AppError(500, insErr.message ?? 'client_tasks insert failed', 'SUPABASE_ERROR');
    await supabaseAdmin
      .from('client_tasks')
...
export async function getClientObligationsTabReadModel(ctx: RequestContext, clientId: string): Promise<ClientObligationsTabResponse | null> {
  const orgId = assertOrg(ctx);
  await ensureClientInOrg(orgId, clientId);
  ...
  const [{ data: rows, error }, { data: taxRow }, { data: profileRow }] = await Promise.all([
    ...
    supabaseAdmin
      .from('client_tax_settings')
      .select(
        'national_insurance_type,national_insurance_monthly_amount,national_insurance_deductions_file_number,income_tax_deductions_enabled,income_tax_deductions_file_number,income_tax_deductions_frequency,vat_frequency,income_tax_advance_enabled,national_insurance_standing_order_until'
      )
      .eq('organization_id', orgId)
      .eq('client_id', clientId)
      .maybeSingle(),
  ...
  if (error) throw new AppError(500, error.message ?? 'obligations read failed', 'SUPABASE_ERROR');
  const all = (rows ?? []) as Array<Record<string, unknown>>;
  const taxSnapshot = (taxRow ?? null) as
    | {
        national_insurance_type: string | null;
        national_insurance_monthly_amount: number | null;
        national_insurance_deductions_file_number: string | null;
    ...
    | null;
  const profileSnapshot = (profileRow ?? null) as
    | {
        material_brought_flag: boolean | null;
        salary_data_received_flag: boolean | null;
        income_data_received_flag: boolean | null;
  ...
  const obligations: ObligationRowView[] = all.map((r) => {
    const status = String(r.status) as ObligationStatus;
    const blockingReason = (r.blocking_reason as string | null) ?? null;
    ...
    return {
      ...
      period_display_he:
        String(r.obligation_type) === 'vat_report'
  ...
  const mm = businessPreviousMonthKey(now);
  const payrollOblForScope =
    obligations.find((o) => o.obligation_type === 'payroll_data' || o.obligation_type === 'payroll_submission') ?? null;
  ...
  if (
    ...
    payrollRowForPeriod = await persistPayrollSalaryReceivedWhenProfileSaysSo(
      orgId,
      clientId,
      mm,
      payrollRowForPeriod,
  ...
  const niTaxApplicable =
    Boolean(taxSnapshot?.income_tax_deductions_enabled) &&
    String(taxSnapshot?.national_insurance_deductions_file_number ?? '').trim() !== '';
  const niFlowActive = niTaxApplicable && payrollReadyForNi;

  let niDeductionsAutoReminderModal: ClientObligationsTabResponse['ni_deductions_auto_reminder_modal'] = null;
  let niDeductionsManualModal: ClientObligationsTabResponse['ni_deductions_manual_modal'] = null;
  ...
  let incomeTaxDeductionsManualModal: ClientObligationsTabResponse['income_tax_deductions_manual_modal'] = null;
  let incomeTaxDeductionsCtxForTable: {
    requirement: IncomeTaxDeductionsRequirement;
    payrollReady: boolean;
    row: IncomeTaxDeductionsPeriodRow;
  } | null = null;
  if (Boolean(taxSnapshot?.income_tax_deductions_enabled) && String(taxSnapshot?.income_tax_deductions_file_number ?? '').trim()) {
    ...
    if (freqItd) {
      ...
      if (reqItd.is_required) {
        ...
        incomeTaxDeductionsManualModal = buildIncomeTaxDeductionsManualModal(
          reqItd,
          rowItd,
          cellItd,
          payrollReadyForNi
  ...
  const payrollManualModal: ClientObligationsTabResponse['payroll_manual_modal'] = payrollOblForScope
  ...
  const overdueCount = obligations.filter(
    (o) =>
      o.status_code === 'not_reported' ||
      o.status_code === 'reported_late' ||
      o.status_code === 'paid_late' ||
      (o.status_code === 'not_paid' && Boolean(o.due_date) && today > o.due_date)
  ).length;
  const dueTodayCount = obligations.filter(
    (o) =>
      (o.status_code === 'waiting_client_response' ||
  ...
  const dueSoonCount = obligations.filter(
    (o) =>
      (o.status_code === 'waiting_client_response' ||
        ...
        o.status_code === 'not_paid') &&
      o.due_date > today &&
      o.due_date <= soonBorder
  ).length;

  const niRelSpec = await loadNiSelfEmployedRelevantObligation(orgId, clientId, now);
  const niObFromDb = niRelSpec
    ? obligations.find(
        (o) => o.obligation_type === 'national_insurance_self_employed_payment' && o.period_key === niRelSpec.period_key
      ) ?? null
    : null;
  const niPlaceholderObligationId = '00000000-0000-0000-0000-000000000001';
  const niOb: ObligationRowView | null = (() => {
    if (niObFromDb) return niObFromDb;
    if (!niRelSpec) return null;
  ...
  const niAmount =
    Number.isFinite(niAmountNum) && niAmountNum > 0 ? niAmountNum : null;
  ...
  const niAutoReminderModal: ClientObligationsTabResponse['ni_auto_reminder_modal'] = null;

  const niManualModal: ClientObligationsTabResponse['ni_manual_modal'] = niRelevant && niOb
    ? {
  ...
  return {
    ...
    table: buildObligationsTable(
    obligations,
      taxSnapshot,
      profileSnapshot,
      now,
      niDeductionsCtxForTable,
      payrollProcessCtxForTable,
      niRelSpec?.period_key ?? null,
      incomeTaxDeductionsCtxForTable
    ),
    obligations,
    period_summary: (() => {
      const base = obligations.slice(0, 12).map((o) => {
        if (o.obligation_type === 'national_insurance_self_employed_payment') {
          ...
          if (o.status_code === 'paid_late') return { period_key: periodKey, status_label_he: 'שולם עם איחור' };
          if (o.status_code === 'paid_on_time') return { period_key: periodKey, status_label_he: 'שולם במועד' };
        }
        ...
        if (stNi === 'paid_on_time' || stNi === 'paid_late') {
          return [
            ...base,
            {
              ...
              status_label_he:
                stNi === 'paid_on_time' ? 'שולם ודווח במועד' : 'שולם עם איחור',
            },
          ];
        }
...
export async function getClientTasksTabReadModel(ctx: RequestContext, clientId: string): Promise<ClientTasksTabResponse | null> {
  const orgId = assertOrg(ctx);
  await ensureClientInOrg(orgId, clientId);
  ...
  if (error) throw new AppError(500, error.message ?? 'tasks read failed', 'SUPABASE_ERROR');
  const rows = (tasksRows ?? []) as Array<Record<string, unknown>>;
  const clientName = ((client as Record<string, unknown> | null)?.display_name as string | null) ?? null;

  const tasks = rows.map((r) => ({
    task_id: String(r.id),
    title_he: String(r.title_he ?? ''),
    description_he: (r.description_he as string | null) ?? null,
    client_display_name: clientName,
    priority_level: (String(r.priority_level) as PriorityLevel) ?? 'low',
...
function readObligationId(payload: Record<string, unknown>): string {
  const id = String(payload.obligation_id ?? '').trim();
  if (!id) throw badRequest('obligation_id required');
  return id;
}

function readTaskId(payload: Record<string, unknown>): string {
  const id = String(payload.task_id ?? '').trim();
  if (!id) throw badRequest('task_id required');
  return id;
}

function readPeriodKey(payload: Record<string, unknown>): string {
  const p = String(payload.period_key ?? '').trim();
  if (!p) throw badRequest('period_key required');
  return p;
}

function readPayrollPeriodKey(payload: Record<string, unknown>): string {
  const p = String(payload.payroll_period_key ?? payload.period_key ?? '').trim();
  if (!p) throw badRequest('payroll_period_key required');
  return p;
}

export async function executeClientObligationsCommand(
  ctx: RequestContext,
  clientId: string,
  body: ClientObligationsCommandBody
): Promise<void> {
  const orgId = assertOrg(ctx);
  await ensureClientInOrg(orgId, clientId);
  ...
  let obligationAuditExtra: Record<string, unknown> = {};
  ...
  switch (body.type) {
    case 'mark_obligation_reported': {
      ...
      const obRow = (ob ?? null) as Record<string, unknown> | null;
      if (!obRow) throw badRequest('obligation not found');
      const currentState = String(obRow.status ?? '') as ObligationStatus;
      ...
      const nextStatus: ObligationStatus = dueDate && reportDate > dueDate ? 'reported_late' : 'reported';
      await supabaseAdmin
        .from('client_obligations')
    ...
    case 'mark_ni_paid': {
      ...
      if (!niRel) throw badRequest('לא חל ביטוח לאומי לעצמאי ללקוח');
      const paidDayYmd = businessYmd(new Date(nowIso));
      const status: ObligationStatus =
        niRel.due_date && paidDayYmd > niRel.due_date ? 'paid_late' : 'paid_on_time';
    ...
    case 'mark_ni_not_paid': {
      ...
      const status: ObligationStatus =
        niRel.due_date && todayYmd > niRel.due_date ? 'paid_late' : 'not_paid';
      await upsertNiSelfEmployedRowFromRelevant(orgId, clientId, niRel, status, nowIso);
      break;
    }
    ...
    case 'set_ni_deductions_step': {
      ...
      const enabled = payload.enabled !== false;
      if (!['reported_102', 'reported_100', 'paid'].includes(step)) throw badRequest('invalid ni_deductions step');
      const cur = await fetchNiDeductionsPeriodRow(orgId, clientId, periodKey);
      ...
      if (step === 'paid' && enabled) {
        const missingSteps: Array<'sent_to_employer' | 'reported_102' | 'reported_100'> = [];
        if (!payrollSt.sent_to_employer) missingSteps.push('sent_to_employer');
        if (!cur.reported_102) missingSteps.push('reported_102');
        if (!cur.reported_100) missingSteps.push('reported_100');
        if (missingSteps.length) {
          throw new AppError(
            400,
            'נדרשים משכורות וטפסי 100/102 לפני סימון תשלום ניכויים',
            'NI_DEDUCTIONS_NOT_READY_FOR_PAYMENT',
            { missing_steps: missingSteps }
          );
        }
      }
      const next: NiDeductionsPeriodRow = { ...cur };
      if (step === 'reported_102') next.reported_102 = enabled;
      if (step === 'reported_100') next.reported_100 = enabled;
      if (step === 'paid') next.paid = enabled;
    ...
    case 'mark_ni_deductions_payroll_in_progress': {
      ...
      const cur = await fetchNiDeductionsPeriodRow(orgId, clientId, periodKey);
      if (cur.not_relevant) throw badRequest('ni_deductions marked not relevant for period');
      await persistNiDeductionsRow(
        orgId,
        clientId,
        periodKey,
        { ...cur, payroll_in_progress: true },
        nowIso
      );
      break;
    }
    ...
    case 'set_payroll_period_sent_to_employer': {
      ...
      const enabled = payload.enabled !== false;
      const prev = await fetchPayrollPeriodState(orgId, clientId, pk);
      if (enabled && !prev.salary_data_received) throw badRequest('נדרשים נתוני שכר לפני סימון משכורות נשלחו');
      const next: PayrollPeriodStateRow = { ...prev };
      if (enabled) {
        next.not_relevant = false;
    ...
    case 'mark_income_tax_deductions_reported': {
      ...
      if (cur.not_relevant) throw badRequest('סומן לא רלוונטי לתקופה');
      await persistIncomeTaxDeductionsRow(
        orgId,
        clientId,
        periodKey,
        { reported: true, paid: cur.paid, not_relevant: false },
        nowIso
      );
      break;
    }
    case 'mark_income_tax_deductions_paid': {
      ...
      await persistIncomeTaxDeductionsRow(
        ...
        clientId,
        periodKey,
        { reported: true, paid: true, not_relevant: false },
        nowIso
      );
      break;
    }
    case 'mark_income_tax_deductions_not_relevant': {
      ...
      await persistIncomeTaxDeductionsRow(
        ...
        clientId,
        periodKey,
        { reported: false, paid: false, not_relevant: true },
        nowIso
        );
      break;
    }
...
export async function executeClientTasksCommand(ctx: RequestContext, clientId: string, body: ClientTasksCommandBody): Promise<void> {
  const orgId = assertOrg(ctx);
  await ensureClientInOrg(orgId, clientId);
  const payload = (body.payload ?? {}) as Record<string, unknown>;
...
export async function runClientOperationsNightPass(
  ctx: RequestContext,
  asOfDate?: string
): Promise<{ processed_clients: number; organization_id: string; as_of_date: string }> {
  const orgId = assertOrg(ctx);
  const now = asOfDate ? new Date(`${asOfDate}T00:00:00.000Z`) : new Date();
  if (Number.isNaN(now.getTime())) throw badRequest('Invalid as_of_date');
  ...
  if (error) throw new AppError(500, error.message ?? 'clients read failed', 'SUPABASE_ERROR');
