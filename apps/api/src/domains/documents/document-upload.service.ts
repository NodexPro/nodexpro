...
export async function uploadDocument(
  ...
  if (fileSize > MAX_FILE_SIZE) throw badRequest(`File too large. Max ${MAX_FILE_SIZE / 1024 / 1024}MB`);
  if (fileSize === 0) throw badRequest('File is empty');
  ...
  const { data: fileAsset, error: fileError } = await supabaseAdmin
    ...
    .select('id')
    .single();
  if (fileError || !fileAsset) throw new Error('Failed to create file asset');
  ...
  const { data: version, error: versionError } = await supabaseAdmin
    ...
    .select()
    .single();
  if (versionError || !version) throw new Error('Failed to create document version');
...
export async function uploadNewVersion(
  ...
  orgId: string,
  documentId: string,
  body: { file_name: string; mime_type?: string | null; file_size?: number | null; file_base64: string }
) {
  assertOrg(ctx, orgId);
  assertPermission(ctx, 'documents:write');
  ...
  const { data: prevVersions } = await supabaseAdmin
    ...
    .limit(1);
  const nextVersion = prevVersions?.length ? ((prevVersions[0] as { version_number: number }).version_number + 1) : 1;
