...
export async function listLinks(ctx: RequestContext, orgId: string, documentId: string) {
  assertOrg(ctx, orgId);
...
export async function addLink(
  ...
  orgId: string,
  documentId: string,
  body: { target_entity_type: string; target_entity_id: string; relation_type?: string; is_primary?: boolean }
) {
  assertOrg(ctx, orgId);
  ...
  const { data: link } = await supabaseAdmin
    ...
    .select()
    .single();
  if (!link) throw new Error('Failed to add link');
...
export async function removeLink(ctx: RequestContext, orgId: string, documentId: string, linkId: string) {
  assertOrg(ctx, orgId);
  assertPermission(ctx, 'documents:write');
