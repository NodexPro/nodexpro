...
function assertOnlyKeys(obj: Record<string, unknown>, allowed: readonly string[], label: string): void {
  for (const key of Object.keys(obj)) {
    if (!allowed.includes(key)) reject(`${label} has unknown key: ${key}`);
  }
}
...
function assertNoMarkup(value: unknown, path: string): void {
  if (typeof value === 'string') {
    for (const re of FORBIDDEN_STRING_PATTERNS) {
      if (re.test(value)) reject(`${path} must not contain HTML/CSS/script`);
    }
...
function walkRejectMarkup(value: unknown, path: string): void {
...
function parseSection(raw: unknown, index: number): OwnerInvoiceLayoutSection {
  ...
  const key = String(raw.key ?? '') as OwnerInvoiceLayoutSectionKey;
  if (!SECTION_KEYS.has(key)) reject(`sections[${index}].key is invalid`);
  ...
  if (!Number.isInteger(order) || order < 0) reject(`sections[${index}].order invalid`);
  ...
  if (!Number.isFinite(min_height_px) || min_height_px < 0) reject(`sections[${index}].min_height_px invalid`);
  ...
  if (zone !== 'upper' && zone !== 'body' && zone !== 'lower') reject(`sections[${index}].zone invalid`);
  ...
  return {
    key,
    order,
    zone: zone as OwnerInvoiceLayoutSection['zone'],
    col_start,
    col_span,
    min_height_px,
    max_height_px,
    height_px,
    alignment: alignment as OwnerInvoiceLayoutSection['alignment'],
    visible: Boolean(raw.visible !== false),
    owner_locked: Boolean(raw.owner_locked),
  };
}

function parseField(raw: unknown, index: number): OwnerInvoiceLayoutFieldPlacement {
  ...
  if (!isOwnerInvoiceCatalogFieldKey(field_key)) reject(`fields[${index}].field_key unknown: ${field_key}`);
  const section_key = String(raw.section_key ?? '') as OwnerInvoiceLayoutSectionKey;
  if (!SECTION_KEYS.has(section_key)) reject(`fields[${index}].section_key invalid`);
  const order = Number(raw.order);
  const width_span = Number(raw.width_span ?? 12);
  if (!Number.isInteger(order) || order < 0) reject(`fields[${index}].order invalid`);
...
function parseTableColumn(raw: unknown, index: number): OwnerInvoiceLayoutTableColumn {
  assertPlainObject(raw, `table.columns[${index}]`);
  assertOnlyKeys(
    raw,
    ['key', 'order', 'visible', 'width_px', 'align', 'owner_locked'],
    `table.columns[${index}]`,
  );
  const key = String(raw.key ?? '').trim();
  if (!key) reject(`table.columns[${index}].key required`);
  const order = Number(raw.order);
  if (!Number.isInteger(order) || order < 0) reject(`table.columns[${index}].order invalid`);
  ...
  if (!['start', 'center', 'end'].includes(align)) reject(`table.columns[${index}].align invalid`);
  return {
    key,
    order,
    visible: Boolean(raw.visible !== false),
    width_px,
    align: align as OwnerInvoiceLayoutTableColumn['align'],
    owner_locked: Boolean(raw.owner_locked),
  };
}

export function parseAndValidateOwnerInvoiceLayoutDefinition(
  raw: unknown,
): OwnerInvoiceLayoutDefinitionV1 {
  ...
  assertOnlyKeys(
    raw,
    ['schema_version', 'grid', 'sections', 'fields', 'table', 'user_branding_bounds'],
    'layout_definition',
  );
  walkRejectMarkup(raw, 'layout_definition');

  if (Number(raw.schema_version) !== 1) reject('schema_version must be 1');
  ...
  if (Number(raw.grid.columns) !== 12) reject('grid.columns must be 12');
  ...
  if (width_px !== 794 || height_px !== 1123) reject('grid.page must be A4 794×1123');
  if (!Number.isInteger(snap_px) || snap_px < 1 || snap_px > 64) reject('grid.snap_px invalid');

  if (!Array.isArray(raw.sections) || raw.sections.length === 0) reject('sections required');
  if (!Array.isArray(raw.fields) || raw.fields.length === 0) reject('fields required');
  ...
  assertOnlyKeys(
    raw.user_branding_bounds,
    ['logo_size_keys_allowed', 'color_theme_keys_allowed'],
...
export function assertSectionResizeWithinBounds(params: {
  section: OwnerInvoiceLayoutSection;
  height_px?: number;
  col_span?: number;
  col_start?: number;
}): void {
