...
export type OwnerInvoiceBuilderZone = {
  zone_key: string;
  section_key: OwnerInvoiceLayoutSectionKey | null;
  label: string;
  order: number;
  left_px: number;
  top_px: number;
  ...
  accept_field_drop: boolean;
  resize_allowed: boolean;
  reorder_allowed: boolean;
  lock_toggle_allowed: boolean;
...
export type OwnerInvoiceBuilderChrome = {
  mode: 'visual_editor';
  page: { width_px: number; height_px: number };
  grid: { columns: 12; snap_px: number; show_columns: true; show_rulers: true; show_guides: true };
...
function zoneActions(params: {
  ...
  if (!params.is_logo) {
    ...
    if (params.resize_allowed) actions.push(OWNER_INVOICE_LAYOUT_COMMANDS.resize_section);
...
export function buildOwnerInvoiceBuilderZones(params: {
  ...
  editable: boolean;
  sample_logo_present: boolean;
  /** INV-13C — labels from template registry / section_model (not FE-hardcoded). */
  section_labels?: Record<string, string>;
}): { chrome: OwnerInvoiceBuilderChrome; zones: OwnerInvoiceBuilderZone[] } {
  ...
  // Pack upper-zone sections that share the same vertical band (side-by-side).
  type Band = { top: number; height: number; sections: typeof sections };
  const bands: Band[] = [];
  let cursorY = 0;
  let i = 0;
  ...
  const zones: OwnerInvoiceBuilderZone[] = [];
