 ...
 */
export const LEGACY_ORG_ROLE_TO_RBAC_ROLE: Record<string, string> = {
  owner: 'owner',
  admin: 'admin',
  member: 'admin',
  staff: 'staff',
...
function expandPermissionCodesWithRbacLegacy(codes: string[]): string[] {
  const out = new Set<string>();
  for (const code of codes) {
    out.add(code);
    for (const leg of RBAC_TO_LEGACY[code] ?? []) out.add(leg);
  }
  return [...out];
}
 ...
 */
export async function mergeLegacyOrganizationUserPermissions(
  legacyRoleCode: string,
  directPermissionCodesFromRolePermissions: string[]
): Promise<string[]> {
  ...
  const rbacCodes = (rows ?? []).map((r) => (r as { permission_code: string }).permission_code);
 ...
 */
export async function loadMembershipWithPermissions(
  userId: string,
  organizationId: string
  ...
  const roleCode = (m as { role_code: string }).role_code;
  ...
  const rbacCodes = (perms ?? []).map((p) => (p as { permission_code: string }).permission_code);
  const legacyCodes = new Set<string>();
  for (const code of rbacCodes) {
    for (const leg of RBAC_TO_LEGACY[code] ?? []) legacyCodes.add(leg);
  }
 ...
 */
export function requireRbacPermission(ctx: RequestContext, orgId: string, permissionCode: string): void {
  if (ctx.organizationId !== orgId || !ctx.membership) throw forbidden('Organization context required');
  if (!hasPermission(ctx.membership.permissions ?? [], permissionCode)) throw forbidden('Insufficient permission');
}
