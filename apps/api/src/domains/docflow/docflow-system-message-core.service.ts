...
export async function assertModuleKeyValid(moduleKey: string): Promise<void> {
  const { data, error } = await supabaseAdmin.from('modules').select('id').eq('code', moduleKey).maybeSingle();
  if (error) throw error;
  if (!data) throw badRequest('Invalid module_key');
}
...
export async function resolveSystemThread(args: {
  ...
  if (args.ruleContextKey) {
    ...
    if (existingThreadId) {
      ...
      if (thread && thread.thread_status !== 'archived') return thread.id;
    }
  }
  ...
  if (openThread?.id) return openThread.id;
  ...
  const { data: created, error: createErr } = await supabaseAdmin
    ...
    .select('id')
    .single();
  if (createErr || !created) throw createErr ?? new Error('Failed to create system thread');
  return created.id;
}
...
type CreateSystemMessageCoreParams = {
  orgId: string;
  clientId: string;
  moduleKey: string;
  ...
  ruleCode: string;
  ruleContextKey: string | null;
  sendModeRaw: 'draft_review_required' | 'auto_send_allowed';
  autoSendAllowedByRule: boolean;
  /** When true, allows published message without auto_send_allowed_by_rule (approved communication draft path). */
  allowPublishWithoutAutoSendRule: boolean;
  /** Controls whether "auto sent" audit/events are emitted for published message creation. */
  emitAutoSentEvent?: boolean;
 ...
 */
export async function createSystemMessageCore(
  params: CreateSystemMessageCoreParams
): Promise<{ threadId: string; messageId: string; reusedExisting: boolean }> {
  const {
    orgId,
    clientId,
    moduleKey,
    ...
    allowPublishWithoutAutoSendRule,
  ...
  const { data: message, error: msgErr } = await supabaseAdmin
    ...
    .select('id')
    .single();
  if (msgErr || !message) throw msgErr ?? new Error('Failed to create system message');
