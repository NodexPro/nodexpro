...
export async function getPortalDocflowAttachmentSignedUrl(params: {
  ...
  if (!row) throw forbidden('Attachment not accessible for this portal session');
  ...
  if (!fileAsset || fileAsset.organization_id !== params.orgId) throw forbidden('File not found');

  const bucket = String(fileAsset.storage_bucket ?? BUCKET_CLIENT_FILES);
  if (bucket === BUCKET_CLIENT_FILES) await ensureBucketExists();
...
export async function getOfficeDocflowAttachmentSignedUrl(params: {
  ...
  const { data: row, error } = await supabaseAdmin
    ...
    .maybeSingle();
  if (error) throw error;
  if (!row) throw forbidden('Attachment not accessible for this client context');
