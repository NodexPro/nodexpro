...
import {
  emitDocflowThreadNeedsAttentionWithIntakeResult,
} from './docflow-work-engine-bridge.js';
...
export type DocflowWorkEngineBackfillResult = {
  scanned: number;
  eligible: number;
  emitted: number;
  skipped: number;
...
async function threadIdsWithUnreadClientReply(orgId: string, threadIds: string[]): Promise<Set<string>> {
  ...
  const lastReadMs = new Map<string, number>();
  for (const e of events ?? []) {
    const tid = String((e as { thread_id: string }).thread_id);
    if (!lastReadMs.has(tid)) lastReadMs.set(tid, new Date(String((e as { created_at: string }).created_at)).getTime());
  }
  const { data: msgs, error: mErr } = await supabaseAdmin
    ...
    .in('thread_id', threadIds);
  if (mErr) throw mErr;
  const maxMsgMs = new Map<string, number>();
  for (const m of msgs ?? []) {
    const tid = String((m as { thread_id: string }).thread_id);
    const ts = new Date(String((m as { created_at: string }).created_at)).getTime();
    const cur = maxMsgMs.get(tid) ?? 0;
    if (ts > cur) maxMsgMs.set(tid, ts);
  }
  const out = new Set<string>();
  for (const tid of threadIds) {
    const msgMax = maxMsgMs.get(tid);
    if (msgMax === undefined) continue;
    const readTs = lastReadMs.get(tid) ?? Number.NEGATIVE_INFINITY;
    if (msgMax > readTs) out.add(tid);
  }
  return out;
}

function mapThreadRow(raw: Record<string, unknown>): ThreadRow | null {
  const id = String(raw.id ?? '').trim();
  const org_id = String(raw.org_id ?? '').trim();
  const client_id = String(raw.client_id ?? '').trim();
  if (!id || !org_id || !client_id) return null;
  return {
    id,
    org_id,
    client_id,
...
async function filterThreadsClientInOrg(orgId: string, rows: Record<string, unknown>[]): Promise<ThreadRow[]> {
  const clientIds = [...new Set(rows.map((r) => String((r as { client_id?: string }).client_id ?? '').trim()).filter(Boolean))];
  ...
  const { data: clients, error: cErr } = await supabaseAdmin
    ...
    .in('id', clientIds);
  if (cErr) throw cErr;
  const allowed = new Set((clients ?? []).map((c) => String((c as { id: string }).id)));
  const out: ThreadRow[] = [];
  for (const r of rows) {
    const m = mapThreadRow(r);
    if (!m || !allowed.has(m.client_id)) continue;
    out.push(m);
  }
  return out;
}
 ...
 */
export async function fetchEligibleDocflowThreadsForWorkEngineBackfill(
  orgId: string,
  limit: number,
): Promise<{ threads: ThreadRow[]; scanned: number }> {
  ...
  const activeThreads = await filterThreadsClientInOrg(orgId, (activeRaw ?? []) as Record<string, unknown>[]);

  const resolvedEligible: ThreadRow[] = [];
  const page = 200;
  let offset = 0;
  ...
  while (resolvedEligible.length < cap && offset < maxResolvedScan) {
    ...
    const inOrg = await filterThreadsClientInOrg(orgId, batch as Record<string, unknown>[]);
    const ids = inOrg.map((t) => t.id);
    const unread = await threadIdsWithUnreadClientReply(orgId, ids);
    for (const row of inOrg) {
      if (!unread.has(row.id)) continue;
      resolvedEligible.push(row);
      if (resolvedEligible.length >= cap) break;
    }
    if (resolvedEligible.length >= cap) break;
  ...
  const pushUnique = (t: ThreadRow) => {
    if (seen.has(t.id)) return;
    seen.add(t.id);
    merged.push(t);
  };
  for (const t of activeThreads) pushUnique(t);
  for (const t of resolvedEligible) pushUnique(t);
  merged.sort((a, b) => {
...
export async function buildBackfillRequestContext(actorUserId: string, orgId: string): Promise<RequestContext> {
  const { data: userRow, error: uErr } = await supabaseAdmin
    ...
    .maybeSingle();
  if (uErr) throw uErr;
  if (!userRow) throw new AppError(400, 'BACKFILL_ACTOR_USER_ID not found in users', 'BACKFILL_ACTOR_INVALID');
  ...
  return {
    user: {
      ...
      authUserId: String((userRow as { auth_user_id: string }).auth_user_id),
...
export async function runDocflowWorkEngineBackfill(opts: {
...
}): Promise<DocflowWorkEngineBackfillResult & { details?: Array<{ threadId: string; outcome: string }> }> {
  ...
  let skipped = 0;
  let errors = 0;
  const details: Array<{ threadId: string; outcome: string }> = [];
  ...
  for (const t of threads) {
    ...
    const intake: IntakeWorkEventMeta = r.intake;
