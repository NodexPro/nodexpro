...
function expandCorsOriginsWithWwwVariants(origins: string[]): string[] {
  ...
  for (const o of origins) {
    try {
      ...
      if (host === 'localhost' || host.startsWith('localhost.') || host === '127.0.0.1') continue;
...
const corsAllowedOrigins = expandCorsOriginsWithWwwVariants(
...
const corsAllowCredentials =
  String(process.env.CORS_ALLOW_CREDENTIALS ?? '')
    .trim()
    .toLowerCase() === 'true';
...
const isLocalDevOrigin = (origin: string) =>
  localDevOriginAllowlist.includes(origin) || origin.startsWith('http://localhost:') || origin.startsWith('http://127.0.0.1:');
const isAllowedOrigin = (origin: string) => (corsAllowedOrigins.length ? corsAllowedOrigins.includes(origin) : isLocalDevOrigin(origin));
