export function inferCardBrand(pan) {
    ...
    if (!Number.isNaN(p4) && p4 >= 3528 && p4 <= 3589)
        return 'jcb';
    ...
    if (/^(6360|6361|6388|6390|5075|5866)/.test(d))
        return 'isracard';
    // Visa
    if (/^4/.test(d))
        return 'visa';
    ...
    if (!Number.isNaN(f4b) && f4b >= 2221 && f4b <= 2720)
        return 'mastercard';
    const f6 = parseInt(d.slice(0, 6), 10);
    if (!Number.isNaN(f6) && f6 >= 222100 && f6 <= 272099)
        return 'mastercard';
    return 'unknown';
}
