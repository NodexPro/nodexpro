 ...
 */
export function maskNormalizedTz(normalized) {
    if (normalized.length < MASK_LAST_DIGITS)
        return '***';
 ...
 */
export function normalizeLegalIdentity(countryCode, legalIdentityType, value) {
 ...
 */
export function hashLegalIdentity(normalizedValue, legalIdentityType) {
 ...
 */
export async function isTrialAlreadyUsed(legalIdentityHash) {
    const { data, error } = await supabaseAdmin
        ...
        .maybeSingle();
    if (error)
        throw new Error('Failed to check trial reuse');
    return !!data;
}
/**
 * Returns true if org has a locked legal identity (cannot be changed by normal users).
 */
export async function hasLockedLegalIdentity(organizationId) {
 ...
 */
export function normalizeAndHash(countryCode, legalIdentityType, value) {
 ...
 */
export async function setLegalIdentity(organizationId, countryCode, legalIdentityType, value) {
 ...
 */
export async function lockAndSetMasked(organizationId, maskedValue) {
    await supabaseAdmin
        .from('organization_legal_identities')
        .update({
        is_locked: true,
 ...
 */
export async function getOwnerIdentityForSettings(organizationId) {
    const { data } = await supabaseAdmin
        .from('organization_legal_identities')
 ...
 */
export async function hasLegalIdentity(organizationId) {
    const { data } = await supabaseAdmin
        .from('organization_legal_identities')
        .select('id')
        .eq('organization_id', organizationId)
