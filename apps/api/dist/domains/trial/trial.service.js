 ...
 */
export async function hasExpiredTrial(organizationId) {
 ...
 */
export async function hasValidTrial(organizationId) {
    const now = new Date().toISOString();
    const { data } = await supabaseAdmin
        .from('organization_trials')
        .select('id')
 ...
 */
export async function getTrialState(organizationId) {
    ...
    const now = new Date();
    let trialStatus = !trial ? 'none' : trial.status;
    let endsAt = trial?.ends_at ?? null;
 ...
 */
export async function startTrial(ctx, organizationId, legalIdentityHash) {
