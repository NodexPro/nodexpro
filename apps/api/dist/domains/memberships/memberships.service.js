...
export async function listMembers(ctx, orgId) {
    if (ctx.organizationId !== orgId)
        throw forbidden('Organization context required');
    if (!ctx.membership?.permissions?.includes('members:read'))
...
export async function addMember(ctx, orgId, params) {
    if (ctx.organizationId !== orgId)
        throw forbidden('Organization context required');
    if (!ctx.membership?.permissions?.includes('members:write'))
        throw forbidden('Insufficient permission');
    ...
    if (existing)
        throw badRequest('User already in organization');
...
export async function updateMember(ctx, orgId, memberId, params) {
    if (ctx.organizationId !== orgId)
        throw forbidden('Organization context required');
    if (!ctx.membership?.permissions?.includes('members:write'))
        throw forbidden('Insufficient permission');
...
export async function removeMember(ctx, orgId, memberId) {
    if (ctx.organizationId !== orgId)
        throw forbidden('Organization context required');
    if (!ctx.membership?.permissions?.includes('members:revoke'))
        throw forbidden('Only owner can revoke access');
    const { data: org } = await supabaseAdmin.from('organizations').select('owner_user_id').eq('id', orgId).single();
    if (!org || org.owner_user_id !== ctx.user.id) {
