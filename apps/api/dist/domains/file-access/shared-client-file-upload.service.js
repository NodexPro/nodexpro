...
export function assertDocflowClientFileUploadAllowed(fileName, mimeType) {
    ...
    if (m === 'application/pdf')
        return;
    if (m.startsWith('application/vnd.openxmlformats'))
        return;
    ...
    if (m === 'application/rtf')
        return;
    if (m.startsWith('text/plain') || m === 'text/csv')
        return;
...
export function assertDocflowClientFileBase64WithinLimit(file_base64) {
    if (typeof file_base64 !== 'string' || !file_base64.length)
 ...
 */
export async function uploadSharedClientFileAssetForOffice(ctx, params) {
    ...
    if (typeof b64 !== 'string' || !b64.length)
        throw badRequest('file_base64 is required');
    if (b64.length > MAX_CLIENT_FILE_BASE64_LENGTH)
        throw badRequest(`File too large. Max ${MAX_CLIENT_FILE_SIZE / 1024 / 1024}MB`);
