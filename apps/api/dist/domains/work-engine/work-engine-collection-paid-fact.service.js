...
import { INCOME_WORK_EVENT_INVOICE_PAID, INVOICE_COLLECTION_FOLLOWUP_WORK_TYPE, shouldAutoCloseCollectionFollowup, } from './work-engine-collection-reminder.pure.js';
...
export async function consumeIncomeInvoicePaidFact(params) {
    ...
    const paymentStateKey = params.eventType === INCOME_WORK_EVENT_INVOICE_PAID
        ? 'paid'
        : Number.isFinite(remaining) && remaining <= 0
            ? 'paid'
            : 'partial';
    if (!shouldAutoCloseCollectionFollowup({
        paymentStateKey,
