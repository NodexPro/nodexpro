...
import { beginAttempt, finalizeAttempt, listAttempts, sendEmail, } from '../delivery/index.js';
...
import { buildCollectionReminderSendIdempotencyKey, COLLECTION_REMINDER_DELIVERY_ENTITY_TYPE, COLLECTION_REMINDER_DELIVERY_SOURCE_MODULE, composeCollectionReminderSendMessage, INVOICE_COLLECTION_FOLLOWUP_WORK_TYPE, resolveCollectionReminderSendGate, } from './work-engine-collection-reminder.pure.js';
import { loadReminderCandidate, } from './work-engine.reminder-review.service.js';
function parsePrimaryChannel(candidate) {
    const primary = String(candidate.channel ?? '')
        .trim()
        .toLowerCase()
        .split(':')[0];
    if (primary === 'email' || primary === 'docflow')
        return primary;
    ...
    for (const item of order) {
        const key = String(item ?? '')
            ...
            .toLowerCase()
            .split(':')[0];
        if (key === 'email' || key === 'docflow')
            return key;
    }
    return 'docflow';
}
async function countPriorDeliveryAttempts(orgId, candidateId) {
...
async function markCandidateSending(params) {
    if (params.candidate.status === 'sending')
...
export async function sendCollectionReminder(params) {
    ...
    const { data: doc, error: dErr } = await supabaseAdmin
        ...
        .select('id, represented_client_id, document_number, document_type, document_status, due_date, currency, totals_snapshot_json, customer_snapshot_json')
    ...
    const clientId = (doc.represented_client_id ? String(doc.represented_client_id) : null) ??
    ...
    if (channel === 'email' && clientId) {
        ...
        if (!recipientEmail) {
            const snap = (doc.customer_snapshot_json ?? {});
    ...
    if (!gate.allowed) {
        throw badRequest(gate.disabled_reason ?? 'Collection reminder cannot be sent', gate.reason_key ?? 'collection_reminder_not_sendable');
    }
    ...
    if (!clientId)
        throw badRequest('Collection reminder has no client target', 'missing_recipient');
