...
import { computeDueDateFromPaymentTerms, incomeCustomerPaymentTermsLabel, isIncomeCustomerPaymentTermsKey, } from '../income/income-customer-payment-terms.pure.js';
...
function applyPriceIncreaseToLinesForCycleIndex(step, profile, cycleIndex) {
...
function resolveProjectionSettings(step, snapshot) {
    const fromStep = parseDocumentSettingsJson({
        ...(snapshot?.document_settings_json ?? {}),
        vat_mode: step.line_items.document_fields?.vat_mode?.value ??
            snapshot?.document_settings_json?.vat_mode ??
            'standard',
...
async function rebuildProjectedLineTotals(step, orgId, documentDate, settings, vatResolution) {
    ...
    const rows = step.line_items.rows.map((row, index) => {
        ...
        const line = (lineKey ? lines.find((item) => item.line_id === lineKey) : null) ?? lines[index] ?? null;
        if (!line)
            return row;
...
function resolveProjectionPaymentTermsDisplay(step) {
...
function resolveProjectionAllocationNumberField(step) {
...
function applyCycleDocumentDate(step, cycleDate) {
...
function applyOverridePayloadToStep(step, override) {
    if (!override)
        return step;
    const settings = parseDocumentSettingsJson(override.document_settings_json);
    const settingsSchema = step.settings_schema.map((field) => {
        const value = override.document_settings_json[field.key];
        if (value == null)
            return field;
...
export async function buildFutureCycleProjectionStep(params) {
    ...
    const effectiveSnapshot = snapshot && params.overridePayload
        ? mergeOverridePayloadIntoTemplateSnapshot(snapshot, params.overridePayload)
        : snapshot;
    const projectedDocumentType = params.baseStep.document_type_key === 'quote' ||
...
export async function attachFutureCycleProjectionPreview(step, preview) {
