...
import { COLLECTION_REMINDER_APPROVE_COMMAND, COLLECTION_REMINDER_DELIVERY_ENTITY_TYPE, COLLECTION_REMINDER_DELIVERY_SOURCE_MODULE, COLLECTION_REMINDER_REVIEW_AGGREGATE_KEY, COLLECTION_REMINDER_SEND_COMMAND, composeCollectionReminderInvoiceTruth, INVOICE_COLLECTION_FOLLOWUP_WORK_TYPE, isCollectionReminderMutatableStatus, resolveCollectionReminderApproveGate, resolveCollectionReminderSendGate, } from './work-engine-collection-reminder.pure.js';
import { loadReminderCandidate, REMINDER_SNOOZE_PRESETS, } from './work-engine.reminder-review.service.js';
function canWrite(viewer) {
    if (!viewer)
        return false;
    return (hasPermission([...viewer.permissions], WORK_ENGINE_PERMISSIONS.write) ||
        hasPermission([...viewer.permissions], WORK_ENGINE_PERMISSIONS.admin));
}
function parseChannels(snapshot, primary) {
    const out = [];
    const seen = new Set();
...
export async function buildCollectionReminderReviewAggregate(params) {
    ...
    if (String(workItem.work_type) !== INVOICE_COLLECTION_FOLLOWUP_WORK_TYPE) {
        throw badRequest('Candidate is not linked to an invoice collection work item', 'not_collection_reminder_candidate');
    }
    ...
    const { data: doc, error: dErr } = await supabaseAdmin
        ...
        .select('id, organization_id, represented_client_id, document_number, document_type, document_status, issue_date, due_date, currency, totals_snapshot_json')
    ...
    const truth = composeCollectionReminderInvoiceTruth({
        ...
        documentDate: doc.issue_date ?? null,
    ...
    const hasRecipient = primaryChannel === 'email' ? !!clientEmail : !!representedClientId;
    ...
    const slaSnap = (candidate.sla_context_snapshot ?? {});
    const dueAt = candidate.suggested_send_at ??
        (slaSnap.due_at ? String(slaSnap.due_at) : null);
    return {
        aggregate_key: COLLECTION_REMINDER_REVIEW_AGGREGATE_KEY,
        ui: {
            title: 'בדיקת תזכורת גבייה',
            subtitle: candidate.status === 'approved' || candidate.status === 'delivery_failed'
                ? 'מאושר — ניתן לשלוח'
...
export async function assertCollectionReminderApprovable(params) {
    ...
    const { data: doc, error: dErr } = await supabaseAdmin
        .from('income_documents')
        .select('id, represented_client_id, document_status, document_type, due_date, totals_snapshot_json')
        .eq('organization_id', params.orgId)
        .eq('id', incomeDocumentId)
        .maybeSingle();
    if (dErr)
    ...
    const channels = parseChannels(params.candidate.channel_order_snapshot, params.candidate.channel);
    ...
    if (!gate.allowed) {
        throw badRequest(gate.disabled_reason ?? 'Collection reminder cannot be approved', gate.reason_key ?? 'collection_reminder_not_approvable');
    }
}
