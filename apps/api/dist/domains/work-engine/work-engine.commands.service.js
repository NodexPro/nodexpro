...
function assertActorMayRequestReview(ctx, current) {
...
function assertActorMayApproveOrReject(ctx, current) {
    const perms = ctx.membership?.permissions ?? [];
    const isReviewer = current.reviewer_user_id === ctx.user.id;
    const canBreakGlass = hasPermission(perms, WORK_ENGINE_PERMISSIONS.reviewBreakGlass) ||
    ...
    switch (command) {
            ...
            return executeWithCommandIdempotency(ctx, orgId, command, payload, async () => {
                ...
                if (isQueueRefreshMode(payload)) {
                    if (toState === 'archived')
                        ...
                        if (quick?.enabled)
                            assertQueueActionEnabled(current, 'mark_waiting_client');
                        else
        ...
        case 'transfer_work_item': {
            return executeWithCommandIdempotency(ctx, orgId, command, payload, async () => {
                ...
                assertExpectedVersion(current.version, expectedVersion);
                if (isQueueRefreshMode(payload))
                    assertQueueActionEnabled(current, 'transfer');
        ...
        case 'request_review': {
            return executeWithCommandIdempotency(ctx, orgId, command, payload, async () => {
                ...
                if (policy.review_gate === 'none') {
                    throw badRequest('Review workflow is not enabled for this work type', 'REVIEW_NOT_APPLICABLE');
                }
                if (current.work_state === 'review_pending') {
                    throw conflict('Work item is already in review', 'WORK_ITEM_ALREADY_IN_REVIEW');
                }
                if (current.work_state !== 'assigned') {
                    throw badRequest(`request_review requires work_state=assigned (current='${current.work_state}')`, 'INVALID_TRANSITION');
                }
                if (!current.assigned_user_id) {
                    throw badRequest('Work item has no assignee', 'INVALID_TRANSITION');
                }
                if (!current.reviewer_user_id) {
                    throw badRequest('Designated reviewer is required before requesting review', 'REVIEWER_REQUIRED');
                }
        ...
        case 'approve_work_item': {
            return executeWithCommandIdempotency(ctx, orgId, command, payload, async () => {
                ...
                if (current.work_state !== 'review_pending') {
                    throw badRequest(`approve_work_item requires work_state=review_pending (current='${current.work_state}')`, 'INVALID_TRANSITION');
                }
                if (!current.reviewer_user_id || !current.assigned_user_id) {
        ...
        case 'reject_work_item': {
            return executeWithCommandIdempotency(ctx, orgId, command, payload, async () => {
                ...
                if (current.work_state !== 'review_pending') {
                    throw badRequest(`reject_work_item requires work_state=review_pending (current='${current.work_state}')`, 'INVALID_TRANSITION');
                }
                if (!current.reviewer_user_id || !current.assigned_user_id) {
                    throw badRequest('Invalid review row (missing reviewer or assignee)', 'INVALID_TRANSITION');
                }
