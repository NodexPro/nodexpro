...
import { buildScheduleRowWorkItemHref, } from './work-engine-invoice-retainer-schedule-row-status.pure.js';
...
function workTypeTitleHe(workType) {
    return WORK_TYPE_LABELS_HE[workType] ?? workType;
}
export function resolveScheduleRowMachineTone(workState) {
    if (workState === 'new' || workState === 'assigned')
        return 'primary';
    if (workState === 'approved')
        return 'success';
    if (workState === 'waiting_human' ||
    ...
    if (workState === 'done' || workState === 'archived')
        return 'muted';
    return 'neutral';
}
export function isActiveMachineWorkItem(workItem) {
    if (!workItem)
        return false;
...
export function resolveScheduleRowMachineState(params) {
    ...
    const waitingReviewDraft = params.waitingReviewWithGeneratedDraft === true &&
