...
export async function scanCollectionReminderCandidatesForOrg(params) {
    ...
    for (let page = 0; page < maxPages; page += 1) {
        ...
        const { data, error } = await supabaseAdmin
            .from('work_items')
            .select('id, org_id, client_id, work_type, work_state, source_entity_id, source_entity_type')
        ...
        const documentIds = Array.from(new Set(rows
        ...
        if (documentIds.length > 0) {
            const { data: docs, error: docErr } = await supabaseAdmin
                ...
                .select('id, document_status, document_type, due_date, totals_snapshot_json, represented_client_id')
        ...
        for (const item of rows) {
            ...
            if (!shouldCreateCollectionReminderCandidate({
