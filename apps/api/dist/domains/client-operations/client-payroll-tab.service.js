...
async function ensurePayrollProfile(orgId, clientId) {
    ...
    if (error)
        throw new AppError(500, error.message ?? 'payroll profile insert failed', 'SUPABASE_ERROR');
}
function boolHe(v) {
    if (v == null)
        return '—';
...
function formatNiWith00(baseIncomeDeductionsFile) {
...
function pickOverride(overrideValue, baseValue) {
...
export async function getPayrollTabReadModel(ctx, clientId) {
    const orgId = assertOrg(ctx);
    if (!canViewPayrollTab(ctx))
        return null;
    ...
    const foundedYear = c.created_at != null && String(c.created_at)
        ...
        sections: [
            ...
            },
            {
                ...
                lines: [
                    ...
                    { label_he: 'דיווח טופס 102', value_he: boolHe(r.form_102_reported) },
                    { label_he: 'דיווח טופס 100', value_he: boolHe(r.form_100_reported) },
            ...
            },
            {
                ...
                lines: [
                    ...
                    { label_he: 'דורש תיקונים', value_he: boolHe(r.complexity_requires_fixes) },
                    {
                        label_he: 'יש עובדים עם הסדרים מיוחדים',
                        value_he: SPECIAL_ARRANGEMENTS_OPTIONS.find((o) => o.value === String(r.complexity_special_arrangements ?? ''))?.label_he ?? '—',
                    },
...
export async function executePayrollTabCommand(ctx, clientId, body) {
    const orgId = assertOrg(ctx);
    if (!canEditPayrollTab(ctx))
    ...
    const { data: updated, error } = await supabaseAdmin
        ...
        .select('id');
    if (error)
        throw new AppError(500, error.message ?? 'payroll command failed', 'SUPABASE_ERROR');
    if (!updated?.length)
