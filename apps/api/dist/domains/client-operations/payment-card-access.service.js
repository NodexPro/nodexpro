...
async function getOrganizationPhone(orgId) {
...
async function deactivateExpiredSessions(orgId, clientId, userId) {
...
export async function getPaymentSecureSessionsForUser(orgId, userId, clientId) {
    ...
    const now = Date.now();
    const pick = (ctx) => {
        const r = rows?.find((x) => x.payment_context === ctx);
        if (!r)
            return { active: false, expires_at: null };
        const exp = new Date(r.expires_at).getTime();
        if (exp <= now)
            return { active: false, expires_at: null };
        return { active: true, expires_at: r.expires_at };
    };
    return {
        vat: pick('vat'),
...
export async function assertPaymentCardSecureSessionActive(ctx, clientId, paymentContext) {
    ...
    if (s.active)
        return;
    throw new AppError(403, 'נדרש אימות טלפוני (קוד לטלפון הארגון) כדי להעתיק את פרטי הכרטיס.', 'SECURE_SESSION_REQUIRED');
}
export async function requestPaymentCardAccessCode(ctx, clientId, paymentContext) {
    const orgId = assertOrg(ctx);
    const userId = ctx.user.id;
    ...
    if (!phone) {
        throw badRequest('לא מוגדר מספר טלפון בארגון. הוסף טלפון בהגדרות הארגון כדי לקבל קוד אימות.', 'ORG_PHONE_MISSING');
    }
...
export async function verifyPaymentCardAccessCode(ctx, clientId, challengeId, code, paymentContext) {
    const orgId = assertOrg(ctx);
    const userId = ctx.user.id;
