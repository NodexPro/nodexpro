 ...
 */
const BI_MONTHLY_REPORT_MONTHS = [0, 2, 4, 6, 8, 10];
function dateOnlyLocal(d) {
    return new Date(d.getFullYear(), d.getMonth(), d.getDate());
}
function formatDateHeDots(d) {
...
function nextMonthlyDue(dayOfMonth, from) {
...
function nextBiMonthlyDue(dayOfMonth, from) {
    const today = dateOnlyLocal(from);
    for (let i = 0; i < 36; i++) {
        const probe = new Date(today.getFullYear(), today.getMonth() + i, 1);
        const y = probe.getFullYear();
        const m = probe.getMonth();
        if (!BI_MONTHLY_REPORT_MONTHS.includes(m))
            continue;
        const lastDay = new Date(y, m + 1, 0).getDate();
        const dom = Math.min(dayOfMonth, lastDay);
        const candidate = new Date(y, m, dom);
...
export function computeNextVatDivuachDueDate(params) {
    const { vat_due_type, vat_frequency } = params;
    const now = params.now ?? new Date();
    if (!vat_due_type || vat_due_type === 'not_relevant')
        return null;
    if (!vat_frequency || vat_frequency === 'not_relevant')
        return null;
    if (vat_due_type !== 'pcn' && vat_due_type !== 'regular')
        return null;
...
export function formatVatDivuachDisplayHe(d) {
...
export function mapVatTypeToProfileDisplayHe(vatType) {
    if (vatType == null || vatType === '')
        return null;
    switch (vatType) {
        case 'yes':
            return 'כן';
 ...
 */
export function computeVatRegistryColumnDisplayHe(businessType, vatType, vatFrequency) {
    if ((businessType ?? '').trim() === OSEK_PATUR_BUSINESS_TYPE_HE)
        return 'פטור';
    if (vatFrequency === 'monthly')
        return 'חודשי';
    if (vatFrequency === 'bi_monthly')
        return 'דו-חודשי';
...
export function computeVatDueRegistryDisplayHe(vat_due_type, vat_frequency, now = new Date()) {
    if (!vat_due_type || vat_due_type === 'not_relevant')
        return null;
    if (!vat_frequency || vat_frequency === 'not_relevant')
        return null;
