...
function heVatFrequency(v, frequencyDisabled) {
    if (frequencyDisabled && !v)
        return '—';
    switch (v) {
        case 'monthly':
            return 'חודשי';
...
function isMaskedExpiryDisplayInPast(expiryRaw) {
...
function buildVatRegistrationEditModel(s, ui) {
...
function buildIncomeTaxAdvancesEditModel(s) {
    const uiSel = s.income_tax_advance_ui_selection ?? (s.income_tax_advance_enabled ? 'yes' : 'choose');
    return {
        fields: [
            {
...
function buildIncomeTaxDeductionsEditModel(s) {
    const uiSel = s.income_tax_deductions_ui_selection ?? (s.income_tax_deductions_enabled ? 'yes' : 'choose');
    return {
        fields: [
            {
...
function buildNationalInsuranceEditModel(s) {
...
function buildVatPaymentEditModel(s) {
    return {
        fields: [
            {
...
function buildIncomeTaxPaymentEditModel(s) {
    return {
        fields: [
            {
...
function buildNotesEditModel(s) {
    return {
        fields: [
            {
                field_key: 'notes',
...
function buildCreditPaymentPanel(s, channel) {
    ...
    const sess = channel === 'vat' ? s.payment_secure_sessions.vat : s.payment_secure_sessions.income_tax;
    if (method !== 'credit' || !masked.last4)
        return null;
...
export function buildTaxTabWorkspaceReadModel(bundle) {
