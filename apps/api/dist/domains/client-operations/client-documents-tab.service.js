...
export const CLIENT_DOCUMENT_DEFAULT_FOLDERS = [
...
async function ensureDocumentProfile(orgId, clientId) {
    const { data: existing } = await supabaseAdmin
        ...
        .maybeSingle();
    if (existing) {
        return { read_model_version: Number(existing.read_model_version ?? 1) };
    }
    const { data: ins, error } = await supabaseAdmin
        ...
        .single();
    if (error || !ins)
        throw new AppError(500, error?.message ?? 'client_document_profiles insert failed', 'SUPABASE_ERROR');
    return { read_model_version: Number(ins.read_model_version ?? 1) };
}
/** Idempotent: insert missing system folders only (no duplicates). One round-trip via upsert. */
export async function ensureClientDocumentFolders(orgId, clientId) {
    ...
    if (error)
        throw new AppError(500, error.message ?? 'default folder upsert failed', 'SUPABASE_ERROR');
}
async function assertExpectedVersion(orgId, clientId, expected) {
...
async function bumpReadModelVersion(orgId, clientId, expected, userId) {
...
async function auditDocuments(ctx, orgId, clientId, command, payload) {
    await writeAudit({
...
function brandVariantForIndex(i) {
    const m = i % 3;
    if (m === 0)
        return 'primary';
    if (m === 1)
...
export async function getClientDocumentsTabReadModel(ctx, clientId, options) {
    const orgId = assertOrg(ctx);
    if (!canViewDocumentsTab(ctx))
        return null;
    ...
    const { data: prof } = await supabaseAdmin
        ...
        .eq('client_id', clientId)
        .single();
    const read_model_version = Number(prof?.read_model_version ?? 1);
    let openFolderId = prof?.open_folder_id ?? null;
    ...
    for (const fid of folderIds)
        counts.set(fid, { n: 0, lastDoc: null });
    if (folderIds.length > 0) {
        ...
        for (const r of (docAgg ?? [])) {
            ...
            if (!cur.lastDoc || ts > new Date(cur.lastDoc).getTime())
                cur.lastDoc = r.updated_at;
        }
    }
    const folderCards = folders.map((f, idx) => {
...
export async function assertClientDocumentFileOpenAllowed(orgId, clientId, fileAssetId) {
    ...
    if (!String(fa.storage_key).startsWith(prefix))
        throw forbidden('File not linked to this client documents workspace');
}
const SIGNED_URL_SEC = 120;
export async function getClientDocumentFileOpenUrl(ctx, orgId, clientId, fileAssetId) {
...
export async function uploadClientWorkspaceDocument(ctx, orgId, clientId, body) {
...
async function countActiveDocsInFolder(orgId, clientId, folderId) {
    const { count, error } = await supabaseAdmin
        ...
        .is('deleted_at', null);
    if (error)
        throw new AppError(500, error.message ?? 'count failed', 'SUPABASE_ERROR');
    return count ?? 0;
}
export async function executeClientDocumentsTabCommand(ctx, clientId, body) {
    const orgId = assertOrg(ctx);
    if (!canEditDocumentsTab(ctx))
    ...
    switch (cmd) {
        ...
        case 'create_client_document_folder': {
            ...
            if (!name_he)
                throw badRequest('שם תיקייה נדרש');
            ...
            if (error)
                throw new AppError(500, error.message ?? 'insert folder failed', 'SUPABASE_ERROR');
        ...
        case 'rename_client_document_folder': {
            ...
            const name_he = String(payload.name_he ?? '').trim();
            if (!folderId || !name_he)
                throw badRequest('נתונים חסרים');
            const row = await fetchFolder(orgId, clientId, folderId);
            if (!row || row.archived_at)
                throw badRequest('תיקייה לא נמצאה');
            if (row.system_key)
                throw badRequest('לא ניתן לשנות שם לתיקיית מערכת');
            const { error } = await supabaseAdmin
                ...
                .eq('client_id', clientId);
            if (error)
                throw new AppError(500, error.message ?? 'rename failed', 'SUPABASE_ERROR');
        ...
        case 'archive_or_delete_client_document_folder': {
            const folderId = String(payload.folder_id ?? '');
            if (!folderId)
                throw badRequest('folder_id נדרש');
            const row = await fetchFolder(orgId, clientId, folderId);
            if (!row || row.archived_at)
                throw badRequest('תיקייה לא נמצאה');
            if (row.system_key)
                throw badRequest('לא ניתן למחוק תיקיית מערכת');
            const n = await countActiveDocsInFolder(orgId, clientId, folderId);
            if (n > 0)
                throw badRequest('לא ניתן למחוק תיקייה עם מסמכים');
            const { error } = await supabaseAdmin
                .from('client_document_folders')
                ...
                .eq('client_id', clientId);
            if (error)
                throw new AppError(500, error.message ?? 'archive folder failed', 'SUPABASE_ERROR');
            await supabaseAdmin
                .from('client_document_profiles')
        ...
        case 'upload_client_document': {
            ...
            if (!folderId || !fileAssetId)
                throw badRequest('folder_id וקובץ נדרשים');
            const folder = await fetchFolder(orgId, clientId, folderId);
            if (!folder || folder.archived_at)
                throw badRequest('תיקייה לא נמצאה');
            await assertClientDocumentFileOpenAllowed(orgId, clientId, fileAssetId);
            const display_label_he = payload.display_label_he == null || payload.display_label_he === ''
                ? null
            ...
            if (error)
                throw new AppError(500, error.message ?? 'document insert failed', 'SUPABASE_ERROR');
        ...
        case 'delete_client_document': {
            ...
            if (!documentId)
                throw badRequest('document_id נדרש');
            ...
            const { error } = await supabaseAdmin
                ...
                .eq('client_id', clientId);
            if (error)
                throw new AppError(500, error.message ?? 'delete document failed', 'SUPABASE_ERROR');
        ...
        case 'open_client_document_folder': {
            ...
            const { error } = await supabaseAdmin
                ...
                .eq('client_id', clientId);
            if (error)
                throw new AppError(500, error.message ?? 'open folder failed', 'SUPABASE_ERROR');
            break;
        }
        default:
