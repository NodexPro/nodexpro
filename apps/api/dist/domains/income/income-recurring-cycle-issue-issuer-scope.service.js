...
import { applyOfficialIncomeIssuerContext, ensureOrgIncomeIssuerProfile, resolveIncomeIssuerBusinessDisplay, } from './income-issuer-context.service.js';
import { loadActiveIncomeIssuerScope } from './income-issuer-scope.service.js';
import { HEBREW_RECURRING_ISSUER_MISMATCH, resolveDraftIssuerRepairPlan, } from './income-recurring-cycle-issue-issuer-scope.pure.js';
...
async function loadTrustedRecurringIssueGraph(params) {
    ...
    const profileClientId = String(profile.represented_client_id);
    ...
    const cycleGeneratedDraftId = cycle.generated_draft_id ?? null;
 ...
 */
export async function resolveAndApplyRecurringCycleIssueIssuerScope(ctx, params) {
...
async function resolveAuthoritativeOfficeClientIdForDraft(params) {
    ...
    if (cycle) {
        const profileId = String(cycle.recurring_profile_id);
        const { data: profile, error: profileErr } = await supabaseAdmin
            .from('income_recurring_document_profiles')
        ...
        if (!profile)
            throw notFound('Recurring profile not found');
        return String(profile.represented_client_id);
    }
    ...
    if (params.draft.issuer_business_id)
        return params.draft.issuer_business_id;
    throw issuerMismatchError();
}
/**
 ...
 */
export async function resolveAndApplyIssuerScopeFromTrustedOfficeDraftIfNeeded(ctx, params) {
    const orgId = ctx.organizationId;
    if (!orgId)
        throw forbidden('Organization context required');
    if (!ctx.user?.id)
    ...
    const isOfficeDraft = draft.represented_client_id != null ||
        linkedCycle != null ||
...
export async function loadRecurringCycleReviewRefsByGeneratedDraft(params) {
    ...
    if (!profile)
        return null;
    return {
        represented_client_id: String(profile.represented_client_id),
 ...
 */
export async function applyOfficialOfficeIssuerContextForRepresentedClient(ctx, representedClientId, source) {
    await applyOfficialIncomeIssuerContext(ctx, {
        acting_mode: 'office_representative',
 ...
 */
export async function prepareRecurringCycleReviewIssuerScope(ctx, params) {
    const orgId = ctx.organizationId;
    if (!orgId)
        throw forbidden('Organization context required');
    if (!ctx.user?.id)
    ...
    await applyOfficialOfficeIssuerContextForRepresentedClient(ctx, params.representedClientId, params.source);
}
