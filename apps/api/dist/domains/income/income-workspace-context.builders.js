...
export function buildAllowedActingModes(perms) {
    return [
        {
            mode: 'self',
            label: 'Office (self)',
...
export function buildIssuerOptions(orgIssuer, clients, perms) {
    const options = [];
    if (perms.view) {
        options.push({
...
export function buildAllowedActions(perms) {
    const actions = [];
    if (perms.view)
        actions.push('view_workspace_context');
    if (perms.edit)
        actions.push('select_issuer_context');
    return actions;
}
