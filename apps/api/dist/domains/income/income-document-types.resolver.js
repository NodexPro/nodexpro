...
async function resolveOrgCountryCode(orgId) {
    const { data } = await supabaseAdmin.from('organizations').select('country_code').eq('id', orgId).single();
    return String(data?.country_code ?? 'IL')
        .trim()
        .toUpperCase() || 'IL';
}
async function tryCountryPackRulesetId(orgId) {
    const { data } = await supabaseAdmin
...
export async function resolveAvailableDocumentTypes(orgId, scope) {
    ...
    if (business_type === 'unknown') {
        warnings.push({
            ...
            message: scope.acting_mode === 'office_representative'
    ...
    const source = 'fallback_il';
    const available_document_types = buildAvailableDocumentTypesForBusiness(business_type, country_code, ruleset_id, source);
    if (country_code !== 'IL') {
        warnings.push({
