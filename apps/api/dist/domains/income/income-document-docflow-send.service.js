...
function assertDocflowSendViewAccess(ctx) {
    const perms = incomeWorkspacePermissionsFromContext(ctx);
    if (!perms.view)
        throw forbidden('income.view required');
}
async function loadIssuedDocumentForDocflowSend(orgId, incomeDocumentId) {
...
async function loadRepresentedClientDisplayName(orgId, clientId) {
    const { data, error } = await supabaseAdmin
        .from('clients')
