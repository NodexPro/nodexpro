...
import { assertIncomeIssuerContextForCommand, ensureOrgIncomeIssuerProfile, incomeWorkspacePermissionsFromContext, } from './income-issuer-context.service.js';
import { loadActiveIncomeIssuerScope } from './income-issuer-scope.service.js';
export function scopeMatchesIssuedDocument(scope, doc) {
    return (doc.organization_id === scope.org_id &&
 ...
 */
export async function resolveIssuerScopeForIssuedDocument(ctx, doc) {
    const active = await loadActiveIncomeIssuerScope(ctx);
    if (scopeMatchesIssuedDocument(active, doc))
        return active;
    ...
    const isOfficeDocument = doc.represented_client_id != null || doc.issuer_business_id !== orgIssuer.id;
    if (!isOfficeDocument) {
        assertRowMatchesIssuerScope(active, doc);
        return active;
    }
    ...
    const representedClient = clientRow ?? null;
    ...
    const label = representedClient?.display_name?.trim() ||
