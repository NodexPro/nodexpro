...
import { parseIncomePaymentMethodKey, resolveIncomeInvoiceOriginalAmount, resolveIncomeInvoicePaymentState, } from '../accounting-base/accounting-base-income-payment.pure.js';
...
import { findAvailableDocumentType, resolveAvailableDocumentTypes, assertDocumentTypeEnabled, } from './income-document-types.resolver.js';
...
import { buildIncomeDocumentPaymentCaseAggregate, } from './income-document-payment-case.read.js';
import { INCOME_COMMAND_RECORD_DOCUMENT_PAYMENT, INCOME_DOCUMENT_LINK_PAYMENT_RECEIPT_FOR_INVOICE, buildIncomePaymentReceiptDetailsText, optionalTrimmedString, } from './income-document-payment.pure.js';
function requirePaymentOrchestrationPerms(ctx) {
    const perms = ctx.membership?.permissions ?? [];
    if (!perms.includes('income.issue'))
...
async function loadInvoice(organizationId, incomeDocumentId) {
    const { data, error } = await supabaseAdmin
        .from('income_documents')
        .select('id, organization_id, issuer_business_id, represented_client_id, document_type, document_number, document_status, currency, issue_date, income_customer_id, customer_snapshot_json, totals_snapshot_json, language')
        .eq('organization_id', organizationId)
        .eq('id', incomeDocumentId)
        .maybeSingle();
    throwIfSupabaseError(error, 'Failed to load income document');
    if (!data)
        throw badRequest('Income document not found');
    return data;
}
async function findOperation(organizationId, idempotencyKey) {
    const { data, error } = await supabaseAdmin
        .from('income_document_payment_operations')
        .select('id, organization_id, idempotency_key, invoice_document_id, payment_id, allocation_id, receipt_draft_id, receipt_document_id, status, amount, currency, payment_date, payment_method_key, reference_number, note')
        .eq('organization_id', organizationId)
...
async function updateOperation(organizationId, operationId, patch) {
...
async function createReceiptDraft(args) {
    const { available_document_types } = await resolveAvailableDocumentTypes(args.scope.org_id, args.scope);
    ...
    if (!docType)
        throw badRequest('receipt document type is not available');
    ...
    const payload = {
        ...
        one_time_customer_snapshot_json: args.invoice.income_customer_id == null
...
async function buildFullResponse(ctx, command, invoiceId, paymentId, allocationId, receiptId, replay, documentsListYear) {
...
export async function executeRecordIncomeDocumentPayment(ctx, body) {
    ...
    try {
        ...
        throw badRequest('payment_method_key is invalid (supported: bank_transfer, cash, check, credit_card, other)');
    }
    ...
    try {
        ...
        if (operation?.status === 'completed' && operation.receipt_document_id && operation.payment_id && operation.allocation_id) {
            return buildFullResponse(ctx, INCOME_COMMAND_RECORD_DOCUMENT_PAYMENT, operation.invoice_document_id, operation.payment_id, operation.allocation_id, operation.receipt_document_id, true, documentsListYear);
        }
        ...
        if (!operation)
            throw conflict('Payment operation could not be started');
        ...
        if (!receiptDocumentId) {
            if (!receiptDraftId)
                throw conflict('Receipt draft missing for payment operation');
        ...
        return buildFullResponse(ctx, INCOME_COMMAND_RECORD_DOCUMENT_PAYMENT, invoice.id, paymentId, allocationId, receiptDocumentId, false, documentsListYear);
    }
    catch (err) {
