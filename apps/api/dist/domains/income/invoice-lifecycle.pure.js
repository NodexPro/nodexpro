 ...
 */
import { isSupportedIncomePaymentDocumentType, resolveIncomeInvoicePaymentState, } from '../accounting-base/accounting-base-income-payment.pure.js';
import { isInvoiceCollectionDocumentType } from './income-work-engine-bridge.pure.js';
function attemptAt(row) {
...
function summarizeChannel(rows) {
    if (rows.length === 0)
        return emptyChannel();
    ...
    for (const row of rows) {
        ...
        if (!lastAttemptAt || at > lastAttemptAt)
            lastAttemptAt = at;
        if (row.result === 'sent') {
            if (!lastSuccessAt || at > lastSuccessAt)
                lastSuccessAt = at;
        }
        if (row.result === 'failed') {
            if (!lastFailureAt || at > lastFailureAt)
                lastFailureAt = at;
        }
    }
    return {
 ...
 * - else not_sent
 */
export function composeInvoiceLifecycleDeliveryDimension(attempts) {
...
export function composeInvoiceLifecyclePaymentState(originalAmount, paidAmount) {
...
export function backendTodayIsoDate(now = new Date()) {
