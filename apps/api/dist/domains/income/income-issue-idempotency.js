...
function normalizeIdempotencyKey(raw) {
    ...
    if (!k)
        return null;
    if (k.length > 256)
        throw badRequest('idempotency_key too long', 'idempotency_key_too_long');
    return k;
}
export function parseIssueIdempotencyKey(body) {
...
export async function abortIncomeIssueIdempotency(leaseRowId) {
    const { error } = await supabaseAdmin
        .from('income_command_idempotency')
        .delete()
        .eq('id', leaseRowId);
