...
function stage(key, state, completed, current) {
    return {
        key,
 ...
 */
export function composeInvoiceLifecycleRibbon(input) {
    const draftCompleted = input.document.document_state_key === 'issued' || Boolean(input.document.source_draft_id);
    ...
    rows.push(stage('credited', 'unavailable', false, false), stage('voided', 'unavailable', false, false), stage('closed', 'unavailable', false, false));
