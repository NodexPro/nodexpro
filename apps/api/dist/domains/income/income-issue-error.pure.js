 ...
 */
export const HEBREW_ISSUER_SCOPE_MISMATCH = 'לא ניתן להפיק את המסמך משום שזהות מנפיק המסמך אינה תואמת ללקוח המיוצג.';
...
export function mapIncomeIssueUserFacingMessage(message) {
    const trimmed = String(message ?? '').trim();
    if (!trimmed)
        return null;
    if (ENGLISH_ISSUER_SCOPE_MESSAGES.has(trimmed))
        return HEBREW_ISSUER_SCOPE_MISMATCH;
    return null;
}
export function mapIncomeIssueFailingStageMessage(failingStage) {
...
export function extractIncomeIssueThrownMessage(error) {
