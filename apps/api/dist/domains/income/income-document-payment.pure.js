 ...
 */
export const INCOME_COMMAND_RECORD_DOCUMENT_PAYMENT = 'record_income_document_payment';
export const INCOME_DOCUMENT_PAYMENT_CASE_KEY = 'income_document_payment_case';
export const INCOME_DOCUMENT_LINK_PAYMENT_RECEIPT_FOR_INVOICE = 'payment_receipt_for_invoice';
...
export function resolvePaymentStateIcon(paymentStateKey) {
...
export function buildIncomeDocumentRecordPaymentForm(params) {
    const remaining = Number.isFinite(params.remainingBalance) && params.remainingBalance > 0
        ? String(Math.round(params.remainingBalance * 100) / 100)
        : '';
    const currency = String(params.currency ?? 'ILS').trim().toUpperCase() || 'ILS';
    const paymentDateDefault = params.paymentDateDefault && /^\d{4}-\d{2}-\d{2}$/.test(params.paymentDateDefault)
