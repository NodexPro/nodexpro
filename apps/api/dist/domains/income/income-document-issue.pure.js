 ...
 */
export function assertDraftReadyToIssue(draft) {
...
export function formatIncomeDocumentNumber(year, sequenceNumber, prefix) {
    if (prefix?.trim())
        return `${prefix.trim()}${sequenceNumber}`;
...
export function buildTotalsSnapshotForIssue(draftTotalsPreview, currency, lineCount) {
    return {
        ...(draftTotalsPreview ?? {}),
        preview: true,
...
export const ACCOUNTING_BASE_POST_AFTER_ISSUE_TODO = 'TEMPORARY_ACCOUNTING_BASE_PENDING: post revenue entry to Accounting Base after income.document_issued';
