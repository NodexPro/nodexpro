 ...
 */
export const IL_ISSUE_MONTH_WINDOW_FALLBACK = {
    months_back: 1,
    months_ahead: 3,
};
export function issueMonthWindowFallbackResolution() {
    return {
...
export function parseIssueMonthWindowFromLegalPayload(payload) {
    if (payload == null || typeof payload !== 'object' || Array.isArray(payload))
        return null;
    const o = payload;
    const back = Number(o.months_back);
    const ahead = Number(o.months_ahead);
    if (!Number.isInteger(back) || !Number.isInteger(ahead))
        return null;
    if (back < 0 || ahead < 0 || back > 24 || ahead > 24)
        return null;
