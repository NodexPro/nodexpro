...
import { defaultIncomeTaxAllocationNumberPolicy, IL_INCOME_TAX_ALLOCATION_NUMBER_POLICY_KEY, parseIncomeTaxAllocationNumberPolicy, } from './income-document-allocation-number.pure.js';
export async function resolveIncomeTaxAllocationNumberPolicyForOrg(orgId, countryCode, date) {
    ...
    try {
        ...
        if (ctx.ruleset_id) {
            const version = await resolveLegalValue(cc, IL_INCOME_TAX_ALLOCATION_NUMBER_POLICY_KEY, day, ctx.ruleset_id);
            ...
            const fromMap = parseIncomeTaxAllocationNumberPolicy(ctx.resolved_values_map[IL_INCOME_TAX_ALLOCATION_NUMBER_POLICY_KEY]);
