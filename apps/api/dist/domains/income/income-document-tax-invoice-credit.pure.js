...
export const INCOME_COMMAND_BEGIN_TAX_INVOICE_CREDIT = 'begin_income_tax_invoice_credit';
export const INCOME_CREDIT_DOCUMENT_TYPE = 'credit_tax_invoice';
export const INCOME_CREDIT_SOURCE_SETTINGS_KEY = 'income_tax_invoice_credit';
export const CREDIT_AMOUNT_EXCEEDS_REMAINING_CREDITABLE = 'CREDIT_AMOUNT_EXCEEDS_REMAINING_CREDITABLE';
export const CREDIT_LINE_EXCEEDS_REMAINING_CREDITABLE = 'CREDIT_LINE_EXCEEDS_REMAINING_CREDITABLE';
export const INCOME_TAX_INVOICE_CREDIT_REASON_OPTIONS = [
...
export function isIncomeTaxInvoiceCreditMode(value) {
    return value === 'full' || value === 'partial';
}
export function parseIncomeTaxInvoiceCreditReasonKey(value) {
    const key = String(value ?? '').trim();
    if (INCOME_TAX_INVOICE_CREDIT_REASON_OPTIONS.some((option) => option.key === key))
        return key;
    return 'other';
}
export function creditReasonLabel(reasonKey) {
    return (INCOME_TAX_INVOICE_CREDIT_REASON_OPTIONS.find((option) => option.key === reasonKey)?.label ??
        'אחר');
}
export function sourceLineIdentityFromSnapshot(line, index) {
 ...
 */
export function resolveCanonicalCreditNoteAmount(totalsSnapshot) {
 ...
 */
export function applySourceDocumentDiscountNetToCreditDraftLines(params) {
    ...
    if (!(before > 0) || !Number.isFinite(after) || after < 0)
        return params.lines;
    const factor = after / before;
    if (!(factor > 0) || factor > 1.0001)
        return params.lines;
    if (Math.abs(factor - 1) < 0.0000001)
        return params.lines;
    return params.lines.map((line) => {
        ...
        return {
            ...line,
            amount_reference: typeof amount === 'number' && Number.isFinite(amount) ? roundMoney2(amount * factor) : amount,
            unit_price_reference: typeof unit === 'number' && Number.isFinite(unit) ? roundMoney2(unit * factor) : unit,
        };
    });
}
export function resolveCreditState(params) {
    ...
    if (credited <= 0)
        return { credit_state: 'none', remaining_creditable_amount: remaining };
    if (remaining <= 0.005)
        return { credit_state: 'full', remaining_creditable_amount: 0 };
...
export function creditSourceReferenceDisplay(sourceInvoiceNumber) {
    const number = String(sourceInvoiceNumber ?? '').trim();
    if (!number)
        return 'זיכוי עבור חשבונית מס';
    return `זיכוי עבור חשבונית מס מספר ${number}`;
}
export function mergeCreditSourceReferenceIntoNotes(notes, reference) {
    const ref = String(reference ?? '').trim();
    const base = String(notes ?? '').trim();
    if (!ref)
        return notes != null && String(notes).trim() ? String(notes) : null;
    if (base.includes(ref))
        return notes != null && String(notes).trim() ? String(notes) : ref;
    return base ? `${base}\n${ref}` : ref;
}
...
export function readCreditDraftSettings(documentSettingsJson) {
    if (!documentSettingsJson || typeof documentSettingsJson !== 'object' || Array.isArray(documentSettingsJson)) {
        return null;
    }
    const raw = documentSettingsJson[INCOME_CREDIT_SOURCE_SETTINGS_KEY];
    ...
    if (rawMap && typeof rawMap === 'object' && !Array.isArray(rawMap)) {
        for (const [draftLineId, entry] of Object.entries(rawMap)) {
            if (!entry || typeof entry !== 'object' || Array.isArray(entry))
                continue;
            const e = entry;
            const source_line_identity = String(e.source_line_identity ?? '').trim();
            if (!source_line_identity)
                continue;
    ...
    return {
        ...
        locked_income_customer_id: o.locked_income_customer_id != null && String(o.locked_income_customer_id).trim()
...
export function writeCreditDraftSettings(documentSettingsJson, settings) {
    return {
 ...
 */
export function preserveIncomeTaxInvoiceCreditInDocumentSettings(existingDocumentSettingsJson, nextSettingsJson) {
    const nextWithoutClientCredit = { ...nextSettingsJson };
    delete nextWithoutClientCredit[INCOME_CREDIT_SOURCE_SETTINGS_KEY];
    if (!existingDocumentSettingsJson ||
        typeof existingDocumentSettingsJson !== 'object' ||
        Array.isArray(existingDocumentSettingsJson)) {
        return nextWithoutClientCredit;
    }
    const existingRaw = existingDocumentSettingsJson[INCOME_CREDIT_SOURCE_SETTINGS_KEY];
...
export function creditConsumeLinesFromDraft(params) {
    ...
    for (const raw of lines) {
        ...
        if (!mapped) {
            ...
            err.code = CREDIT_LINE_EXCEEDS_REMAINING_CREDITABLE;
            throw err;
        }
...
export function decideAtomicCreditConsume(params) {
    try {
        ...
        return {
            ok: false,
            code: e.code ?? CREDIT_AMOUNT_EXCEEDS_REMAINING_CREDITABLE,
        };
    }
    for (const line of params.lines ?? []) {
        if (line.requestedQuantity - line.remainingQuantity > 0.0009 ||
...
export function assertCreditAmountWithinRemaining(params) {
    ...
    if (requested - remaining > 0.005) {
        ...
        err.code = CREDIT_AMOUNT_EXCEEDS_REMAINING_CREDITABLE;
        throw err;
    }
}
