...
import { INCOME_DOCUMENT_LINK_PAYMENT_RECEIPT_FOR_INVOICE, INCOME_DOCUMENT_PAYMENT_CASE_KEY, INCOME_COMMAND_RECORD_DOCUMENT_PAYMENT, resolvePaymentStateIcon, } from './income-document-payment.pure.js';
export async function listPaymentReceiptLinksForInvoice(organizationId, invoiceDocumentId) {
    const { data, error } = await supabaseAdmin
        .from('income_document_links')
        .select('target_document_id, payment_id, allocation_id, allocated_amount, currency, relationship_label, relationship_key, created_at')
...
async function countIssuedByType(organizationId, issuerBusinessId, representedClientId) {
    let q = supabaseAdmin
        .from('income_documents')
    ...
    for (const row of data ?? []) {
        const t = String(row.document_type);
        if (t === 'tax_invoice')
            tax_invoice += 1;
        if (t === 'receipt')
            receipt += 1;
...
export async function buildIncomeDocumentPaymentCaseAggregate(ctx, organizationId, incomeDocumentId, opts) {
    ...
    const counters = await countIssuedByType(organizationId, abCase.issuer_business_id, abCase.represented_client_id);
    const newly = opts?.newlyIssuedReceiptId != null
        ? linked.find((r) => r.document_id === opts.newlyIssuedReceiptId) ?? null
        : null;
    const canRecord = abCase.allowed_actions.some((a) => a.action_key === 'record_payment' && a.enabled);
    const recordReason = abCase.allowed_actions.find((a) => a.action_key === 'record_payment')?.reason ?? null;
    ...
    const issueYear = issueYearFromIso(String(issueDateRow?.issue_date ?? ''));
    const documentsListYear = opts?.documentsListYear != null && Number.isFinite(opts.documentsListYear)
        ? Number(opts.documentsListYear)
        : issueYear;
    let documentsByType = null;
    let sourceInvoiceRow = null;
    let recordPaymentForm = null;
...
export async function assertIncomeDocumentExistsInOrg(organizationId, incomeDocumentId) {
    const { data, error } = await supabaseAdmin
        .from('income_documents')
        .select('id')
        .eq('organization_id', organizationId)
...
export function requireIncomeViewOrPaymentPerm(ctx) {
