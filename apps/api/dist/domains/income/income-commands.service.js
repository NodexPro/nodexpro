...
export async function executeIncomeCommand(ctx, body, auditMeta) {
    ...
    if (command === INCOME_COMMAND_SELECT_RECIPIENT) {
        ...
        if (!row) {
            throw notFound('Income recipient not found for the active issuer. Refresh the list and try again.', 'INCOME_RECIPIENT_NOT_FOUND');
        }
        const overlay = {
        ...
        if (searchQuery)
            overlay.search_query = searchQuery;
