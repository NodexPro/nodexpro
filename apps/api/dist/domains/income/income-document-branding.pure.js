...
export const LEGACY_COLOR_KEY_TO_THEME = {
...
function buildStyleMiniPreview(styleKey, theme) {
...
function buildAccentColorTheme(key, label, accentHex, options) {
...
export const INCOME_COLOR_THEME_PRESETS = COLOR_THEME_DEFS.map((t) => ({
    ...t,
    gradient: { ...t.gradient },
...
export const INCOME_LAYOUT_TEMPLATES = [
    {
        key: 'logo_left_client_right',
        label: 'לוגו משמאל · לקוח מימין',
        mini_preview_markup: '<div class="nx-studio-layout-mini nx-studio-layout-mini--llcr" dir="rtl"><span></span><span></span></div>',
        advanced_only: true,
    },
    {
        key: 'logo_top_client_below',
        label: 'לוגו למעלה · לקוח מתחת',
        mini_preview_markup: '<div class="nx-studio-layout-mini nx-studio-layout-mini--ltcb" dir="rtl"><span></span><span></span></div>',
        advanced_only: true,
    },
    {
        key: 'israeli_classic',
        label: 'קלאסי ישראלי',
        mini_preview_markup: '<div class="nx-studio-layout-mini nx-studio-layout-mini--il" dir="rtl"><span></span><span></span></div>',
        advanced_only: true,
    },
    {
        key: 'logo_right_client_left',
        label: 'לוגו מימין · לקוח משמאל',
        mini_preview_markup: '<div class="nx-studio-layout-mini nx-studio-layout-mini--lrl" dir="rtl"><span></span><span></span></div>',
        advanced_only: true,
    },
];
export const INCOME_LOGO_SIZE_OPTIONS = [
...
export function getColorThemePresets() {
...
const STUDIO_COLOR_THEME_LABELS = {
...
export const INCOME_DOCUMENT_TYPE_STYLE_GROUP_DEFS = [
    {
...
export function normalizeDocumentTypeStyleGroupKey(value) {
    const key = String(value ?? '').trim();
    return INCOME_DOCUMENT_TYPE_STYLE_GROUP_KEYS.includes(key)
        ? key
        : null;
}
export function resolveDocumentTypeStyleGroupKey(documentType) {
    const key = String(documentType ?? '').trim();
    if (key === 'tax_invoice' || key === 'tax_invoice_receipt')
        return 'tax_group';
    if (key === 'receipt')
        return 'receipt';
    if (key === 'credit_tax_invoice' || key === 'credit_note')
        return 'credit';
    if (key === 'deal_invoice' || key === 'quote')
        return 'quote_deal';
    return 'quote_deal';
}
export function normalizeColorThemeKey(key) {
    const trimmed = String(key ?? '').trim();
    if (!trimmed)
        return DEFAULT_COLOR_THEME_KEY;
    if (INCOME_COLOR_THEME_PRESETS.some((preset) => preset.key === trimmed))
        return trimmed;
...
export function normalizeLegacyDocumentColorThemeKey(key) {
    const normalized = normalizeColorThemeKey(key);
    return normalized === 'black_white' ? DEFAULT_COLOR_THEME_KEY : normalized;
}
export function parseDocumentTypeStyleOverridesJson(raw) {
    ...
    for (const groupKey of INCOME_DOCUMENT_TYPE_STYLE_GROUP_KEYS) {
        ...
        if (!resolveDocumentStyleTemplate(document_style_key) || !resolveColorThemePreset(color_theme_key))
            continue;
        out[groupKey] = { document_style_key, color_theme_key };
    }
    return out;
}
export function serializeDocumentTypeStyleOverridesJson(overrides) {
    const out = {};
    for (const groupKey of INCOME_DOCUMENT_TYPE_STYLE_GROUP_KEYS) {
...
export function resolveEffectiveStyleForGroup(groupKey, overrides) {
...
export function buildDocumentTypeStyleGroups(overrides) {
...
export function mergeDocumentTypeStyleOverrides(current, incoming) {
    return { ...current, ...incoming };
}
export function applyDocumentTypeStyleOverridesFromBody(row, body) {
    let overrides = parseDocumentTypeStyleOverridesJson(row.document_type_style_overrides);
    if (body.document_type_style_overrides &&
        typeof body.document_type_style_overrides === 'object' &&
        !Array.isArray(body.document_type_style_overrides)) {
        overrides = mergeDocumentTypeStyleOverrides(overrides, parseDocumentTypeStyleOverridesJson(body.document_type_style_overrides));
    }
    const groupKey = normalizeDocumentTypeStyleGroupKey(body.selected_document_type_group_key) ?? 'quote_deal';
...
export function getStudioNavigationSections() {
    return [
...
export function buildDisplayOptionControls(display) {
    return [
...
export function mergeDisplayOptionsFromStudioBody(body, current, clientPos) {
    const bool = (key, fallback) => body[key] === undefined ? fallback : parseBooleanBody(body[key]);
    return {
        ...current,
...
export function mergePaymentMethodsFromStudioBody(body, methods) {
...
export function buildStudioSampleIssuerIdentityPreview() {
    return buildIssuerIdentityPreview({
        ...
        read_only: true,
        helper_text: 'פרטי העסק במסמך נלקחים מפרופיל העסק או מהלקוח המיוצג בעת הפקת מסמך — כאן מוצגת דוגמה בלבד.',
...
/** @deprecated Use getColorThemePresets */
export function getDocumentStylePresets() {
...
export function getLogoSizeOptions() {
    return INCOME_LOGO_SIZE_OPTIONS.map((o) => ({ ...o }));
}
export function resolveColorThemePreset(key) {
    const trimmed = String(key ?? '').trim();
    if (!trimmed)
        return null;
...
export function resolveDocumentStylePreset(key) {
    const k = key.trim();
...
export function resolveLayoutTemplate(key) {
    const k = key.trim();
    return INCOME_LAYOUT_TEMPLATES.find((t) => t.key === k) ?? null;
}
export function resolveLogoSizeKey(value) {
...
export function resolveColorThemeKeyForRow(row) {
    ...
    return normalizeColorThemeKey(matchColorThemeKeyFromLegacyColors(row.primary_color, row.table_header_color, row.totals_color));
}
export function resolveEffectiveLayoutTemplateKey(row) {
...
export function matchColorThemeKeyFromLegacyColors(primaryColor, tableHeaderColor, totalsColor) {
    ...
    for (const preset of INCOME_COLOR_THEME_PRESETS) {
        if (preset.table_header_color === t &&
...
export function matchDocumentStyleKeyFromLegacyColors(primaryColor, tableHeaderColor, totalsColor) {
    return matchColorThemeKeyFromLegacyColors(primaryColor, tableHeaderColor, totalsColor);
}
export function applyColorThemeToColorColumns(theme) {
    return {
...
export function applyDocumentStyleToColorColumns(theme) {
...
export function applyDocumentStyleTemplateKey(styleKey) {
...
export function applyBrandingStyleToResolvedProfile(base, styleKey, colorThemeKey) {
...
export function resolveBrandingProfileForDocumentTypeGroup(row, assets, groupKey, draftOverrides) {
    const base = resolveBrandingProfile(row, assets);
    const overrides = draftOverrides ?? parseDocumentTypeStyleOverridesJson(row.document_type_style_overrides);
    const effective = resolveEffectiveStyleForGroup(groupKey, overrides);
    return applyBrandingStyleToResolvedProfile(base, effective.document_style_key, effective.color_theme_key);
}
...
export function resolveBrandingPreviewThemePalette(theme) {
    return {
...
export function resolveLogoSizeDimensions(logoSizeKey) {
...
export function formatDocumentNumberDisplay(numberPreview) {
