...
export function resolveDraftDeliveryContactEmail(deliveryContactJson) {
    if (!deliveryContactJson || typeof deliveryContactJson !== 'object')
        return null;
    const raw = deliveryContactJson.email;
...
export function resolveIssueAndSendRecipientEmail(params) {
    const bodyEmail = params.body_recipient_email != null && String(params.body_recipient_email).trim()
        ? String(params.body_recipient_email).trim()
        : null;
    if (bodyEmail)
        return normalizeIncomeDocumentRecipientEmail(bodyEmail);
