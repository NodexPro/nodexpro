...
export function isLikelyInternalShortCode(value) {
    const s = value.trim();
    if (!s || s.length > 8)
        return false;
    if (/[\u0590-\u05FF]/.test(s) || /\s/.test(s))
    ...
    if (!/^[A-Za-z0-9][A-Za-z0-9_-]*$/.test(s))
        return false;
    if (s.length <= 6 && /^[A-Z0-9_-]+$/.test(s))
        return true;
    if (s.length <= 4)
        return true;
    return false;
}
export function hasPublicBusinessSignals(party) {
    if (party.tax_id?.trim())
        return true;
...
function resolvePublicDisplayName(party, fallbackDisplayName) {
    const raw = party.display_name.trim();
    if (!raw || isLikelyInternalIdentifier(raw))
        return fallbackDisplayName;
    if (isLikelyInternalShortCode(raw) && !hasPublicBusinessSignals(party))
        return fallbackDisplayName;
    return raw;
}
