...
async function loadIssuedDocumentForDocflow(orgId, incomeDocumentId) {
    const { data, error } = await supabaseAdmin
        .from('income_documents')
        .select('id, organization_id, issuer_business_id, represented_client_id, document_type, document_number, document_status, issue_date, due_date, currency, pdf_render_status, pdf_asset_id, customer_snapshot_json, totals_snapshot_json, language')
...
export async function executeSendIncomeDocumentByDocflow(ctx, body, deps = defaultDeps) {
    ...
    if (!client)
        throw notFound('Represented client profile not found');
    const language = doc.language === 'en' ? 'en' : 'he';
    const messageSnapshot = buildIncomeDocumentDocflowMessageSnapshot({
        documentTypeLabel: documentTypeLabel(doc.document_type, language),
    ...
    const deliveryIdempotencyKey = buildIncomeDocumentDocflowDeliveryIdempotencyKey(incomeDocumentId, commandIdempotencyKey);
    ...
    let docflowThreadId = attempt.docflowThreadId;
    let docflowMessageId = attempt.docflowMessageId;
    if (!idempotentReplay) {
        await writeAudit({
            organizationId: scope.org_id,
