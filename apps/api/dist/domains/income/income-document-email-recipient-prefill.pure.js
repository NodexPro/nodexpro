...
export function normalizeIncomeDocumentRecipientEmailPrefill(email) {
    const trimmed = email != null ? String(email).trim() : '';
    return trimmed ? trimmed : null;
}
/** @deprecated Use normalizeIncomeDocumentRecipientEmailPrefill */
export const normalizeRepresentedClientRecipientEmailPrefill = normalizeIncomeDocumentRecipientEmailPrefill;
export function resolveCustomerSnapshotEmail(customerSnapshotJson) {
    if (!customerSnapshotJson || typeof customerSnapshotJson !== 'object')
        return null;
    return normalizeIncomeDocumentRecipientEmailPrefill(customerSnapshotJson.email != null ? String(customerSnapshotJson.email) : null);
}
/**
...
export function resolveIssuedDocumentEmailRecipientPrefill(params) {
    ...
    const hasSavedCustomer = params.incomeCustomerId != null && String(params.incomeCustomerId).trim() !== '';
    if (hasSavedCustomer) {
