...
const BRANDING_MIGRATION_HINT = 'Run Supabase migrations 130–133 for income_document_branding_profiles on production.';
...
async function persistBrandingProfilePatch(rowId, orgId, patch, context) {
    ...
    if (error) {
        ...
        for (const col of STUDIO_OPTIONAL_COLUMNS) {
            if (col in nextPatch && isSupabaseMissingColumnError(error, col)) {
...
export async function loadResolvedBrandingProfileForDocumentType(scope, documentType) {
    const row = await ensureIncomeDocumentBrandingProfile(scope);
    ...
    return resolveBrandingProfileForDocumentTypeGroup(row, { logo_data_url, signature_data_url }, groupKey);
}
...
async function buildDocumentBrandingStudio(scope, resolved, row, options) {
    ...
    const groupDef = INCOME_DOCUMENT_TYPE_STYLE_GROUP_DEFS.find((entry) => entry.group_key === selectedGroupKey) ??
    ...
    const resolvedForGroup = resolveBrandingProfileForDocumentTypeGroup(row, { logo_data_url, signature_data_url }, selectedGroupKey, overrides);
...
export function buildDocumentBrandingSettingsEntrypoint(permissions) {
...
function mergeBrandingDraftBodyOntoRow(row, body) {
    const patch = {};
...
async function resolveBrandingProfileForRowWithGroup(row, groupKey) {
    const logo_data_url = row.logo_file_asset_id
        ? await fileAssetToDataUrl(row.logo_file_asset_id)
        : null;
    const signature_data_url = row.signature_file_asset_id
        ...
        : null;
    const overrides = parseDocumentTypeStyleOverridesJson(row.document_type_style_overrides);
    return resolveBrandingProfileForDocumentTypeGroup(row, { logo_data_url, signature_data_url }, groupKey, overrides);
}
export async function previewIncomeDocumentBrandingProfileDraft(scope, body) {
    const row = await ensureIncomeDocumentBrandingProfile(scope);
    ...
    const groupDef = INCOME_DOCUMENT_TYPE_STYLE_GROUP_DEFS.find((entry) => entry.group_key === groupKey) ??
        INCOME_DOCUMENT_TYPE_STYLE_GROUP_DEFS[0];
    const emailParts = buildEmailTemplateStudioParts(resolved);
    return {
        studio_live_preview: buildStudioSampleLivePreview({
...
function applyModalBrandingPatch(row, body, patch) {
    ...
    const overrides = applyDocumentTypeStyleOverridesFromBody(row, body);
    if (body.document_type_style_overrides !== undefined ||
    ...
    patch.display_options = serializeDisplayOptionsJson(mergeDisplayOptionsFromStudioBody(body, current, clientPos));
    patch.client_block_position = clientPos;
    if (DEFAULT_PAYMENT_METHODS.some((method) => body[`payment_method_${method.key}`] !== undefined)) {
        patch.payment_methods = serializePaymentMethodsJson(mergePaymentMethodsFromStudioBody(body, parsePaymentMethodsJson(row.payment_methods)));
    }
}
...
export async function updateIncomeDocumentBrandingProfile(scope, body) {
    ...
    if (section === 'modal') {
        ...
        if (docStyleKey && resolveDocumentStyleTemplate(docStyleKey)) {
            Object.assign(patch, applyDocumentStyleTemplateKey(docStyleKey));
        }
        patch.company_subtitle = optionalTrimmedString(body.company_subtitle, 500);
        const clientPos = normalizeClientBlockPosition(body.client_block_position ?? row.client_block_position);
        const current = parseDisplayOptionsJson(row.display_options, clientPos);
