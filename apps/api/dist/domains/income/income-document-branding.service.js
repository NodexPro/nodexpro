...
const BRANDING_MIGRATION_HINT = 'Run Supabase migrations 130_income_document_branding_profiles.sql, 131_income_document_branding_document_style_key.sql, and 132_income_document_branding_studio_keys.sql on production.';
const STUDIO_OPTIONAL_COLUMNS = ['color_theme_key', 'layout_template_key', 'logo_size_key'];
...
function formatIssuerAddressLine(addressJson) {
    if (!addressJson || typeof addressJson !== 'object' || Array.isArray(addressJson))
        return null;
    const o = addressJson;
    const parts = [o.line1, o.line2, o.city, o.postal_code].filter((p) => typeof p === 'string' && p.trim());
...
async function resolveBrandingProfileForRow(row) {
    const logo_data_url = row.logo_file_asset_id
        ? await fileAssetToDataUrl(row.logo_file_asset_id)
        : null;
    const signature_data_url = row.signature_file_asset_id
...
function applyModalBrandingPatch(row, body, patch) {
    ...
    const colorKey = String(body.color_theme_key ?? body.color_preset_key ?? '').trim() ||
