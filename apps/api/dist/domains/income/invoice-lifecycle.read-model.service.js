...
import { backendTodayIsoDate, composeInvoiceLifecycleDeliveryDimension, composeInvoiceLifecycleDueDimension, composeInvoiceLifecyclePaymentState, } from './invoice-lifecycle.pure.js';
import { INVOICE_LIFECYCLE_AGGREGATE_KEY, } from './invoice-lifecycle.types.js';
...
async function loadLastPaymentAt(organizationId, incomeDocumentId) {
    ...
    const paymentIds = Array.from(new Set((allocRows ?? []).map((r) => String(r.payment_id)).filter(Boolean)));
    ...
    const date = payRows?.[0]?.payment_date;
...
function buildAllowedActions(params) {
    ...
    const canRecord = params.hasAccountingPaymentWrite &&
...
export async function buildInvoiceLifecycleAggregate(params) {
    ...
    const { data: docRaw, error: docErr } = await supabaseAdmin
        ...
        .select('id, organization_id, issuer_business_id, represented_client_id, document_type, document_number, document_status, issue_date, due_date, source_draft_id, totals_snapshot_json, pdf_render_status, pdf_asset_id')
    ...
    const hasAccountingPaymentWrite = (params.ctx.membership?.permissions ?? []).includes('accounting_base.payment.write');
    ...
    const finalizationDim = { state_key: 'open' };
    const collectionDim = {
