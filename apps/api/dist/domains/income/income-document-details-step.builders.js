...
function renderIncomePreviewHtml(params) {
    const p = params;
    const metaLine = (label, value) => value ? `<div><span class="k">${escapeHtml(label)}</span> ${escapeHtml(value)}</div>` : '';
    const partyLine = (label, value) => value ? `<div><span class="k">${escapeHtml(label)}</span> ${escapeHtml(value)}</div>` : '';
    return `
  ...
  <div class="section">
    ...
    <div class="totals">
      <div class="box">
        ...
        : ''}
        ${p.totals.vat
        ? `<div class="row"><span>${escapeHtml(p.totals.vat_label ?? 'מע״מ')}</span><span>${escapeHtml(p.totals.vat)}</span></div>`
        : ''}
