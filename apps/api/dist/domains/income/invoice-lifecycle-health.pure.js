...
function resolveHealthStateKey(warnings) {
 ...
 */
export function composeInvoiceLifecycleHealth(input) {
    const warnings = [];
    ...
    if (input.payment.state_key === 'paid' && input.collection.active) {
        warnings.push(warning({
            ...
            message: 'Accounting Base reports remaining_balance = 0 (paid), but an active invoice_collection_followup work item still exists.',
    ...
    if (input.delivery.state_key === 'sent' &&
        ...
        warnings.push(warning({
            ...
            message: 'At least one successful send exists; a later delivery attempt failed. Delivery state remains sent.',
