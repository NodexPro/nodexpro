...
async function persistWizardDraft(scope, draftId, patch, auditPayload, existingRow) {
    ...
    return { ...existing, ...patch, id: draftId, status: existing.status };
}
...
async function buildOverlayForDraft(scope, draftId, canEdit, rowOverride, docTypeOverride, stepOptions) {
    const row = rowOverride ?? (await loadWizardDraftRow(scope, draftId));
    const docType = docTypeOverride !== undefined
        ? docTypeOverride
...
async function validationForRow(scope, row, docType) {
    ...
    const totalsPreview = await computeDraftTotalsPreview(lines, 'ILS', settings, vatResolution, documentDate);
    ...
    const priorCache = row.draft_totals_preview_json &&
        ...
        : {};
    let document_number_preview = typeof priorCache.document_number_preview === 'string'
...
export async function resolveIncomeRecipientSelectedForDraft(scope, body, recipientOverlay = {}) {
    if (recipientOverlay.selected)
        return recipientOverlay.selected;
...
export async function deleteIncomeDocumentLine(scope, body) {
    ...
    const docType = await resolveDocType(scope, row.document_type);
    const serialized = serializeDraftLines(lines);
    return wizardDraftMutationOverlay(scope, draft_id, row, { ...row, draft_lines_json: serialized }, docType, { draft_lines_json: serialized }, { action: 'delete_line', line_id });
}
export async function reorderIncomeDocumentLines(scope, body) {
    ...
    const docType = await resolveDocType(scope, row.document_type);
    const serialized = serializeDraftLines(lines);
    return wizardDraftMutationOverlay(scope, draft_id, row, { ...row, draft_lines_json: serialized }, docType, { draft_lines_json: serialized }, { action: 'reorder_lines' });
}
