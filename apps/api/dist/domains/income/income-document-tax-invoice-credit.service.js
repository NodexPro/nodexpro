...
import { callConsumeIncomeTaxInvoiceCreditRpc, callReverseIncomeTaxInvoiceCreditConsumeRpc, } from '../accounting-base/accounting-base-customer-credit.service.js';
import { draftLinesFromIssuedSnapshot, serializeConversionDocumentSettings, serializeConvertedDraftLines, } from './income-document-conversion.pure.js';
import { applySourceDocumentDiscountNetToCreditDraftLines, CREDIT_AMOUNT_EXCEEDS_REMAINING_CREDITABLE, CREDIT_LINE_EXCEEDS_REMAINING_CREDITABLE, INCOME_COMMAND_BEGIN_TAX_INVOICE_CREDIT, INCOME_CREDIT_DOCUMENT_TYPE, creditConsumeLinesFromDraft, isIncomeTaxInvoiceCreditMode, parseIncomeTaxInvoiceCreditReasonKey, readCreditDraftSettings, resolveCanonicalCreditNoteAmount, resolveCreditNoteDraftDocumentSettings, resolveCreditState, sourceLineIdentityFromSnapshot, writeCreditDraftSettings, } from './income-document-tax-invoice-credit.pure.js';
import { loadIssuedCreditAmountsByInvoice } from './income-document-tax-invoice-credit.read.js';
export { buildTaxInvoiceCreditAction, loadCreditSourceReferenceForDocument, loadIssuedCreditAmountsByInvoice, loadIssuedCreditRowsForInvoices, } from './income-document-tax-invoice-credit.read.js';
async function loadSourceInvoice(orgId, documentId) {
    const { data, error } = await supabaseAdmin
        .from('income_documents')
        .select('id, organization_id, represented_client_id, issuer_business_id, document_type, document_status, document_number, income_customer_id, customer_snapshot_json, currency, language, notes, lines_snapshot_json, totals_snapshot_json, customer_po_reference, source_draft_id, legal_snapshot_json')
...
async function findCreditLinkByDraft(orgId, draftId) {
    const { data, error } = await supabaseAdmin
        .from('income_document_credit_links')
        .select('id, source_invoice_id, credit_draft_id, credit_document_id, status, credited_amount_reference, source_invoice_number, lines_json')
        .eq('organization_id', orgId)
...
async function findCreditLinkByIdempotency(orgId, key) {
    const { data, error } = await supabaseAdmin
        .from('income_document_credit_links')
        .select('id, source_invoice_id, credit_draft_id, credit_document_id, status, credited_amount_reference, source_invoice_number, lines_json')
        .eq('organization_id', orgId)
...
async function ensureCreditControl(params) {
    ...
    if (params.sourceLines.length === 0)
        return;
    const { error: lineErr } = await supabaseAdmin.from('income_invoice_credit_line_control').upsert(params.sourceLines.map((line) => ({
        source_invoice_id: params.invoice.id,
        organization_id: params.orgId,
...
function sourceLinesFromInvoice(invoice) {
...
export function assertCreditDraftIdentityLocked(params) {
    ...
    if (params.incomeCustomerId != null &&
...
export async function executeBeginIncomeTaxInvoiceCredit(ctx, body) {
    ...
    const reasonNote = body.reason_note != null && String(body.reason_note).trim() ? String(body.reason_note).trim() : null;
    ...
    if (!docType)
        throw badRequest('credit_tax_invoice is not available');
    ...
    const documentSettingsJson = writeCreditDraftSettings(serializeConversionDocumentSettings(documentSettings), creditSettings);
    const oneTimeSnapshot = !source.income_customer_id && source.customer_snapshot_json ? source.customer_snapshot_json : null;
...
async function buildCreditCommandResponse(params) {
    ...
    let workspace = await buildIncomeWorkspaceAggregate(params.ctx, resumed.scope, resumed.result.recipientOverlay, resumed.result.wizardOverlay);
...
export async function assertAndConsumeCreditOnIssue(params) {
    ...
    try {
        ...
        throw conflict(e instanceof Error ? e.message : 'הזיכוי חורג מהכמות/סכום הנותרים בשורת המקור', CREDIT_LINE_EXCEEDS_REMAINING_CREDITABLE);
    }
