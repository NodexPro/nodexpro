...
function assignIfDefined(target, patch) {
    for (const [k, v] of Object.entries(patch)) {
        if (v !== undefined)
            target[k] = v;
    }
}
function mergeOverviewPartial(result, patch) {
    if (patch.summary) {
        assignIfDefined(result.summary, patch.summary);
    }
    if (patch.charts) {
        if (patch.charts.new_clients_by_month)
            result.charts.new_clients_by_month = patch.charts.new_clients_by_month;
        if (patch.charts.filing_schedules)
            assignIfDefined(result.charts.filing_schedules, patch.charts.filing_schedules);
    }
    if (patch.operations) {
        if (patch.operations.clients_per_worker)
            result.operations.clients_per_worker = patch.operations.clients_per_worker;
        if (patch.operations.clients_per_worker_status)
            result.operations.clients_per_worker_status = patch.operations.clients_per_worker_status;
        if (patch.operations.report_deadlines)
            assignIfDefined(result.operations.report_deadlines, patch.operations.report_deadlines);
    }
...
export async function getDashboardOverviewAggregated(ctx) {
    const providers = getDashboardProviders();
