...
export async function ensureAppUser(authUserId, email, fullName) {
    ...
    const { data: created } = await supabaseAdmin
        ...
        .single();
    if (created)
        await writeAudit({ organizationId: null, actorUserId: created.id, entityType: 'user', action: AUDIT_ACTIONS.USER_CREATED, payload: { email } });
    return created?.id;
}
