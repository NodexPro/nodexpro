...
export function normalizeSmsDestination(raw) {
...
async function sendRecoverySmsE164(toE164, code) {
...
export async function verifyPlatformOwnerRecoveryOtp(params) {
    ...
    if (!id)
        throw badRequest('recovery_session_id required');
    const { data: row, error } = await supabaseAdmin
        ...
        .maybeSingle();
    if (error || !row)
        throw badRequest('Invalid recovery session');
    const now = Date.now();
...
export async function completePlatformOwnerPasswordReset(params) {
    ...
    const exp = new Date(String(row.expires_at)).getTime();
    if (now > exp)
        throw badRequest('Recovery session expired');
    if (row.used_at)
        throw badRequest('Recovery session already used');
    if (!row.otp_verified_at)
