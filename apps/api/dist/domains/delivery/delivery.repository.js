...
export class SupabaseDeliveryAttemptRepository {
    async insertAttempt(input) {
        ...
        if (!data)
            throw badRequest('Failed to create delivery attempt');
    ...
    async findById(organizationId, attemptId) {
        const { data, error } = await supabaseAdmin
            .from('delivery_attempts')
            .select('*')
            .eq('organization_id', organizationId)
...
export const defaultDeliveryAttemptRepository = new SupabaseDeliveryAttemptRepository();
