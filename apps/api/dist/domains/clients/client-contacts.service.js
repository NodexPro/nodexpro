...
export async function listContacts(ctx, orgId, clientId) {
    assertOrg(ctx, orgId);
...
export async function addContact(ctx, orgId, clientId, body) {
    assertOrg(ctx, orgId);
    ...
    const contactTexts = contacts.map((c) => [c.full_name, c.email, c.phone].filter(Boolean).join(' '));
...
export async function updateContact(ctx, orgId, clientId, contactId, body) {
    assertOrg(ctx, orgId);
    assertPermission(ctx, 'clients:write');
    const { data: client } = await supabaseAdmin.from('clients').select('id, display_name, legal_name, tax_id, external_code, email, phone').eq('id', clientId).eq('organization_id', orgId).single();
    if (!client)
    ...
    if (body.email !== undefined)
        updatePayload.email = body.email?.trim() ?? null;
    ...
    const { data: contact, error } = await supabaseAdmin
        ...
        .single();
    if (error || !contact)
        throw forbidden('Contact not found');
    const contacts = await listContacts(ctx, orgId, clientId);
    const contactTexts = contacts.map((c) => [c.full_name, c.email, c.phone].filter(Boolean).join(' '));
    await refreshClientSearchIndexWithContacts(orgId, clientId, buildClientSearchText(client), contactTexts);
    await addTimelineEvent({
...
export async function deleteContact(ctx, orgId, clientId, contactId) {
    assertOrg(ctx, orgId);
    assertPermission(ctx, 'clients:write');
...
export async function setPrimaryContact(ctx, orgId, clientId, contactId) {
    assertOrg(ctx, orgId);
    assertPermission(ctx, 'clients:write');
