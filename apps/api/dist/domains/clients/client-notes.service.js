...
export async function listNotes(ctx, orgId, clientId, options) {
    assertOrg(ctx, orgId);
...
export async function addNote(ctx, orgId, clientId, body) {
    assertOrg(ctx, orgId);
    ...
    if (!noteText)
        throw badRequest('note_text is required');
...
export async function viewSensitiveNote(ctx, orgId, clientId, noteId) {
    assertOrg(ctx, orgId);
    ...
    if (!note || !note.is_sensitive)
        throw forbidden('Note not found or not sensitive');
    await writeAudit({
        organizationId: orgId,
        actorUserId: ctx.user.id,
        entityType: 'client_note',
 ...
 */
export async function updateNote(ctx, orgId, clientId, noteId, body) {
    assertOrg(ctx, orgId);
    assertPermission(ctx, 'clients:write');
    const { data: client } = await supabaseAdmin.from('clients').select('id').eq('id', clientId).eq('organization_id', orgId).single();
    if (!client)
    ...
    if (body.is_sensitive !== undefined)
        updates.is_sensitive = body.is_sensitive;
    const { data: note, error } = await supabaseAdmin
        .from('client_notes')
        .update(updates)
        .eq('id', noteId)
    ...
    await writeAudit({
        ...
        payload: { client_id: clientId, is_sensitive: note.is_sensitive },
    });
    return note;
}
/**
 ...
 * Object-level: client and note must belong to org; note must belong to client.
 */
export async function deleteNote(ctx, orgId, clientId, noteId) {
    assertOrg(ctx, orgId);
    assertPermission(ctx, 'clients:write');
    const { data: client } = await supabaseAdmin.from('clients').select('id').eq('id', clientId).eq('organization_id', orgId).single();
    if (!client)
