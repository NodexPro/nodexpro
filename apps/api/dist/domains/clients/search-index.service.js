 ...
 */
export async function upsertClientSearchIndex(organizationId, clientId, searchText) {
 ...
 */
export async function refreshClientSearchIndexWithContacts(organizationId, clientId, clientSearchText, contactTexts) {
 ...
 */
export async function searchClients(organizationId, query, options = {}) {
    ...
    return (data ?? []).map((r) => ({ entityId: r.entity_id }));
}
/**
 * Search clients and return full client rows in one query (avoids N+1).
 */
export async function searchClientsWithData(organizationId, query, options = {}) {
