...
router.post('/:id/clients/bulk/export', authMiddleware, requireOrg, requirePermission('clients:write'), async (req, res, next) => {
    try {
        if (req.params.id !== req.context.organizationId)
            return res.status(403).json({ code: 'FORBIDDEN', message: 'Organization context required' });
        const clientIds = Array.isArray(req.body?.clientIds) ? req.body.clientIds.filter((id) => typeof id === 'string') : [];
...
router.post('/:id/clients/:clientId/tags', authMiddleware, requireOrg, requirePermission('clients:write'), async (req, res, next) => {
    try {
        ...
        if (!tagId)
            return res.status(400).json({ code: 'BAD_REQUEST', message: 'tagId required' });
