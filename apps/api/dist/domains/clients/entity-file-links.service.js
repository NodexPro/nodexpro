...
export async function listFilesForClient(ctx, orgId, clientId) {
    assertOrg(ctx, orgId);
...
export async function attachFileToClient(ctx, orgId, clientId, body) {
    assertOrg(ctx, orgId);
    ...
    if (fileAssetId) {
        ...
        if (!fileName)
            throw badRequest('file_name is required when file_asset_id is not provided');
        ...
        if (body.file_base64) {
            ...
            if (typeof body.file_base64 !== 'string')
                throw badRequest('Invalid file_base64');
 ...
 */
export async function assertCanAccessFileViaClient(ctx, orgId, fileAssetId) {
    assertOrg(ctx, orgId);
    assertPermission(ctx, 'clients:read');
    const { data: link } = await supabaseAdmin
        ...
        .single();
    if (!link)
        throw forbidden('File not linked to a client you can access');
 ...
 */
export async function getFileOpenUrl(ctx, orgId, clientId, fileAssetId) {
    assertOrg(ctx, orgId);
    assertPermission(ctx, 'clients:read');
    const { data: client } = await supabaseAdmin.from('clients').select('id').eq('id', clientId).eq('organization_id', orgId).single();
    if (!client)
    ...
    const { data: link } = await supabaseAdmin
        ...
        .single();
    if (!link)
        throw forbidden('File not linked to this client');
    const { data: fileAsset } = await supabaseAdmin
        .from('file_assets')
        .select('id, storage_key, organization_id')
        .eq('id', fileAssetId)
        .single();
    if (!fileAsset || fileAsset.organization_id !== orgId)
        throw forbidden('File not found');
    await ensureBucketExists();
 ...
 */
export async function removeFileFromClient(ctx, orgId, clientId, fileAssetId) {
    assertOrg(ctx, orgId);
    assertPermission(ctx, 'clients:write');
    const { data: client } = await supabaseAdmin.from('clients').select('id').eq('id', clientId).eq('organization_id', orgId).single();
    if (!client)
    ...
    const { data: link } = await supabaseAdmin
        ...
        .single();
    if (!link)
        throw forbidden('File link not found for this client');
    const { error } = await supabaseAdmin
        ...
        .eq('organization_id', orgId);
    if (error)
        throw new Error('Failed to remove file link');
    await addTimelineEvent({
        organizationId: orgId,
        entityType: ENTITY_TYPE_CLIENT,
        entityId: clientId,
