...
router.get('/:id/modules/state', authMiddleware, requireOrg, requirePermission('modules:read', 'subscriptions:read'), async (req, res, next) => {
    try {
        ...
        if (startMs)
            console.log(`[modules] GET /:id/modules/state org=${req.params.id} totalMs=${Date.now() - startMs}`);
...
router.post('/:id/modules/:moduleId/activate', authMiddleware, requireOrg, requirePermission('modules:write'), async (req, res, next) => {
    try {
        ...
        if (!result.success)
            return res.status(400).json({ code: 'ACTIVATION_BLOCKED', message: result.blockReason ?? 'Cannot activate' });
...
router.post('/:id/modules/:moduleId/select-plan', authMiddleware, requireOrg, requirePermission('modules:write'), async (req, res, next) => {
    try {
        if (req.params.id !== req.context.organizationId)
            return res.status(403).json({ code: 'FORBIDDEN', message: 'Organization context required' });
        const { modulePlanId } = req.body;
        if (!modulePlanId)
            return res.status(400).json({ code: 'BAD_REQUEST', message: 'modulePlanId required' });
