 ...
 */
export async function resolveEntitlement(organizationId, moduleId) {
    const mod = await supabaseAdmin.from('modules').select('id, is_system').eq('id', moduleId).single();
    if (!mod.data)
        return { status: 'not_entitled', reason: 'Module not found' };
...
export function isSessionEnabledModuleEntitlementStatus(status) {
...
export async function resolveEntitlementsForOrganization(organizationId, moduleIds) {
    const map = new Map();
