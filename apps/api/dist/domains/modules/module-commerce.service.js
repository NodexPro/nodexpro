...
export async function listPlansForModule(moduleId) {
...
export async function getOrgModuleSubscription(organizationId, moduleId) {
    ...
    if (!sub)
        return null;
    const plan = supabaseEmbedOne(sub.module_plans);
    if (!plan)
        return null;
    return {
        id: sub.id,
...
export async function selectPlan(ctx, organizationId, moduleId, modulePlanId) {
    ...
    if (!plan.data)
        throw badRequest('Invalid plan for this module');
...
export async function changePlan(ctx, organizationId, moduleId, modulePlanId) {
    if (ctx.organizationId !== organizationId)
        throw forbidden('Organization context required');
    const mod = await supabaseAdmin.from('modules').select('id, code, is_system').eq('id', moduleId).single();
    if (!mod.data)
    ...
    const { data: sub } = await supabaseAdmin
        ...
        .single();
    if (!sub)
        throw badRequest('No subscription to change');
    await supabaseAdmin
        .from('organization_module_subscriptions')
