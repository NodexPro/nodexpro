 ...
 */
import { buildInvoiceFamilySectionModel, buildInvoiceWidgetCatalogGroups, INVOICE_TEMPLATE_FAMILY, } from './owner-document-invoice-family.adapter.pure.js';
import { getOwnerDocumentTemplateRegistryEntry, listOwnerDocumentTemplateFamilies, resolveActiveOwnerDocumentTemplateFamily, } from './owner-document-template-registry.pure.js';
...
export function buildOwnerDocumentPlatformAggregateSlice(params) {
    ...
    if (!registry || registry.status !== 'active') {
        ...
        const section_model = buildInvoiceFamilySectionModel(params.definition, invoice.section_labels);
        return {
            ...
            note: 'Requested template family is registered but not yet active. Showing Invoice foundation.',
        };
    }
    const section_model = buildInvoiceFamilySectionModel(params.definition, registry.section_labels);
    return {
        platform_aggregate_key: OWNER_DOCUMENT_PLATFORM_AGGREGATE_KEY,
