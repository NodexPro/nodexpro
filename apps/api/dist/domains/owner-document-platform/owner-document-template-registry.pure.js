 ...
 */
import { buildInvoiceFamilyWidgetCatalog, INVOICE_SECTION_LABELS, INVOICE_TEMPLATE_FAMILY, } from './owner-document-invoice-family.adapter.pure.js';
import { OWNER_DOCUMENT_TEMPLATE_FAMILIES } from './owner-document-platform.types.js';
const PENDING_NOTE = 'Registered for Universal Document Platform. Layout seed/widgets activate in a later phase.';
function pendingFamily(family, label) {
    return {
...
function buildInvoiceRegistryEntry() {
    return {
...
export function listOwnerDocumentTemplateFamilies() {
    seedDefaultRegistry();
...
export function getOwnerDocumentTemplateRegistryEntry(family) {
    seedDefaultRegistry();
    const key = String(family ?? '').trim();
    return REGISTRY.get(key) ?? null;
}
export function resolveActiveOwnerDocumentTemplateFamily(requested) {
    seedDefaultRegistry();
    const key = String(requested ?? '').trim();
    const entry = REGISTRY.get(key);
    if (entry?.status === 'active')
        return entry.template_family;
 ...
 */
export function registerOwnerDocumentTemplateFamily(entry) {
    seedDefaultRegistry();
    if (!OWNER_DOCUMENT_TEMPLATE_FAMILIES.includes(entry.template_family)) {
...
export function resetOwnerDocumentTemplateRegistryForTests() {
    REGISTRY.clear();
    seedDefaultRegistry();
}
