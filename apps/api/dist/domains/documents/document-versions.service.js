...
export async function listVersions(ctx, orgId, documentId) {
    assertOrg(ctx, orgId);
...
export async function getDocumentOpenUrl(ctx, orgId, documentId, versionId) {
    assertOrg(ctx, orgId);
    assertPermission(ctx, 'documents:read');
    ...
    if (versionId) {
        const { data: v } = await supabaseAdmin
            ...
            .single();
        if (!v)
            throw forbidden('Version not found');
        versionRow = v;
    }
    else {
        const versionIdToUse = doc.current_version_id;
        if (!versionIdToUse)
            throw badRequest('Document has no current version');
        const { data: v } = await supabaseAdmin
            .from('document_versions')
            .select('id, file_asset_id')
            ...
            .single();
        if (!v)
            throw forbidden('Current version not found');
        versionRow = v;
    }
