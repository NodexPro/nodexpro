...
import { buildAccountingCategoriesWorkspaceAggregate, buildAccountingEntryDetailsAggregate, buildAccountingEntriesWorkspaceAggregate, buildAccountingPeriodsWorkspaceAggregate, buildAccountingSummaryWorkspaceAggregate, } from './accounting-base-read-models.service.js';
...
function reqString(payload, key) {
    const val = String(payload[key] ?? '').trim();
    if (!val)
        throw badRequest(`${key} required`);
    return val;
}
function reqNumber(payload, key) {
    const num = Number(payload[key]);
    if (!Number.isFinite(num))
        throw badRequest(`${key} required`);
    return num;
}
function reqLinkTargetType(payload, key) {
    const raw = reqString(payload, key);
    const allowed = ['document', 'client', 'module_entity', 'other', 'accounting_entry'];
    if (!allowed.includes(raw)) {
        throw badRequest(`Unsupported ${key}: ${raw}`);
    }
    return raw;
}
async function audit(ctx, organizationId, entityType, entityId, action, payload) {
    await writeAudit({
        organizationId,
...
async function assertLinkTargetTenantSafe(organizationId, targetType, targetId) {
    if (targetType === 'client') {
        const { data } = await supabaseAdmin
            ...
            .maybeSingle();
        if (!data)
            throw badRequest('Invalid link target: client not found in organization');
        return;
    }
    if (targetType === 'document') {
        const { data } = await supabaseAdmin
            ...
            .maybeSingle();
        if (!data)
            throw badRequest('Invalid link target: document not found in organization');
        return;
    }
    if (targetType === 'accounting_entry') {
        const { data } = await supabaseAdmin
            ...
            .maybeSingle();
        if (!data)
            throw badRequest('Invalid link target: accounting entry not found in organization');
        return;
    }
    ...
    if (!targetId.trim())
        throw badRequest('target_entity_id required');
}
async function handleCreatePeriod(ctx, organizationId, payload) {
...
async function handleClosePeriod(ctx, organizationId, payload) {
    requirePermission(ctx, PERMISSIONS.PERIOD_MANAGE);
    const periodId = reqString(payload, 'period_id');
    const period = await forCommandGetPeriod(ctx, organizationId, periodId);
    if (period.status !== 'locked')
        throw conflict('Only locked period can be closed');
...
async function ensurePeriodWritableForEntry(ctx, organizationId, periodId) {
    const period = await forCommandGetPeriod(ctx, organizationId, periodId);
    if (period.status === 'closed')
        throw conflict('Closed period cannot be mutated');
}
async function handleCreateEntry(ctx, organizationId, payload) {
    ...
    const created = await forCommandCreateEntry(ctx, organizationId, {
        ...
        direction: reqString(payload, 'direction'),
...
async function handleUpdateDraftEntry(ctx, organizationId, payload) {
    requirePermission(ctx, PERMISSIONS.ENTRY_WRITE);
    const entryId = reqString(payload, 'entry_id');
    const entry = await forCommandGetEntry(ctx, organizationId, entryId);
    if (entry.posting_state !== 'draft')
        throw conflict('Only draft entry can be updated');
...
async function handleFinalizeEntry(ctx, organizationId, payload) {
    requirePermission(ctx, PERMISSIONS.ENTRY_WRITE);
    const entryId = reqString(payload, 'entry_id');
    const entry = await forCommandGetEntry(ctx, organizationId, entryId);
    if (entry.posting_state !== 'draft')
        throw conflict('Only draft entry can be finalized');
    await ensurePeriodWritableForEntry(ctx, organizationId, entry.period_id);
...
async function handleArchiveEntry(ctx, organizationId, payload) {
    requirePermission(ctx, PERMISSIONS.ENTRY_WRITE);
    const entryId = reqString(payload, 'entry_id');
    const entry = await forCommandGetEntry(ctx, organizationId, entryId);
    if (entry.status === 'archived')
        throw conflict('Entry already archived');
    await ensurePeriodWritableForEntry(ctx, organizationId, entry.period_id);
...
async function handleCreateCategory(ctx, organizationId, payload) {
...
async function handleUpdateCategory(ctx, organizationId, payload) {
    requirePermission(ctx, PERMISSIONS.CATEGORY_MANAGE);
...
async function handleDeactivateCategory(ctx, organizationId, payload) {
    requirePermission(ctx, PERMISSIONS.CATEGORY_MANAGE);
    const categoryId = reqString(payload, 'category_id');
    await forCommandGetCategory(ctx, organizationId, categoryId);
...
async function handleLinkEntryToEntity(ctx, organizationId, payload) {
...
async function handleUnlinkEntryFromEntity(ctx, organizationId, payload) {
    requirePermission(ctx, PERMISSIONS.LINK_MANAGE);
    ...
    if (linkRow?.accounting_entry_id) {
        return {
            ...
            entryId: linkRow.accounting_entry_id,
        };
    }
    return { primary: 'accounting_entries_workspace_aggregate' };
}
async function handleRecomputeSummary(ctx, organizationId, payload) {
    requirePermission(ctx, PERMISSIONS.SUMMARY_RECOMPUTE);
    const period_id = payload.period_id == null || String(payload.period_id).trim() === '' ? undefined : String(payload.period_id).trim();
...
async function buildRefreshedEnvelope(ctx, organizationId, target) {
    ...
    if (target.primary === 'accounting_entry_details_aggregate') {
        if (!target.entryId)
            throw badRequest('entry_id required for entry details aggregate');
...
export async function executeAccountingBaseCommand(ctx, organizationId, body) {
    ...
    else if (type === 'lock_period')
        refreshTarget = await handleLockPeriod(ctx, organizationId, payload);
    else if (type === 'close_period')
        refreshTarget = await handleClosePeriod(ctx, organizationId, payload);
    else if (type === 'create_entry')
        refreshTarget = await handleCreateEntry(ctx, organizationId, payload);
    else if (type === 'update_draft_entry')
        refreshTarget = await handleUpdateDraftEntry(ctx, organizationId, payload);
    else if (type === 'finalize_entry')
        refreshTarget = await handleFinalizeEntry(ctx, organizationId, payload);
    else if (type === 'archive_entry')
        refreshTarget = await handleArchiveEntry(ctx, organizationId, payload);
    else if (type === 'create_category')
        refreshTarget = await handleCreateCategory(ctx, organizationId, payload);
    else if (type === 'update_category')
        refreshTarget = await handleUpdateCategory(ctx, organizationId, payload);
    else if (type === 'deactivate_category')
        refreshTarget = await handleDeactivateCategory(ctx, organizationId, payload);
    else if (type === 'link_entry_to_entity')
        refreshTarget = await handleLinkEntryToEntity(ctx, organizationId, payload);
    else if (type === 'unlink_entry_from_entity')
        refreshTarget = await handleUnlinkEntryFromEntity(ctx, organizationId, payload);
    else if (type === 'recompute_summary')
        refreshTarget = await handleRecomputeSummary(ctx, organizationId, payload);
    else
        throw badRequest(`Unknown accounting base command type: ${type}`);
    const refreshed = await buildRefreshedEnvelope(ctx, organizationId, refreshTarget);
    const additional_refreshed = [];
    if (refreshTarget.primary === 'accounting_entries_workspace_aggregate' &&
        refreshTarget.entryId) {
        additional_refreshed.push(await buildRefreshedEnvelope(ctx, organizationId, {
            primary: 'accounting_entry_details_aggregate',
