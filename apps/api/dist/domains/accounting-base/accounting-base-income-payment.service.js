...
import { ACCOUNTING_BASE_COMMAND_RECORD_AND_ALLOCATE_INCOME_PAYMENT, ACCOUNTING_BASE_INCOME_PAYMENT_CASE_KEY, ACCOUNTING_BASE_PAYMENT_WRITE_PERMISSION, incomePaymentMethodLabel, isSupportedIncomePaymentDocumentType, parseIncomePaymentMethodKey, resolveIncomeInvoiceOriginalAmount, resolveIncomeInvoicePaymentState, roundMoney2, } from './accounting-base-income-payment.pure.js';
import { buildIncomeInvoicePaymentCaseAggregate, } from './accounting-base-income-payment-case.read.js';
export const ACCOUNTING_BASE_RECORD_AND_ALLOCATE_INCOME_PAYMENT_RPC = 'accounting_base_record_and_allocate_income_payment';
...
function reqIsoDate(value, field) {
    const s = String(value ?? '').trim();
    if (!/^\d{4}-\d{2}-\d{2}$/.test(s))
        throw badRequest(`${field} must be YYYY-MM-DD`);
    return s;
}
function reqPositiveAmount(value) {
    const n = Number(value);
    if (!Number.isFinite(n) || n <= 0)
        throw badRequest('amount must be a positive number');
    return roundMoney2(n);
}
function reqCurrency(value) {
    const s = String(value ?? '').trim().toUpperCase();
    if (!/^[A-Z]{3}$/.test(s))
        throw badRequest('currency must be a 3-letter code');
    return s;
}
async function loadIncomeDocument(organizationId, incomeDocumentId) {
    const { data, error } = await supabaseAdmin
        .from('income_documents')
        .select('id, organization_id, issuer_business_id, represented_client_id, document_type, document_number, document_status, currency, issue_date, due_date, totals_snapshot_json, customer_snapshot_json')
...
async function auditFailure(ctx, organizationId, payload) {
    try {
        await writeAudit({
            organizationId,
...
function mapAtomicPaymentRpcError(err) {
...
export async function callRecordAndAllocateIncomePaymentRpc(input) {
    const { data, error } = await supabaseAdmin.rpc(ACCOUNTING_BASE_RECORD_AND_ALLOCATE_INCOME_PAYMENT_RPC, {
...
export async function executeRecordAndAllocateIncomePayment(ctx, organizationId, payload) {
    ...
    const referenceNumber = payload.reference_number == null || String(payload.reference_number).trim() === ''
        ? null
    ...
    try {
        ...
        const credited = (await loadIssuedCreditAmountsByInvoice(organizationId, [doc.id])).get(doc.id) ?? 0;
        ...
        if (!rpcResult.replay) {
            ...
            const eventType = stateAfter.payment_state_key === 'paid'
            ...
            if (meta) {
                await writeAudit({
                    ...
                    entityId: doc.id,
                    action: eventType === INCOME_WORK_EVENTS_DEFERRED[0]
