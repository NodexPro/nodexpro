...
export async function buildAccountingEntriesWorkspaceAggregate(ctx, organizationId) {
...
export async function buildAccountingEntryDetailsAggregate(ctx, organizationId, entryId) {
    const agg = await getAccountingBaseRefreshedAggregate(ctx, organizationId);
    const perms = getPermissions(ctx);
...
export async function buildAccountingPeriodsWorkspaceAggregate(ctx, organizationId) {
    const agg = await getAccountingBaseRefreshedAggregate(ctx, organizationId);
    const perms = getPermissions(ctx);
...
export async function buildAccountingCategoriesWorkspaceAggregate(ctx, organizationId) {
    const agg = await getAccountingBaseRefreshedAggregate(ctx, organizationId);
    const perms = getPermissions(ctx);
    const rows = agg.categories.map((c) => {
        ...
        return {
            ...
            status: c.status,
            ownership: c.is_system ? 'system' : 'organization',
...
export async function buildAccountingSummaryWorkspaceAggregate(ctx, organizationId) {
    const agg = await getAccountingBaseRefreshedAggregate(ctx, organizationId);
    const perms = getPermissions(ctx);
    const categories = categoryMap(agg.categories);
    const periods = periodMap(agg.periods);
    const totalsByDirectionMap = new Map();
...
export async function buildAccountingBaseWorkspaceAggregates(ctx, organizationId) {
