...
import { buildOwnerInvoiceLayoutIssueFreezeFromPublished, } from './owner-invoice-document-layout-issue-freeze.pure.js';
...
import { assertOwnerInvoiceLayoutVersionMutable, moveOwnerInvoiceLayoutField, moveOwnerInvoiceLayoutSection, placeOwnerInvoiceLayoutField, planPublishOwnerInvoiceLayoutVersion, resizeOwnerInvoiceLayoutSection, setOwnerInvoiceFieldVisibility, setOwnerInvoiceSectionLock, setOwnerInvoiceTableColumn, } from './owner-invoice-document-layout-mutations.pure.js';
...
import { OWNER_INVOICE_LAYOUT_AGGREGATE_KEY, OWNER_INVOICE_LAYOUT_COMMANDS, OWNER_INVOICE_LAYOUT_DOCUMENT_TYPE_GROUPS, OWNER_INVOICE_LAYOUT_KEY_DEFAULT, isOwnerInvoiceLayoutCommand, } from './owner-invoice-document-layout.types.js';
...
function asDocumentTypeGroup(value) {
    const v = typeof value === 'string' && value.trim() ? value.trim() : 'all';
    if (!OWNER_INVOICE_LAYOUT_DOCUMENT_TYPE_GROUPS.includes(v)) {
...
function mapRow(raw) {
    return {
        ...
        document_type_group: raw.document_type_group,
        ...
        based_on_version_id: raw.based_on_version_id ?? null,
...
async function auditLayout(ctx, action, entityId, payload) {
    await writeAudit({
        organizationId: null,
...
async function loadVersionById(id) {
    ...
    if (!data)
        throw notFound('Owner invoice layout version not found');
    return mapRow(data);
}
function pickWorkingVersion(versions) {
...
const OWNER_BUILDER_WIDTH_SPAN_PRESETS = [3, 4, 6, 8, 9, 12];
const OWNER_BUILDER_ALIGNMENTS = ['start', 'center', 'end', 'stretch'];
function sectionConstraints(definition) {
    return definition.sections.map((s) => {
        ...
        return {
            ...
            width_percent_labels: Object.fromEntries(allowed_col_spans.map((span) => [
                String(span),
                span === 3
                    ? '25%'
                    : span === 4
...
export async function buildOwnerInvoiceDocumentBuilderAggregate(ctx, opts) {
    ...
    const country_code = opts?.country_code === undefined ? 'IL' : asOptionalString(opts.country_code);
    ...
    let working = opts?.version_id != null && String(opts.version_id).trim()
    ...
    if (!working && versions.length === 0) {
        ...
        const builder = buildOwnerInvoiceBuilderZones({
            ...
            section_labels: platform.section_labels ?? INVOICE_SECTION_LABELS,
        });
        return {
            ...
            notes: {
                legacy_policy: 'Issued documents without owner_layout_version_id/snapshot keep exact legacy render path.',
                vat_boundary: 'VAT rates are Owner Legal Control / Country Pack only — never in layout JSON.',
                builder_zones: 'builder_zones are Owner visual-editor chrome only — never in Preview/PDF/issued HTML.',
                universal_platform: 'INV-13C: Builder is Universal Document Platform; invoice is first active template family.',
            },
        };
    }
    if (!working)
...
async function refreshed(ctx, payload, versionId) {
    return {
        aggregate_key: OWNER_INVOICE_LAYOUT_AGGREGATE_KEY,
        aggregate: await buildOwnerInvoiceDocumentBuilderAggregate(ctx, {
            ...
            country_code: payload.country_code === undefined ? 'IL' : asOptionalString(payload.country_code),
...
async function nextVersionNumber(params) {
    ...
    const max = Number(data?.[0]?.version_number ?? 0);
    return max + 1;
}
async function persistDefinition(ctx, version, definition, auditAction, command, payload) {
...
async function handleCreateDraft(ctx, payload) {
    ...
    const country_code = payload.country_code === undefined ? 'IL' : asOptionalString(payload.country_code);
    ...
    const template_family = resolveActiveOwnerDocumentTemplateFamily(asOptionalString(payload.template_family));
    const { data, error } = await supabaseAdmin
        .from('owner_invoice_document_layout_versions')
        .insert({
        layout_key,
...
async function handlePublish(ctx, payload) {
...
async function mutateDraft(ctx, payload, command, auditAction, mutator) {
    const version_id = asString(payload.version_id, 'version_id');
    const version = await loadVersionById(version_id);
    const next = mutator(version.layout_definition_json);
    return persistDefinition(ctx, version, next, auditAction, command, payload);
}
export async function executeOwnerInvoiceLayoutCommand(ctx, command, payload) {
    try {
        assertPlatformOwner(ctx);
    }
    catch (error) {
    ...
    switch (command) {
        ...
        case OWNER_INVOICE_LAYOUT_COMMANDS.move_section:
            return mutateDraft(ctx, payload, command, AUDIT_ACTIONS.OWNER_INVOICE_LAYOUT_SECTION_MOVED, (def) => moveOwnerInvoiceLayoutSection(def, {
        ...
        case OWNER_INVOICE_LAYOUT_COMMANDS.resize_section:
            return mutateDraft(ctx, payload, command, AUDIT_ACTIONS.OWNER_INVOICE_LAYOUT_SECTION_RESIZED, (def) => resizeOwnerInvoiceLayoutSection(def, {
                section_key: asString(payload.section_key, 'section_key'),
                ...
                alignment: payload.alignment == null
                    ? undefined
                    : asString(payload.alignment, 'alignment'),
            }));
        case OWNER_INVOICE_LAYOUT_COMMANDS.move_field:
            return mutateDraft(ctx, payload, command, AUDIT_ACTIONS.OWNER_INVOICE_LAYOUT_FIELD_MOVED, (def) => moveOwnerInvoiceLayoutField(def, {
        ...
        case OWNER_INVOICE_LAYOUT_COMMANDS.place_field:
            return mutateDraft(ctx, payload, command, AUDIT_ACTIONS.OWNER_INVOICE_LAYOUT_FIELD_PLACED, (def) => placeOwnerInvoiceLayoutField(def, {
                field_key: asString(payload.field_key, 'field_key'),
                section_key: asString(payload.section_key, 'section_key'),
                order: payload.order == null ? undefined : Number(payload.order),
            }));
        case OWNER_INVOICE_LAYOUT_COMMANDS.set_section_lock:
            return mutateDraft(ctx, payload, command, AUDIT_ACTIONS.OWNER_INVOICE_LAYOUT_SECTION_LOCK_SET, (def) => setOwnerInvoiceSectionLock(def, {
                section_key: asString(payload.section_key, 'section_key'),
                owner_locked: Boolean(payload.owner_locked),
            }));
        case OWNER_INVOICE_LAYOUT_COMMANDS.set_field_visibility:
            return mutateDraft(ctx, payload, command, AUDIT_ACTIONS.OWNER_INVOICE_LAYOUT_FIELD_VISIBILITY_SET, (def) => setOwnerInvoiceFieldVisibility(def, {
                field_key: asString(payload.field_key, 'field_key'),
                visible: Boolean(payload.visible),
            }));
        case OWNER_INVOICE_LAYOUT_COMMANDS.set_table_column:
            return mutateDraft(ctx, payload, command, AUDIT_ACTIONS.OWNER_INVOICE_LAYOUT_TABLE_COLUMN_SET, (def) => setOwnerInvoiceTableColumn(def, {
                column_key: asString(payload.column_key, 'column_key'),
                visible: payload.visible == null ? undefined : Boolean(payload.visible),
                width_px: payload.width_px === undefined
                    ? undefined
                    : payload.width_px == null
                        ? null
                        : Number(payload.width_px),
                order: payload.order == null ? undefined : Number(payload.order),
                align: payload.align == null
                    ? undefined
                    : asString(payload.align, 'align'),
            }));
...
export async function resolveActivePublishedOwnerInvoiceLayoutFreeze(params) {
    ...
    return buildOwnerInvoiceLayoutIssueFreezeFromPublished(data);
}
/** Tenant users must never call Owner layout mutations. */
export function rejectTenantOwnerInvoiceLayoutMutation() {
