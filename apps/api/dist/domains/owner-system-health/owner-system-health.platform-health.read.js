 ...
 */
import { buildSystemHealthRowId, resolveSystemHealthIssue, } from './owner-system-health.pure.js';
import { loadDeliveryFailureGroups, loadIncomePdfFailuresByOrg, loadWorkEventFailureGroups, pingDatabase, } from './owner-system-health.shared-read.js';
...
export async function loadPlatformHealthRows(lastCheckedAt) {
    ...
    const deliveryLastSeen = deliveryGroups.reduce((max, g) => {
        if (!g.last_seen_at)
            return max;
        if (!max || g.last_seen_at > max)
            return g.last_seen_at;
        return max;
    }, null);
    if (deliveryCount > 0) {
    ...
    const eventLastSeen = workEventGroups.reduce((max, g) => {
        if (!g.last_seen_at)
            return max;
        if (!max || g.last_seen_at > max)
            return g.last_seen_at;
    ...
    const intakeUnsupported = workEventGroups.filter((g) => g.sample_reason?.includes('Unsupported schema_version'));
    ...
    for (const group of deliveryGroups.slice(0, 20)) {
        const issue = resolveSystemHealthIssue(group.sample_reason?.toLowerCase().includes('timeout') ? 'smtp_timeout' : 'delivery_failed', group.sample_reason);
        legacyRows.push({
