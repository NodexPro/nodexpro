...
import { uploadSharedClientFileAssetForOffice, uploadSharedClientFileAssetForPortal, } from '../domains/file-access/shared-client-file-upload.service.js';
import { assertClientBelongsToOrg, assertDocflowEntitled, assertDocflowMessageScope, assertDocflowThreadScope, reqString, } from '../domains/docflow/docflow.guards.js';
...
officeBaseRouter.get('/aggregates/floating-widget', async (req, res, next) => {
    try {
...
router.post('/internal/scheduler/run-daily', async (req, res, next) => {
    try {
...
officeRouter.get('/aggregates/client-tab', async (req, res, next) => {
    try {
        const ctx = req.context;
        const orgId = ctx.organizationId;
...
officeRouter.get('/aggregates/communication-rule-run-review', async (req, res, next) => {
    try {
        const ctx = req.context;
        const orgId = ctx.organizationId;
...
officeRouter.get('/aggregates/invites-management', async (req, res, next) => {
    try {
        const ctx = req.context;
        const orgId = ctx.organizationId;
...
async function handleOfficeCommand(req, res, next, command) {
    try {
        const ctx = req.context;
        const out = await executeDocflowOfficeCommand(ctx, command, req.body?.payload ?? req.body ?? {});
        res.json(out);
        return;
    }
    catch (e) {
...
officeRouter.post('/commands/invite-client', async (req, res, next) => handleOfficeCommand(req, res, next, 'invite_client_to_docflow'));
officeRouter.post('/commands/revoke-client-portal-access', async (req, res, next) => handleOfficeCommand(req, res, next, 'revoke_client_portal_access'));
officeRouter.post('/commands/invite-selected-clients', async (req, res, next) => handleOfficeCommand(req, res, next, 'invite_selected_clients_to_docflow'));
officeRouter.post('/commands/invite-all-clients', async (req, res, next) => handleOfficeCommand(req, res, next, 'invite_all_clients_to_docflow'));
officeRouter.post('/commands/resend-invite', async (req, res, next) => handleOfficeCommand(req, res, next, 'resend_invite'));
officeRouter.post('/commands/revoke-invite', async (req, res, next) => handleOfficeCommand(req, res, next, 'revoke_invite'));
officeRouter.post('/commands/issue-invite-delivery', async (req, res, next) => handleOfficeCommand(req, res, next, 'issue_docflow_invite_delivery'));
officeRouter.post('/commands/create-client-thread', async (req, res, next) => handleOfficeCommand(req, res, next, 'create_client_thread'));
officeRouter.post('/commands/change-thread-status', async (req, res, next) => handleOfficeCommand(req, res, next, 'change_thread_status'));
officeRouter.post('/commands/assign-thread-to-user', async (req, res, next) => handleOfficeCommand(req, res, next, 'assign_thread_to_user'));
...
officeRouter.post('/commands/create-docflow-document-request', async (req, res, next) => handleOfficeCommand(req, res, next, 'create_docflow_document_request'));
...
officeRouter.get('/files/:fileAssetId/open', async (req, res, next) => {
    try {
        const ctx = req.context;
        const orgId = ctx.organizationId;
        const clientId = String(req.query.client_id ?? '').trim();
        ...
        if (!fileAssetId)
            throw badRequest('fileAssetId is required');
...
officeRouter.post('/commands', async (req, res, next) => {
    try {
        ...
        if (command === 'run_communication_rule' ||
...
portalRouter.post('/commands/accept-invitation', async (req, res, next) => {
    try {
...
portalRouter.post('/files/upload', async (req, res, next) => {
    try {
        const rawToken = String(req.headers['x-client-portal-session'] ?? req.body?.portal_session_token ?? '').trim();
        if (!rawToken)
            throw badRequest('portal session token is required');
...
portalRouter.get('/aggregates/client-portal-inbox', async (req, res, next) => {
    try {
...
portalRouter.get('/files/:fileAssetId/open', async (req, res, next) => {
    try {
        const rawToken = String(req.headers['x-client-portal-session'] ?? req.query.portal_session_token ?? '').trim();
        if (!rawToken)
            throw badRequest('portal session token is required');
...
async function handlePortalCommand(req, res, next, command) {
    try {
        const payload = (req.body?.payload ?? req.body ?? {});
        ...
        const out = await executeDocflowPortalCommand(command, payload);
        res.json(out);
        return;
    }
    catch (e) {
