...
import { completePlatformOwnerPasswordReset, requestPlatformOwnerRecovery, verifyPlatformOwnerRecoveryOtp, } from '../domains/auth/platform-owner-password-recovery.service.js';
const router = Router();
router.post('/request', rateLimit({
    windowMs: 15 * 60 * 1000,
    max: 5,
...
router.post('/verify', rateLimit({
    ...
    try {
        const recovery_session_id = typeof req.body?.recovery_session_id === 'string' ? req.body.recovery_session_id : '';
...
router.post('/complete', rateLimit({
    windowMs: 15 * 60 * 1000,
    max: 10,
    keyGenerator: (req) => req.ip ?? 'unknown',
}), async (req, res, next) => {
