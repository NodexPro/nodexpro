-- Single office unread definition (matches getUnreadForOffice in docflow-read-models.service.ts):
-- published client messages, created_by_type <> 'office', after last thread_read_marked_office
-- for that org+client+thread; threads not archived; clients scoped to org.

create or replace function public.docflow_office_unread_messages_total(p_org_id uuid)
returns bigint
language sql
stable
as $$
  ...
  inner join public.client_message_threads t
    on t.id = m.thread_id and t.org_id = m.org_id and t.client_id = m.client_id
...
create or replace function public.docflow_office_unread_messages_for_clients(
  p_org_id uuid,
  p_client_ids uuid[]
)
returns table (
...
as $$
  with ids as (
    select distinct unnest(p_client_ids) as cid
  ),
  agg as (
    select
      t.client_id as cid,
    ...
    inner join public.clients c on c.id = t.client_id and c.organization_id = p_org_id
    inner join public.client_messages m
      on m.thread_id = t.id and m.org_id = t.org_id and m.client_id = t.client_id
    where t.org_id = p_org_id
      and t.thread_status <> 'archived'
      and t.client_id in (select cid from ids)
      and m.message_status = 'published'
      and m.created_by_type <> 'office'
      and m.created_at > coalesce(
        (
  ...
  select i.cid, coalesce(a.cnt, 0)::bigint
  from ids i
  left join agg a on a.cid = i.cid;
$$;
...
as $$
  select
    ...
    public.docflow_office_unread_messages_total(p_org_id) as unread_replies_count,
    (
      select count(*)::bigint
      from public.client_message_threads t
      inner join public.clients c on c.id = t.client_id and c.organization_id = p_org_id
          ...
          where m2.thread_id = t.id
            ...
            and m2.created_at > coalesce(
              (
                ...
                where e2.org_id = m2.org_id
                  and e2.client_id = m2.client_id
                  and e2.thread_id = m2.thread_id
...
revoke all on function public.docflow_office_unread_messages_total(uuid) from public;
grant execute on function public.docflow_office_unread_messages_total(uuid) to service_role;

revoke all on function public.docflow_office_unread_messages_for_clients(uuid, uuid[]) from public;
grant execute on function public.docflow_office_unread_messages_for_clients(uuid, uuid[]) to service_role;
