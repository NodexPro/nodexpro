-- Batched end-customer document stats for invoices-tab dual population aggregate.
-- Scoped by (represented office client, income_customer_id). No N+1.
-- Mirrors unpaid/credit rules from 163 for office-client stats.

create or replace function public.income_client_document_management_end_customer_stats(
  p_org_id uuid
)
returns table (
...
as $$
  with office_issued as (
    ...
    where d.organization_id = p_org_id
      ...
      and d.income_customer_id is not null
  ...
  issued_agg as (
    ...
    where oi.client_id is not null
      and oi.income_customer_id is not null
    group by oi.client_id, oi.income_customer_id
  ),
  office_drafts as (
    select
      d.income_customer_id,
  ...
  draft_agg as (
    ...
    where client_id is not null
      and income_customer_id is not null
    group by client_id, income_customer_id
  ),
  pair_ids as (
    select client_id, income_customer_id from issued_agg
    union
    select client_id, income_customer_id from draft_agg
  )
  select
    p.client_id as represented_client_id,
    p.income_customer_id,
  ...
  from pair_ids p
  left join issued_agg i
    on i.client_id = p.client_id
   and i.income_customer_id = p.income_customer_id
  left join draft_agg d
    on d.client_id = p.client_id
   and d.income_customer_id = p.income_customer_id;
$$;

grant execute on function public.income_client_document_management_end_customer_stats(uuid) to service_role;

comment on function public.income_client_document_management_end_customer_stats(uuid) is
  'Invoices dual-population: document counters + AB unpaid remaining per (represented_client_id, income_customer_id).';
